- high prio:
    ...
    - bin_diff for input, send older input? <- does it really make sense without prediction margin
    ...
    - dragging dummy with weak hook => no feet animation
    - render order involving tee hands still wrong
    - render order with hook still wrong
    - merge https://github.com/ddnet/ddnet/pull/8544
    - events client prediction & automatic recognition if out of sync with server
    - config bool values don't accept string true/false
    - account server: maybe split email steam id etc from account table (and login tokens then too)
    ...
    - laser projectile is not created directly
    ...
    - ingame sound is still wrong, spartial not working as desired.
