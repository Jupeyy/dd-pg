- high prop:
    - vk pipeline can drop without cleanup (no wrapper yet)
    - vk subrender pass pipeline creation needs a threaded AllAtOnce loading
    - consider naga_oil's module imports for code cleanup
    - vk testing: use the plugin interface to check command correctness (?)
    - vk pipeline cache needs to respect driver version
    - msaa & switching passes need testing and thinking
    - uploading objects to wasm module uses wasm func call
        (rewrite to change linear memory)
    - stencil buffer doesn't respect alpha
    - console ideas:
    ...
    - vk completely compile pipelines async to rendering? (a.k.a. disable rendering during that period)
    ...
    - benchmark frame time (for vk uploading etc.) -- done, doesn't help a lot, but maybe a tiny bit
