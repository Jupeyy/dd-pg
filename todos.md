- high prop:
    - editor (blue prints, coop)
        ...
        - tune layer should have tune zones as property
        - shift + tile brush => repeating insert
    - dummy controls
    - dummy connect ui (as mini screne etc.)
    - video rendering (too hard for now with pure rust libs?)
    - assets download from server (http server), layered containers?
    - language id for client
    - use download resource server ip, instead of localhost hardcoded
    - demos
    - console
    - file hashes, capital or lowercase?
    - quic disallow "migration"? (server config)
    - bsdiff crate is ass slow, that sadly won't scale well :/
