...
pub struct PredictionTiming {
    ...
    pub(crate) smooth_adjustment_time: f64,
...
impl PredictionTiming {
    ...
    /// 1ms: assumed frame time jitter
    pub const PREDICTION_MARGIN_FRAME_TIME: Duration = Duration::from_millis(1);
    /// overhead by the network implementation.
    /// 4ms: 2ms + 2ms overhead of the network implementation on the server and on the client.
    pub const PREDICTION_MARGIN_NETWORK: Duration = Duration::from_millis(4);
    ...
    pub fn max_frametime(&self) -> Duration {
        *self.frame_time_max.iter().max().unwrap()
    }
    ...
    pub fn add_packet_stats(&mut self, time_stamp: Duration, packets_sent: u64, packets_lost: u64) {
    ...
    pub fn add_snap(&mut self, snap_diff: f64, timestamp: Duration) {
            ...
            || lag_weight.is_some()
        {
            ...
            self.smooth_adjustment_time = adjust_factor;
        } else {
    ...
    pub fn smooth_adjustment_time(&mut self) -> f64 {
        ...
        let scale = if self.smooth_adjustment_time < 0.0 {
            1.0 // 10.0
        } else {
            1.0
        };
        ...
        let res = (self.smooth_adjustment_time / fps) * scale;
        self.smooth_adjustment_time -= res;
        res
    }
}
...
mod test {
    ...
    fn jitter_tests() {
        ...
        for i in 0..snap_off + 100 {
            ...
            let smooth_adjustment_time = timer.smooth_adjustment_time;
    ...
    fn jitter_tests_test(
        ...
        let mut time = cur_time - timer.pred_max_smooth();
        ...
        for i in 0..snap_count {
            ...
            let time_snap = snaps[i].0;
            let time_client = time_snap.as_secs_f64() - time.as_secs_f64();
            let time_server = snaps[i].1 as f64 * 1.0 / snaps_per_sec as f64;
