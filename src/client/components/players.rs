...
use client_render::{
    ...
    map::{render_pipe::Camera, render_tools::RenderTools},
    render::{
...
use crate::{
    ...
    render::weapons::{WeaponGunSpec, WeaponHammerSpec},
};
...
impl Players {
    ...
    pub fn render(&mut self, pipe: &mut PlayerRenderPipe) {
        ...
        // hook (head, chain)
        // -> hand
        let ticks_per_second = pipe.game.game_tick_speed();
        ...
        for player_render_info in self.render_info_helper.drain(..) {
            ...
            if render_hook {
                // current hook
                let cur_hook = pipe.hooks.get_or_default(
                    "TODO:",
                    pipe.graphics,
                    pipe.fs,
                    pipe.io_batcher,
            ...
            */
                ...
                let mut weapon_pos =
                    render_pos + dir * gun_spec.offset_x - dir * recoil as f32 * 10.0;
            ...
            let skin = pipe.skins.get_or_default(
                "TODO:",
                pipe.graphics,
                pipe.fs,
                pipe.io_batcher,
