...
impl Players {
    pub fn new(graphics: &mut Graphics) -> Self {
        ...
        (0..NUM_WEAPONS).for_each(|wi| {
            ...
            let mut quad = SQuad::new()
                .from_width_and_height_centered(width, height)
                .with_color(&ubvec4::new(255, 255, 255, 255));
            let offset_normal = if weapon == WeaponType::Ninja {
                graphics.quad_container_handle.quad_container_add_quads(
                    &quad_container_index,
                    &[quad.from_width_and_height_centered(160.0 * scale_x, 160.0 * scale_y)],
                )
            } else {
                graphics
                    .quad_container_handle
            ...
            let offset_flipped = if weapon == WeaponType::Ninja {
                graphics.quad_container_handle.quad_container_add_quads(
                    &quad_container_index,
                    &[quad.from_width_and_height_centered(160.0 * scale_x, 160.0 * scale_y)],
                )
        ...
        let hook_chain_quad_offset = graphics
            .quad_container_handle
            .quad_container_add_quads(&quad_container_index, &[quad]);
    ...
    pub fn render(&mut self, pipe: &mut PlayerRenderPipe) {
        ...
        for player_render_info in self.render_info_helper.drain(..) {
            ...
            if render_hook {
                // current hook
                let cur_hook = pipe.hooks.get_or_default("TODO:", pipe.graphics);
            ...
            let weapon_texture = pipe.weapons.get_or_default("TODO:", pipe.graphics);
            ...
            */
            ...
            let skin = pipe.skins.get_or_default("TODO:", pipe.graphics);
