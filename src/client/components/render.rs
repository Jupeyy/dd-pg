...
use graphics::graphics::{Graphics, GraphicsQuadContainerInterface, QuadContainerBuilder, SQuad};
use graphics_render_traits::{GraphicsRenderGeometry, GraphicsRenderQuadContainer};
use graphics_types::{rendering::ColorRGBA, types::QuadContainerIndex};
use math::math::{
...
use crate::{
    ...
    containers::{ctf::CTFContainer, pickups::PickupContainer},
};

use super::{particle_manager::ParticleManager, players::get_weapon_as_quad};
...
impl Render {
    pub fn new(sys: &System, graphics: &mut Graphics) -> Self {
        ...
        let ctf_flag_offset = graphics.quad_container_add_quads(&items_quad_container, &[quad]);
        ...
        let pickup_sprite_off = graphics.quad_container_add_quads(&items_quad_container, &[quad]);
        ...
        (0..WeaponType::NumWeapons as usize)
            ...
            .for_each(|(index, wi)| {
                ...
                let offset_normal =
                    graphics.quad_container_add_quads(&items_quad_container, &[quad]);
        ...
        let quad = *SQuad::new()
            .from_width_and_height_centered(32.0 * sprite_scale.0, 32.0 * sprite_scale.1)
            .with_color(&ubvec4::new(255, 255, 255, 255))
            .with_uv_from_points(&vec2::new(0.0, 0.0), &vec2::new(1.0, 1.0));

        let projectile_sprite_off =
            graphics.quad_container_add_quads(&items_quad_container, &[quad]);
        ...
        let particle_splat_off = graphics.quad_container_add_quads(&items_quad_container, &[quad]);

        graphics.quad_container_upload(&items_quad_container);
    ...
    pub fn destroy(self, graphics: &mut Graphics) {
        graphics.delete_quad_container(self.items_quad_container);
    }

    pub fn render(&mut self, pipe: &mut RenderPipe) {
        let intra_tick_ratio = intra_tick_time_to_ratio(pipe.client_data.intra_tick_time);
    ...
    pub fn render_projectile(
        ...
        if ty == WeaponType::Grenade {
            ...
            quad_scope.set_rotation(
                (pipe.sys.time_get_nanoseconds().as_secs_f32() as f64 * PId * 2.0 * 2.0
                    + ItemID as f64/* TODO: <- ? */) as f32,
            );
        } else {
    ...
    pub fn render_pickup(&mut self, pipe: &mut RenderPipe, pickup: &PickupRenderInfo) {
        let ty = pickup.ty;
        let CurWeapon = pickup.weapon;
        let QuadOffset = 2;
        let angle = 0.0;

        let intra_tick = pipe.client_data.intra_tick_time;
        let intra_tick_ratio = intra_tick_time_to_ratio(pipe.client_data.intra_tick_time);
    ...
    pub fn render_flag(&mut self, pipe: &mut RenderPipe, flag: &FlagRenderInfo) {
        let angle = 0.0;
        let size = 42.0;
    ...
    pub fn render_laser(&mut self, pipe: &mut RenderPipe, pCurrent: &LaserRenderInfo) {
        let ty = pCurrent.ty;

        let mut rgb: ColorRGBA = Default::default();
        let from = pCurrent.from;
        let pos = pCurrent.pos;
        ...
        if laser_len > 0.0 {
            ...
            let intra_tick_ratio = intra_tick_time_to_ratio(pipe.client_data.intra_tick_time);
            let ticks = (pipe.cur_tick - pCurrent.start_tick) as f32 + intra_tick_ratio as f32;

            let ms = (ticks / 50.0) * 1000.0;
