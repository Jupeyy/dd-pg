...
use base_io::io::IO;
use client_containers::{ctf::CTFContainer, pickups::PickupContainer, weapons::WeaponContainer};
use client_map::client_map::ClientMapFile;
use client_render_base::{
    map::render_pipe::Camera, render::canvas_mapping::map_canvas_for_ingame_items,
};
use graphics::{
    graphics::Graphics,
    handles::quad_container::QuadContainer,
    quad_container::Quad,
    streaming::{quad_scope_begin, DrawScopeImpl},
};
use graphics_types::{
...
use super::{effects::Effects, particle_manager::ParticleManager, players::get_weapon_as_quad};
...
impl Render {
    ...
    pub fn render_projectile(
        ...
        if ty == WeaponType::Grenade {
            let mut effects = Effects::new(pipe.particle_manager);
            ...
            let mut effects = Effects::new(pipe.particle_manager);
            effects.bullet_trail(&pos, alpha, 0.0);
        ...
        match ty {
            ...
            WeaponType::Shotgun => quad_scope.set_texture(&weapon.shotgun.projectiles[0]),
            WeaponType::Grenade => quad_scope.set_texture(&weapon.grenade.projectiles[0]),
            WeaponType::Laser => todo!(),
            WeaponType::Ninja => todo!(),
        }
    ...
    pub fn render_pickup(
        ...
        let pickup_tex = pipe.pickup_container.get_or_default("default");
    ...
    pub fn render_flag(
        ...
        let ctf_tex = pipe.ctf_container.get_or_default("default");
    ...
    pub fn render_laser(
        ...
        if laser_len > 0.0 {
            ...
            let mut a = ms / 125.0; // TODO: m_pClient->GetTunes(TuneZone).m_LaserBounceDelay;
            a = a.clamp(0.0, 1.0);
            let ia = 1.0 - a;

            // do outline
            let mut quad_scope = pipe.graphics.stream_handle.quads_begin();
            quad_scope.set_state(base_state);
            quad_scope.set_colors_from_single(outer_color.r, outer_color.g, outer_color.b, alpha);
            let mut out = vec2::new(dir.y, -dir.x) * (7.0 * ia);

            quad_scope.quads_draw_tl(&[StreamedQuad::from_free_form(
                from.x - out.x,
                from.y - out.y,
                from.x + out.x,
                from.y + out.y,
