...
use client_types::chat::{MsgSystem, ServerMsg};
use client_types::killfeed::KillfeedMsg;
use client_ui::connecting::user_data::ConnectMode;
...
use shared_base::network::messages::{NetworkStr, WeaponType};
use shared_base::{
        ...
        match self.cur_client_connection_state {
            ClientConnectionState::Connecting => {
                ...
                if pipe.map.is_fully_loaded() {
                    let mut player_info = MsgObjPlayerInfo::explicit_default();
                    if let Some(p) = pipe.config.players.get(0) {
                        player_info.name = NetworkStr::from(&p.name).unwrap();
                        player_info.clan = NetworkStr::from(&p.clan).unwrap();
                        // TODO:
                        player_info.skin_body.name = NetworkStr::from(&p.skin.name).unwrap();
                    }
                    pipe.network
    ...
    pub fn on_msg(
        ...
        match msg {
                ...
                if let Some((_, game)) = pipe.map.try_get_data_and_game_mut() {
                    if game.convert_to_game_state(&snapshot) {
                        ...
                        self.all_players_info_helper
                            ...
                            .for_each(|(_, info)| {
                                pipe.skin_container
                                    .get_or_default(info.skin_body.name.as_str());
                            });
            ...
            ServerToClientMessage::QueueInfo(info) => {
                *pipe.client_data.connect_info.borrow_mut() = ConnectMode::Queue { msg: info };
            ...
            ServerToClientMessage::Chat(chat_msg) => {
                if let Some((_, game)) = pipe.map.try_get_data_and_game_mut() {
                    pipe.client_data
                        ...
                        .push_back(ServerMsg::Chat(super::chat::from_net_msg(
                            game,
                            chat_msg.msg,
                            &pipe.string_pool,
                        )));
