        ...
        match msg {
            ...
            ServerToClientMessage::Snapshot {
                ...
                let snapshot = if let Some(diff_id) = diff_id {
                    pipe.client_data
                        ...
                        .map(|old| {
                            ...
                            snapshot.clear();
                            let patch_res = bsdiff::patch(old, &mut std::io::Cursor::new(&*patch), &mut snapshot);
                    ...
                    .is_some_and(|id| id >= snap_id)
                {
                    ...
                    // set local players
                    pipe.client_data
                        .handle_local_players_from_snapshot(local_players);
                    ...
                    let time_in_pred = Duration::from_nanos(
                        (predict_max.as_nanos() % tick_time.as_nanos()) as u64,
                    );
