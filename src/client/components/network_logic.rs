...
use crate::{
    client::component::{
        ...
        ComponentLoadable, ComponentRenderable, ComponentUpdatable, ComponentUpdatePipe,
        GameMsgPipeline,
    },
...
impl ComponentLoadable for NetworkLogic {
    ...
    fn destroy(self, _pipe: &mut crate::client::component::ComponentDestroyPipe) {}
}
...
impl ComponentGameMsg for NetworkLogic {
    fn on_msg(
        ...
        match msg {
            ...
            ServerToClientMessage::Snapshot((overhead_time, snap)) => {
                if let Some((_, game)) = pipe.map.try_get_data_and_game_mut() {
                    if game.convert_to_game_state(&snap) {
                        // set local players
                        snap.local_players.iter().for_each(|c| {
                            if !pipe.client_data.local_players.contains_key(&c.game_el_id) {
                                ...
                                pipe.client_data
                                    .local_players
                                    .insert(c.game_el_id.clone(), local_player);
                            }
                            // sort
                            pipe.client_data.local_players.to_back(&c.game_el_id);
                        });
                        ...
                        let ticks_to_catch_up = ping_nanos / TIME_UNTIL_TICK.as_nanos() as u64;
                        let time_over_tick = ping_nanos % TIME_UNTIL_TICK.as_nanos() as u64;
                        ...
                        pipe.skin_container.update(pipe.graphics);
                    }
                }
            }
            ...
            ServerToClientMessage::QueueInfo(_) => todo!(),
        }
    }
