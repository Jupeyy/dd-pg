...
use pool::{
    datatypes::{PoolBTreeMap, PoolVecDeque, StringPool},
...
use super::{
    client::ClientPlayerInputPerTick, component::GameMsgPipeline,
    components::network_logic::NetworkLogic, input::input_handling::DeviceToLocalPlayerIndex,
};
        ...
        *self = match selfi {
            ...
            Game::Loading(LoadingGame {
                ...
                if map.is_fully_loaded() {
                    let player_info = if let Some(p) = config_game.players.first() {
                        NetworkCharacterInfo {
                            name: NetworkString::new(&p.name).unwrap(),
                            clan: NetworkString::new(&p.clan).unwrap(),
                            country: NetworkString::new(&p.country).unwrap(),
                            skin: NetworkResourceKey::from_str_lossy(&p.skin.name),

                            skin_info: if p.skin.custom_colors {
                                NetworkSkinInfo::Custom {
                                    body_color: p.skin.body_color.into(),
                                    feet_color: p.skin.feet_color.into(),
                                }
                            } else {
                                NetworkSkinInfo::Original
                            },

                            weapon: NetworkResourceKey::from_str_lossy(&p.weapon),
                            freeze: NetworkResourceKey::from_str_lossy(&p.freeze),
                            ninja: NetworkResourceKey::from_str_lossy(&p.ninja),
                            game: NetworkResourceKey::from_str_lossy(&p.game),
                            ctf: NetworkResourceKey::from_str_lossy(&p.ctf),
                            hud: NetworkResourceKey::from_str_lossy(&p.hud),
                            entities: NetworkResourceKey::from_str_lossy(&p.entities),
                            emoticons: NetworkResourceKey::from_str_lossy(&p.emoticons),
                            particles: NetworkResourceKey::from_str_lossy(&p.particles),
                            hook: NetworkResourceKey::from_str_lossy(&p.hook),
                        }
                    } else {
