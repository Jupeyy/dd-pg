...
use game_interface::{
    ...
    types::{
        character_info::NetworkCharacterInfo, network_string::NetworkString,
...
use shared_base::{
    ...
    network::messages::{GameModification, MsgClReady, RenderModification},
};
use shared_network::{
...
use ui_base::{types::UiState, ui::UiCreator};
use url::Url;
    ...
    pub fn update(
        ...
        *self = match selfi {
            ...
            Game::PrepareConnect(PrepareConnectGame {
                ...
                if task.as_ref().is_none_or(|task| task.is_finished()) && dicts_task.is_finished() {
                    Self::connect(
                        &connect_info,
                        &graphics,
                        &graphics_backend,
            ...
            Game::Loading(LoadingGame {
                ...
                if map.is_fully_loaded() {
                    ...
                    if let Err(err) = map.err() {
                        connect_info.set(ConnectModes::ConnectingErr { msg: err });
                    }
                    Self::Loading(LoadingGame {
                        network,
                        game_event_generator_client,
            ...
            Game::Connecting(connecting) => match msg {
                ServerToClientMessage::ServerInfo { info, overhead } => {
                    ...
                    let render_props = RenderGameCreateOptions {
                        ...
                        render_mod: RenderModTy::render_mod(&info.render_mod, config_game),
                    };
                    *self = Self::load(
                        &connecting.graphics,
