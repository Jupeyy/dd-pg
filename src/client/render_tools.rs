...
use graphics_base::streaming::{rotate, DrawScopeImpl, GraphicsStreamHandleInterface};
...
impl RenderTools {
        ...
        F: Fn(&mut RenderPipeline, i32, i32, &mut ColorRGBA),
    {
        ...
        let mut draw_quads = pipe.graphics.stream_handle.quads_tex_3d_begin();
    ...
    {
        let mut draw_triangles = pipe.graphics.stream_handle.triangles_begin();
    ...
    pub fn render_eval_envelope(
        ...
        let tile_millis =
            (time_nanos / std::time::Duration::from_millis(1).as_nanos() as u32).as_millis() as u64;
        for i in 0..num_points as usize - 1 {
            if tile_millis >= points[i].time as u64 && tile_millis <= points[i + 1].time as u64 {
