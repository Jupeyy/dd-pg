...
pub struct STileLayerVisuals {
    ...
    pub width: u32,
    pub height: u32,
    pub buffer_container_index: BufferContainerIndex,
    pub is_textured: bool,
}

impl STileLayerVisuals {
    pub fn new() -> Self {
        Default::default()
    }
...
pub struct ClientMapBufferUploadData {
    pub tile_layer_uploads: Vec<(usize, &'static mut [u8], bool, usize)>,
    pub quad_layer_uploads: Vec<(usize, &'static mut [u8], bool, usize)>,
}
...
impl ClientMapBuffered {
    ...
    fn map_destroy(&mut self, graphics: &mut Graphics) {
        //clear everything and destroy all buffers
        if !self.tile_layer_visuals.is_empty() {
            let s = self.tile_layer_visuals.len();
            for i in 0..s {
                graphics.DeleteBufferContainer(
                    &mut self.tile_layer_visuals[i].buffer_container_index,
                    true,
                );
            }
            self.tile_layer_visuals.clear();
        }
        if !self.quad_layer_visuals.is_empty() {
            let s = self.quad_layer_visuals.len();
            for i in 0..s {
                graphics.DeleteBufferContainer(
                    &mut self.quad_layer_visuals[i].buffer_container_index,
                    true,
                );
            }
            self.quad_layer_visuals.clear();
        }
    }
    ...
    pub fn prepare_upload(
        ...
        for g in 0..map.NumGroups() as usize {
            ...
            for l in 0..group.num_layers as usize {
                ...
                if let MapLayer::Tile(_) = layer {
                    ...
                    while cur_overlay < overlay_count + 1 {
                        ...
                        let tile_layer_visuals = &mut self.tile_layer_visuals;
                        let visual_index = tile_layer_visuals.len();
                        tile_layer_visuals.push(STileLayerVisuals::default());
                    ...
                    let quad_layer_visuals = &mut self.quad_layer_visuals;
                    let visual_index = quad_layer_visuals.len();
                    quad_layer_visuals.push(SQuadLayerVisuals::default());
        ...
        upload_data
            .tile_layer_uploads
            .resize_with(self.tile_layer_visuals.len(), || Default::default());

        self.tile_layer_visuals
            .par_iter_mut()
            .zip(upload_data.tile_layer_uploads.par_iter_mut())
            .enumerate()
            .for_each(|(index, (visuals, upload_data))| {
                ...
                if let MapLayer::Tile(tile_map) = layer {
                            ...
                            if is_entity_layer {
                                ...
                                if is_tele_layer {
                                    if let MapTileLayerDetail::Tele(tele_tiles) = &tile_map.1 {
                                        ...
                                        flags = 0;
                                        if cur_overlay == 1 {
                                            if index != TileNum::TILE_TELECHECKIN as u8
                                                && index != TileNum::TILE_TELECHECKINEVIL as u8
                                            {
                    ...
                    visuals.buffer_container_index = BufferContainerIndex::None;
        ...
        upload_data
            .quad_layer_uploads
            .resize_with(self.quad_layer_visuals.len(), || Default::default());

        self.quad_layer_visuals
            .par_iter_mut()
            .zip(upload_data.quad_layer_uploads.par_iter_mut())
            .enumerate()
            .for_each(|(index, (_q_layer_visuals, upload_data))| {
    ...
    pub fn upload_map(
        ...
        upload_data.tile_layer_uploads.drain(..).for_each(
            |(visual_index, raw_data, textured, indices_count)| {
                ...
                let buffer_object_index =
                    graphics.create_buffer_object(raw_data.len(), raw_data, 0);

                // then create the buffer container
                let mut container_info = SBufferContainerInfo::default();
                container_info.stride = if textured {
                    std::mem::size_of::<f32>() * 2 + std::mem::size_of::<vec3>()
                } else {
                    0
                };
                container_info.vert_buffer_binding_index = buffer_object_index;
                ...
                self.tile_layer_visuals[visual_index].buffer_container_index =
                    graphics.CreateBufferContainer(&container_info);
                // and finally inform the backend how many indices are required
                graphics.IndicesNumRequiredNotify(indices_count);
            },
        );
        ...
        upload_data.quad_layer_uploads.drain(..).for_each(
            |(visual_index, raw_data, textured, indices_count)| {
                ...
                self.quad_layer_visuals[visual_index].buffer_container_index =
                    graphics.CreateBufferContainer(&container_info);
                // and finally inform the backend how many indices are required
                graphics.IndicesNumRequiredNotify(indices_count);
            },
