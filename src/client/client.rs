...
use game_interface::{
    client_commands::{ClientCameraMode, ClientCommand},
...
use super::{
    components::client_stats::{ClientStats, ClientStatsRenderPipe, DebugHudRenderPipe},
        ...
        if let Game::Active(game) = &mut self.game {
            ...
            let mut render_game_input = RenderGameInput {
                ...
                settings: RenderGameSettings {
                    ...
                    map_sound_volume: self.config.game.snd.map_sound_volume
                        * self.config.game.snd.global_volume,
                    pixels_per_point: self.graphics.canvas_handle.window_pixels_per_point()
                        * self.config.game.cl.ingame_ui_scale,
                },
                ext: game_state.collect_render_ext(),
            };

            type CharacterInfos = PoolLinkedHashMap<CharacterId, CharacterInfo>;
            type StageRenderInfos = PoolLinkedHashMap<StageId, StageRenderInfo>;
            let mut fill_for_player = {
                |client_player: (&PlayerId, &mut ClientPlayer),
                 ...
                 -> (PlayerId, RenderGameForPlayer) {
                    ...
                    let (cam_mode, force_scoreboard_visible) = match character_info
                        .and_then(|c| c.player_info.as_ref())
                    {
                        Some(info) => (
                            match info.cam_mode {
                                PlayerCameraMode::Default => RenderPlayerCameraMode::Default,
                                PlayerCameraMode::Free => RenderPlayerCameraMode::AtPos(vec2::new(
                                ...
                                PlayerCameraMode::LockedOn(character_id) => {
                                    RenderPlayerCameraMode::OnCharacter {
                                        character_id,
                        ...
                        None => (RenderPlayerCameraMode::Default, false),
                    };
                    (
                        player_id,
                        RenderGameForPlayer {
                ...
                for player_event in player_events {
                    match player_event {
                        ...
                        PlayerFeedbackEvent::SpectatorSelection(ev) => match ev {
                            ...
                            SpectatorSelectionEvent::Selected(spectated_character) => {
                                game.map.game.client_command(
                                    &player_id,
                                    ClientCommand::SetCameraMode(ClientCameraMode::Normal(Some(
                                        spectated_character,
                                    ))),
                                );
                                game.network.send_unordered_to_server(
                                    &GameMessage::ClientToServer(ClientToServerMessage::PlayerMsg(
                                        (
                                            player_id,
                                            ClientToServerPlayerMessage::SwitchToCamera(
                                                ClientCameraMode::Normal(Some(spectated_character)),
                                            ),
                                        ),
                                    )),
                                );
                ...
                for ui_event in ui_events {
                    match ui_event {
                        UiEvent::StartLocalServer => {
                            start_local_server(&self.sys, self.shared_info.clone());
                        }
    ...
    fn run(&mut self, native: &mut dyn NativeImpl) {
        ...
        if let Game::Active(game) = &mut self.game {
            ...
            if has_input {
                ...
                let show_cursor = player.is_some_and(|(_, p)| p.emote_wheel_active);
            ...
            fn apply_input(
                ...
                tick_inps: &mut LinkedHashMap<u64, PoolLinkedHashMap<PlayerId, PlayerInput>>,
