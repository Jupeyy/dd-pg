    ...
    fn render_game(&mut self, native: &mut dyn NativeImpl) {
        if let Game::Active(game) = &mut self.client {
                        ...
                        *voted,
                        v.remaining_time.saturating_sub(
                            self.cur_time
                                ...
                                .saturating_sub(game.game_data.prediction_timing.ping_average()),
                        ),
                    )
                }),
                character_infos,
                    ...
                    *requires_scoreboard |= client_player.show_scoreboard;
                    ...
                    RenderGameForPlayer {
                        render_for_player: RenderForPlayer {
                            ...
                            emote_wheel_info: if client_player.emote_wheel_active && !is_menu_open {
                                Some((
                                    client_player.emote_wheel_ty.take(),
                                    self.inp_manager.clone_inp().egui,
                                ))
                            } else {
                                None
            ...
            if self.config.game.cl.dummy.mini_screen {
                if let Some(player) = render_game_input.players.get_mut(0) {
                    let dummies = game
                        .game_data
                        .local_players
                        .iter()
                    ...
                    player
                        .observed_players
                        .extend(dummies.map(|(&player_id, _)| ObservedPlayer::Dummy {
                            local_player_info:
                                game_state.collect_character_local_render_info(&player_id),
                            player_id,
            ...
            for (player_id, player_events) in res.player_events {
                for player_event in player_events {
                    match player_event {
                        ...
                        PlayerFeedbackEvent::EmoteWheel(ev) => match ev {
                            EmoteWheelEvent::EmoticonSelected(_) => todo!(),
                            EmoteWheelEvent::EyeSelected(_) => todo!(),
                        },
                    }
                }
            }
            ...
            if self.debug_hud.ui.ui_state.is_ui_open {
                self.debug_hud.render(&mut DebugHudRenderPipe {
                    ...
                    prediction_timing: &game.game_data.prediction_timing,
...
impl FromNativeLoadingImpl<ClientNativeLoadingImpl> for ClientNativeImpl {
    fn new(
        ...
        let client_stats = ClientStats::new(&loading.sys, &creator);
...
impl FromNativeImpl for ClientNativeImpl {
    fn run(&mut self, native: &mut dyn NativeImpl) {
        ...
        if let Game::Active(game) = &mut self.client {
            ...
            game.game_data.prediction_timing.add_frametime(
            ...
            game.game_data.last_game_tick = Duration::from_secs_f64(
                (game.game_data.last_game_tick.as_secs_f64()
                    + game.game_data.prediction_timing.smooth_adjustment_time())
                .clamp(0.0, f64::MAX),
            );
        }
