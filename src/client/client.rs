...
pub fn ddnet_main(mut sys: System, cert: &[u8], shared_info: Arc<ServerInfo>) {
    ...
    let mut graphics_backend = GraphicsBackend::new();
...
struct ClientNativeImpl {
    ...
    _entities_container: EntitiesContainer, // TODO:
    ...
    graphics: Graphics,
    graphics_backend: Rc<RefCell<GraphicsBackend>>,
}
...
impl FromNativeLoadingImpl<ClientNativeLoadingImpl> for ClientNativeImpl {
    fn new(mut loading: ClientNativeLoadingImpl, native: &mut dyn NativeImpl) -> Self {
        ...
        let default_hud = HudContainer::load("default", &loading.fs, &loading.io_batcher);
        ...
        let graphics_backend = Rc::new(RefCell::new(loading.graphics_backend));
        ...
        let skin_container = SkinContainer::new(default_skin, &mut graphics);
        let weapon_container = WeaponContainer::new(default_weapon, &mut graphics);
        let hook_container = HookContainer::new(default_hook, &mut graphics);
        let ctf_container = CTFContainer::new(default_ctf, &mut graphics);
        let pickup_container = PickupContainer::new(default_pickup, &mut graphics);
        let entities_container = EntitiesContainer::new(default_entities, &mut graphics);
        let hud_container = HudContainer::new(default_hud, &mut graphics);
...
impl FromNativeImpl for ClientNativeImpl {
    ...
    fn resized(&mut self, native: &mut dyn NativeImpl, new_width: u32, new_height: u32) {
        let window_props = self.graphics_backend.borrow_mut().resized(
