...
use ui_wasm_manager::{UIWasmLoadingType, UIWasmManagerBase};
use winit::{
    event::WindowEvent,
...
use shared_base::{
    ...
    network::{
        messages::{MsgClAddLocalPlayer, MsgClChatMsg, MsgClInput},
...
use super::{
    components::client_stats::{ClientStats, ClientStatsRenderPipe},
    game_events::{GameEventPipeline, GameEventsClient},
    input::input_handling::{InputHandling, InputHandlingEvent},
};

type UIWasmManager = UIWasmManagerBase<Config>;
                ...
                for player_event in player_events {
                    match player_event {
                        ...
                        ChatEvent::PlatformOutput(output) => {
                            self.inp_manager
                                .handle_platform_output(native.borrow_window(), output);
                        }
                    }
                }
            }
        ...
        if let Some(editor) = &mut self.editor {
            ...
            if let Some(demo_player) = &mut self.demo_player {
                ...
                if let Some(output) = self.ui_manager.render(
                    ...
                    &mut self.graphics,
                    &self.graphics_backend,
                    &mut UIPipe::new(self.sys.time_get_nanoseconds(), &mut self.config),
                ...
                for ui_event in ui_events {
                    match ui_event {
                        ...
                        UiEvent::Connect { addr } => {
                            self.client =
                                Game::new(&self.io, &self.connect_info, &self.cert, &addr).unwrap();
                        }
                        UiEvent::Disconnect => {
...
impl FromNativeLoadingImpl<ClientNativeLoadingImpl> for ClientNativeImpl {
    fn new(
        ...
        benchmark.bench("loading ui font files");
        ...
        let menu_map = ClientMapLoading::new(
            &format!(
                "../../themes/{}",
                loading.config_engine.ui.menu_background_map
            ),
            None,
            None,
            &io,
...
impl FromNativeImpl for ClientNativeImpl {
    fn run(&mut self, native: &mut dyn NativeImpl) {
        ...
        if let Game::Active(game) = &mut self.client {
                        ...
                        .send_unordered_to_server(&GameMessage::ClientToServer(
                            ClientToServerMessage::PlayerMsg((
                                ...
                                ClientToServerPlayerMessage::Input(MsgClInput {
                                    ...
                                    diff_id: local_player.server_input_id,
            ...
            let mut pred_inps = game.player_inputs_pool.new();
            ...
            game.client_data.last_game_tick = Duration::from_secs_f64(
                (game.client_data.last_game_tick.as_secs_f64()
                    + game.client_data.prediction_timing.smooth_time())
                .clamp(0.0, f64::MAX),
            );
        }
