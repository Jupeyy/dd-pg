...
impl LoadSkin {
    fn get_blood_color(body_img: &[u8], body_width: usize, body_height: usize) {
        let pixel_step = 4;
    ...
    fn grey_scale(
        ...
        left_eyes_outline: &mut [PngResultPersistent; TeeSkinEye::Count as usize],

        right_eyes: &mut [PngResultPersistent; TeeSkinEye::Count as usize],
        right_eyes_outline: &mut [PngResultPersistent; TeeSkinEye::Count as usize],
    ) {
        let pixel_step = 4;
        // create grey scales
        ...
        left_eyes_outline.iter_mut().for_each(|tex| {
            Self::make_grey_scale(tex);
        });
        ...
        right_eyes_outline.iter_mut().for_each(|tex| {
            Self::make_grey_scale(tex);
        });
    ...
    pub async fn load_skin(&mut self, fs: &FileSystem, skin_name: &str) -> anyhow::Result<()> {
        ...
        self.metrics.body.from_texture(
            &self.body_outline.data,
            self.body_outline.width,
            0,
            0,
            self.body_outline.width,
            self.body_outline.height,
        );

        self.metrics.feet.from_texture(
            &self.left_foot.data,
            self.left_foot.width,
            0,
            0,
            self.left_foot.width,
            self.left_foot.height,
        );
        self.metrics.feet.from_texture(
            &self.left_foot_outline.data,
            self.left_foot_outline.width,
            0,
            0,
            self.left_foot_outline.width,
            self.left_foot_outline.height,
        );
    ...
    fn convert(self, graphics: &mut Graphics) -> Skin {
        Skin {
            ...
            left_eyes: self
                .left_eyes
                .iter()
                .map(|eye| Self::load_file_into_texture(graphics, eye.clone(), &self.skin_name))
