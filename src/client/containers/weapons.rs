...
impl Weapon {
    fn destroy(mut self, graphics: &mut Graphics) {
        graphics.unload_texture(self.tex);
        graphics.unload_texture(self.cursor);
        self.projectiles.drain(..).for_each(|tex| {
...
impl ContainerItemInterface for Weapons {
    fn destroy(self, graphics: &mut Graphics) {
        self.gun.destroy(graphics);
        self.shotgun.destroy(graphics);
    }
}
