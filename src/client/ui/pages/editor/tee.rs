...
use base::{
    config::Config,
    system::{SystemTime, SystemTimeInterface},
};
...
use crate::{
    render::{
        ...
        tee::{RenderTee, TeeRenderInfo, TeeRenderSkinTextures, RENDER_TEE_BODY_SIZE_BASE},
    },
    ui::types::UIState,
};
...
pub struct TeeEditorPipe<'a> {
    pub graphics: &'a mut Graphics,
    pub sys: &'a SystemTime,
...
impl TeeEditor {
    ...
    pub fn tee_editor_page(
        &mut self,
        ui: &mut egui::Ui,
        pipe: &mut TeeEditorPipe,
        ...
        if self.items.is_empty() {
            ...
            let mut item = self.items.get_mut("test").unwrap();
        ...
        self.data.anim_time = pipe.sys.time_get_nanoseconds();
                                        ...
                                        ui.vertical_centered(|ui| {
                                            ...
                                            if let Some(item) = item_res {
                                                Self::render_central_panel(
                                                    &self.data, ui, item, pipe, ui_state,
                                                );
                                            }
                                        });
                                    });
    ...
    fn render_anim_preview(
        ...
        let response = ui.allocate_response(rect_unscaled.size(), egui::Sense::click_and_drag());
        let ui_rect = egui::Rect::from_min_max(
            egui::Pos2 {
                x: rect_unscaled.min.x * ui_state.zoom_level,
                y: rect_unscaled.min.y * ui_state.zoom_level,
            },
            egui::Pos2 {
                x: rect_unscaled.max.x * ui_state.zoom_level,
                y: rect_unscaled.max.y * ui_state.zoom_level,
            },
        );
        ...
        let mut lines = pipe.graphics.backend_handle.lines_begin();
        lines.lines_draw_tl(&[
            Line {
                vertices: [vec2 { x: 0.0, y: 0.0 }, vec2 { x: 0.0, y: 0.0 }],
            },
            Line {
                vertices: [vec2 { x: 0.0, y: 0.0 }, vec2 { x: 0.0, y: 0.0 }],
            },
    ...
    pub fn render_central_panel(
        ...
        match item.cur_edit.as_str() {
            "anim" => {
                Self::render_anim_preview(editor_data, ui, item, pipe, ui_state);
            }
            _ => {}
        }
    }
    ...
    fn render_anim_key_frame_key_paint(
        ...
        let to_screen = emath::RectTransform::from_to(
            egui::Rect::from_center_size(egui::Pos2::ZERO, rect.square_proportions() / 1.0),
            rect,
        );

        rect = egui::Rect::from_min_max(response.rect.min, response.rect.max);

        let _paint_line = |points: [egui::Pos2; 2], color: Color32, width: f32| {
            let line = [to_screen * points[0], to_screen * points[1]];

            // culling
            if rect.intersects(egui::Rect::from_two_pos(line[0], line[1])) {
                shapes.push(egui::Shape::line_segment(line, (width, color)));
            }
        };

        let mut paint_rect = |corners: &[egui::Pos2; 2], color: Color32| {
            let rect_points = [corners[0], corners[1]];

            shapes.push(egui::Shape::rect_filled(
                egui::Rect::from_min_max(rect_points[0], rect_points[1]),
                0.0,
                color,
            ));
        };
