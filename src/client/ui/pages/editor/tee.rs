...
use graphics_render_traits::GraphicsRenderHandles;
use serde_value::Value;

use graphics::graphics::Graphics;

use graphics_types::{rendering::State, types::Line};
...
impl TeeEditorData {
    ...
    pub fn destroy(self, graphics: &mut Graphics) {
        self.tee_renderer.destroy(graphics)
    }
}
...
impl TeeEditor {
    ...
    fn render_anim_preview(
        ...
        let mut lines = graphics.get_render_handles().0.lines_begin();
        lines.lines_draw_tl(&[
            Line {
    ...
    pub fn render_central_panel(
        ...
        match item.cur_edit.as_str() {
            "anim" => {
                Self::render_anim_preview(editor_data, ui, item, pipe, ui_state, graphics);
            }
            _ => {}
        }
    }
...
impl UIRenderCallbackFunc for TeeEditor {
    fn render(
        &mut self,
        ui: &mut egui::Ui,
        pipe: &mut UIPipe<UIWinitWrapper>,
                                        ...
                                        ui.vertical_centered(|ui| {
                                            ...
                                            if let Some(item) = item_res {
                                                Self::render_central_panel(
                                                    &self.data, ui, item, pipe, ui_state, graphics,
                                                );
                                            }
                                        });
                                    });
