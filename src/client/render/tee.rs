use graphics::graphics::{
    Graphics, GraphicsQuadContainerInterface, QuadContainerBuilder, QuadContainerIndex, SQuad,
};

use graphics_types::rendering::{ColorRGBA, ETextureIndex, State};
use math::math::{vector::vec2, PI};

use super::animation::AnimState;

#[derive(Copy, Clone, PartialEq, PartialOrd)]
pub enum TeeEyeEmote {
    Normal = 0,
    Angry,
    Pain,
    Happy,
...
pub struct SkinTextures {
    body: ETextureIndex,
    body_outline: ETextureIndex,

    left_foot: ETextureIndex,
    left_foot_outline: ETextureIndex,

    right_foot: ETextureIndex,
    right_foot_outline: ETextureIndex,

    left_hand_foot: ETextureIndex,
    left_hand_outline: ETextureIndex,

    right_hand_foot: ETextureIndex,
    right_hand_outline: ETextureIndex,

    left_eyes: [ETextureIndex; 9],
    right_eyes: [ETextureIndex; 9],
}

pub enum TeeRenderSkinTextures {
    Original(SkinTextures),
    Colorable(SkinTextures),
}

impl TeeRenderSkinTextures {
...
pub struct TeeRenderInfo {
    ...
    pub render_skin: TeeRenderSkinTextures,
...
impl RenderTee {
    pub fn new(graphics: &mut Graphics) -> Self {
        ...
        graphics.quad_container_upload(&tee_quad_container);

        Self {
            tee_quad_container: tee_quad_container,
            body_offset,
            body_outline_offset,
            eye_offset,
            foot_offset,
    ...
    pub fn render_tee_math(
        ...
        // eye
        let EyeScale = base_size * 0.40;
        let eye_height = if emote == TeeEyeEmote::Blink {
            base_size * 0.15
        } else {
            EyeScale
        };
        let EyeSeparation = (0.075 - 0.010 * direction.x.abs()) * base_size;
        let Offset = vec2 {
            x: direction.x * 0.125,
        ...
        let eye_pos_left = vec2 {
            x: body_pos.x - EyeSeparation + Offset.x,
        ...
        let eye_scale_left = vec2 {
            x: -EyeScale / (64.0 * 0.4) * anim.left_eye.scale.x,
            y: eye_height / (64.0 * 0.4) * anim.left_eye.scale.y,
        };

        let eye_size_right = vec2 {
            x: EyeScale / (64.0 * 0.4) * anim.right_eye.scale.x,
            y: eye_height / (64. * 0.4) * anim.right_eye.scale.y,
        };
    ...
    pub fn render_tee_body(
        ...
        graphics.quad_container_handle.RenderQuadContainerAsSprite(
    ...
    pub fn render_tee_eyes(
        ...
        _eye_left_color: &ColorRGBA,
        _eye_right_color: &ColorRGBA,
        _alpha: f32,
        skin_textures: &SkinTextures,
    ) {
        let QuadOffset = self.eye_offset;
        let EyeQuadOffset = 0;
        let tee_eye_index = emote as usize - TeeEyeEmote::Normal as usize;
