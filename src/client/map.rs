...
use shared_base::{
    ...
    mapdef::{
        ...
        MapLayer, MapLayerTile, MapLayerTypes,
    },
    types::GameTickType,
};
...
impl RenderMap {
    ...
    pub fn render(&self, pipe: &mut RenderPipeline) {
        ...
        for g in 0..pipe.map.num_groups() as usize {
            ...
            if (!pipe.config.gfx.no_clip || self.map_type == RenderMapTypes::FullDesign)
                && (*group).version >= 2
            ...
            for l in 0..group.num_layers as usize {
                ...
                if self.map_type == RenderMapTypes::All {
                    ...
                    if self.map_type == RenderMapTypes::BackgroundForced {
                        if layer.get_tile_layer_base().item_layer == MapLayerTypes::Tiles as i32
                            && !pipe.config.cl.background_show_tile_layers
                        {
                            continue;
                        }
                    }
                ...
                if let MapLayer::Tile(MapLayerTile(_, _, _)) = layer {
                    if do_render || is_game_layer {
                        let tile_layer_and_overlay_count: usize;
                        if is_front_layer {
                            tile_layer_and_overlay_count = 1;
                        } else if is_switch_layer {
                            tile_layer_and_overlay_count = 3;
                ...
                let mut entity_overlay_val = pipe.config.cl.overlay_entities;
