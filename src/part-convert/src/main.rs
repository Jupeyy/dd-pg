...
fn main() {
    ...
    converted
        ...
        .into_iter()
        .enumerate()
        .for_each(|(index, splat)| write_part(splat, &args.output, &format!("splat{index}")));
    ...
    converted
        ...
        .for_each(|(index, explosion)| {
            write_part(explosion, &args.output, &format!("explosion{index}"))
        });
    ...
    converted
        ...
        .into_iter()
        .enumerate()
        .for_each(|(index, hit)| write_part(hit, &args.output, &format!("hit{index}")));
}
