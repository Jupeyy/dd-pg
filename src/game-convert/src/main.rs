...
fn write_part(part: Game06Part, output: &Path, name: &str) {
...
fn main() {
    ...
    converted
        ...
        .for_each(|(index, muzzle)| {
            write_part(
                muzzle,
                &args.output,
                &format!("weapons/default/gun/muzzle{index}"),
            )
        });
    converted
        .muzzle_shotgun
        ...
        .for_each(|(index, muzzle)| {
            write_part(
                muzzle,
                &args.output,
                &format!("weapons/default/shotgun/muzzle{index}"),
            )
        });
    converted
        .muzzle_ninja
        ...
        .for_each(|(index, muzzle)| {
            write_part(
                muzzle,
                &args.output,
                &format!("ninjas/default/muzzle{index}"),
            )
        });
    ...
    write_part(
        ...
        &args.output,
        "huds/default/vanilla/heart",
    );
    write_part(
        converted.health_empty,
        &args.output,
        "huds/default/vanilla/heart_empty",
    );
    write_part(
        converted.armor_full,
        &args.output,
        "huds/default/vanilla/shield",
    );
    write_part(
        converted.armor_empty,
        &args.output,
        "huds/default/vanilla/shield_empty",
    );
