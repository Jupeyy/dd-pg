...
use master_server_types::{
    ...
    response::{RegisterError, RegisterResponse},
    servers::{SerializedServer, SerializedServers, Server},
};
use rand::random;
...
impl Timekeeper {
    ...
    fn convert_from_system_time(&self, system: SystemTime) -> Timestamp {
...
async fn read_dump(path: &Path, timekeeper: Timekeeper) -> Result<Dump<'static>, io::Error> {
    ...
    let timestamp =
        timekeeper.convert_from_system_time(fs::metadata(&path).await?.modified().unwrap());
    buffer.clear();
...
fn handle_register(
    ...
    let (correct_challenge, should_send_challenge) =
        if let Protocol::VPg = register.address.protocol {
            (true, false)
        } else {
            (
                register
...
async fn main() {
    ...
    if let Some(path) = &read_write_dump {
        match read_dump(Path::new(&path), timekeeper).await {
