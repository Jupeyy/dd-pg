...
impl LoadSkinTexturesData {
    pub(crate) fn load_skin(
        ...
        let load_eyes =
            |eye_name: &'static str| -> anyhow::Result<[PngResultPersistent; TEE_EYE_COUNT]> {
                {
        ...
        Ok(Self {
            ...
            sound: LoadSkinSounds {
                ground_jump: {
                    ...
                    for i in 0..8 {
                        jumps.push(load_sound_file_part_and_upload(
                            sound_mt,
                            files,
                            default_files,
                            skin_name,
...
impl ContainerLoad<Skin> for LoadSkin {
    fn load(
        ...
        match files {
            ContainerLoadedItem::Directory(files) => Self::load_skin(
                graphics_mt,
                sound_mt,
                &files,
                default_files,
