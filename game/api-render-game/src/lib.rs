...
pub fn render_game_new() {
    ...
    let resource_download_server: Option<Url> = read_param_from_host(1);
    let config: ConfigEngine = read_param_from_host(2);
    let window_props: WindowProps = read_param_from_host(3);
    let fonts: Arc<UiFontData> = read_param_from_host(4);
    unsafe { GRAPHICS.borrow().canvas_handle.resized(window_props) };
    unsafe {
        API_RENDER_GAME.create(map_file, resource_download_server, &config, fonts);
    };
}
...
impl ApiRenderGame {
    fn create(
        ...
        let state =
            unsafe { mod_render_game_new(map_file, resource_download_server, config, fonts) };
