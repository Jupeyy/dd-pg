...
impl fmt::Display for UnknownProtocol {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        "protocol must be one of tw-0.5+udp, tw-0.6+udp, tw-0.7+udp or ddrs-0.1+quic".fmt(f)
    }
}
...
impl serde::de::Visitor<'_> for ProtocolVisitor {
    ...
    fn expecting(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.write_str("one of \"tw-0.5+udp\", \"tw-0.6+udp\", \"tw-0.7+udp\" or \"ddrs-0.1+quic\"")
    }
    fn visit_str<E: serde::de::Error>(self, v: &str) -> Result<Protocol, E> {
        ...
        Protocol::from_str(v).map_err(|_| invalid_value())
    }
}
...
impl serde::de::Visitor<'_> for AddrVisitor {
    ...
    fn visit_str<E: serde::de::Error>(self, v: &str) -> Result<Addr, E> {
        let invalid_value = || E::invalid_value(serde::de::Unexpected::Str(v), &self);
        Addr::from_str(v).map_err(|_| invalid_value())
    }
}
...
impl serde::de::Visitor<'_> for RegisterAddrVisitor {
    ...
    fn visit_str<E: serde::de::Error>(self, v: &str) -> Result<RegisterAddr, E> {
        let invalid_value = || E::invalid_value(serde::de::Unexpected::Str(v), &self);
        RegisterAddr::from_str(v).map_err(|_| invalid_value())
    }
}
