...
use base_fs_traits::traits::FileSystemWatcherItemInterface;
use base_io::io::IO;
use cache::Cache;
...
impl GameStateInterface for GameStateWasmManager {
    ...
    fn collect_players_scoreboard_info(
        &self,
        ingame_scoreboard_infos: &mut Vec<shared_game::player::player::PlayerScoreboardInfo>,
        spectator_scoreboard_infos: &mut Vec<shared_game::player::player::PlayerScoreboardInfo>,
        pool: &pool::datatypes::StringPool,
    ) {
        self.state.as_ref().collect_players_scoreboard_info(
            ingame_scoreboard_infos,
            spectator_scoreboard_infos,
            pool,
        )
    }
