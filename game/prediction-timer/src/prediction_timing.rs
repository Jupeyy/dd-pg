...
impl PredictionTimer {
    ...
    pub fn add_ping(&mut self, ping: Duration, cur_time: Duration) {
        ...
        if whole_second > self.timing.cur_whole_second {
            let cur_ping = &self.timing.last_secs_of_pings[0];
            self.timing.jitter_range = cur_ping.max - cur_ping.min;
    ...
    pub fn pred_max_smooth(&mut self) -> Duration {
        ...
        Duration::from_secs_f64(
            (max_ping.as_secs_f64() + max_frame_time.as_secs_f64() + self.timing.smooth_max_ping)
                .clamp(0.0, f64::MAX),
        )
    }
    ...
    pub fn add_packet_stats(&mut self, time_stamp: Duration, packets_sent: u64, packets_lost: u64) {
        self.timing.packets_sent = packets_sent;
        self.timing.packets_lost = packets_lost;
    }
