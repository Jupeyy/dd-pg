...
use anyhow::anyhow;
use base::{benchmark::Benchmark, hash::Hash};
...
pub fn legacy_to_new(
    ...
    let map_legacy = io.io_batcher.spawn(async move {
        ...
        let map = fs.open_file(path).await?;
    ...
    let map = map_legacy.into_map(&images)?;
    ...
    let images: HashMap<Hash, LegacyMapToNewRes> = map
        ...
        .images
        .iter()
        .chain(map.resources.image_arrays.iter())
        .map(|res| {
            (
                res.blake3_hash,
                LegacyMapToNewRes {
                    buf: images_new.get(&res.name).unwrap().clone(),
                    ty: "png".to_string(),
                    name: res.name.clone(),
    ...
    let sounds: HashMap<Hash, LegacyMapToNewRes> = map
        ...
        .map(|res| {
            (
                res.blake3_hash,
                LegacyMapToNewRes {
                    buf: sounds.get(&res.name).unwrap().data.clone().unwrap(),
                    ty: "ogg".to_string(), // TODO: or .opus
                    name: res.name.clone(),
                    blake3_hash: res.blake3_hash,
                },
            )
