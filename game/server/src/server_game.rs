...
use shared_base::{
    network::messages::{GameModification, RenderModification},
...
pub struct ServerGame {
    pub players: LinkedHashMap<PlayerId, ServerPlayer>,
    ...
    pub queued_inputs: LinkedHashMap<GameTickType, LinkedHashMap<PlayerId, PlayerInput>>,

    pub spatial_world: Option<SpatialWorld>,

    // pools
    pub(crate) inps_pool: Pool<LinkedHashMap<PlayerId, CharacterInputInfo>>,
}
    ...
    pub fn new(
            ...
            || RESERVED_DDNET_NAMES.contains(&render_mod_lower.as_str())
        {
            ...
            RenderModification::RequiresWasm {
                name: render_mod.try_into().unwrap(),
