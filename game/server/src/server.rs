...
use shared_base::{
    ...
    network::{
        messages::{MsgClChatMsg, MsgClReady, MsgSvChatMsg, MsgSvServerInfo},
...
use shared_network::{
    game_event_generator::{GameEventGenerator, GameEvents, GameEventsSigned},
    messages::{
...
impl Server {
    pub fn new(
        ...
        *shared_info.sock_addr.lock().unwrap() = Some(sock_addr);
        ...
        Ok(Self {
            ...
            game_server: ServerGame::new(&config_game.sv.map, &thread_pool, &io)?,
    ...
    pub fn try_client_connect(&mut self, con_id: &NetworkConnectionID, timestamp: &Duration) {
        ...
        if self.can_another_player_connect() {
            ...
            let server_info = MsgSvServerInfo {
                map: NetworkString::new(&self.game_server.map.name).unwrap(),
                map_blake3_hash: self.game_server.map.blake3_hash.into(),
                ...
                game_type: NetworkString::new("idm").unwrap(),
            };
            self.network.send_unordered_to(
                &GameMessage::ServerToClient(ServerToClientMessage::ServerInfo {
                    info: server_info,
                    time_until_next_tick: time_until_tick(self.game_server.game.game_tick_speed())
                        .saturating_sub(self.sys.time_get_nanoseconds() - self.last_tick_time),
                }),
                con_id,
            );
                ...
                match player_msg {
                    ...
                    ClientToServerPlayerMessage::RemLocalPlayer => {
                        if player.players.len() > 1 && player.players.remove(player_id).is_some() {
                            self.game_server.player_drop(player_id);
                        }
                    }
    ...
    fn handle_msg(
        &mut self,
        con_id: &NetworkConnectionID,
        signature: Option<(Vec<u8>, Signature)>,
        game_msg: GameMessage,
    ) {
        // check signature, if wanted
        let signature_valid_or_not_exists = if let Some((bytes, signature)) = signature {
            if let GameMessage::ClientToServer(ClientToServerMessage::Ready(MsgClReady {
                public_key: Some(public_key),
                ..
            })) = &game_msg
            {
                public_key.verify(&bytes, &signature).is_ok()
            } else if let Some(ClientAuth { public_key }) = self
                .clients
                .get(con_id)
                .map(|client| client.auth.as_ref())
                .flatten()
            {
                public_key.verify(&bytes, &signature).is_ok()
            } else {
        ...
        if !signature_valid_or_not_exists {
            return;
        }
                ...
                match client_to_server_msg {
                    ClientToServerMessage::Ready(player_info) => {
                        ...
                        if was_client_readied {
                            let unique_identifier = if let (Some(public_key), Some(client)) =
                                (player_info.public_key, self.clients.get_mut(con_id))
                            {
                                let hash = generate_hash_for(public_key.as_bytes());
                                // WARNING: if this line ever gets changed, make sure it's secure
                                client.auth = Some(ClientAuth { public_key });
                                Some(hash)
                            } else {
                                None
                            };
                        ...
                        if self.can_another_player_connect() {
                            self.add_player_for_client(
                                ...
                                &PlayerClientInfo {
                                    ...
                                    unique_identifier: if let Some(auth) = self
                                        .clients
                                        .get(con_id)
                                        .map(|con| con.auth.as_ref())
                                        .flatten()
                                    {
                                        Some(generate_hash_for(auth.public_key.as_bytes()))
                                    } else {
                                        None
                                    },
                                },
                ...
                for (con_id, timestamp_nanos, event) in events.drain(..) {
                    ...
                    match event {
                        ...
                        GameEvents::NetworkMsg(game_msg) => {
                            self.handle_msg(&con_id, signature, game_msg)
                        }
                    }
                }
                game_ev_gen
            ...
            while is_next_tick(cur_time, &mut self.last_tick_time, ticks_in_a_second) {
                ...
                for (con_id, client) in &mut self.clients {
                    ...
                    let (snap_diff, diff_id) = if let Some(client_snap_id) = &client.client_snap_id
                    {
                        ...
                        snap.clear();
                        if let Ok(_) = bsdiff::diff(&client_snap_id.snapshot, &new_snap, &mut *snap)
                        {
    ...
    fn reload(&mut self) {
        ...
        self.game_server =
            ServerGame::new(&self.config_game.sv.map, &self.thread_pool, &self.io).unwrap();
        ...
        let server_info = MsgSvServerInfo {
            ...
            resource_server_fallback: self
                ...
                .as_ref()
                .map(|server| server.port),
            game_type: NetworkString::new("dm").unwrap(),
        };
