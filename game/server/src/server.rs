...
use shared_base::{
    ...
    network::messages::{MsgObjPlayerInfo, MsgSvPlayerInfo, MsgSvServerInfo, NetworkStr},
};
    ...
    pub fn new(
        ...
        *shared_info.sock_addr.lock().unwrap() = Some(sock_addr);
        ...
        Self {
            ...
            max_players: config.sv.max_players,
            ...
            game_server: ServerGame::new(&sys, &config.sv.map, &thread_pool, &fs, &io_batcher),
            ...
            player_ids_pool: Pool::with_sized(config.sv.max_players, || {
                LinkedHashSet::with_capacity(config.sv.max_players_per_ip)
            }),
            player_infos_pool: MtPool::with_sized(config.sv.max_players, || {
                Vec::with_capacity(config.sv.max_players)
            }),
    ...
    fn handle_player_msg(
        ...
        match player_msg {
            ...
            ClientToServerPlayerMessage::RemLocalPlayer => {
                let player = self.clients.get_mut(con_id);
                if let Some(player) = player {
                    if player.player_ids.remove(player_id) {
...
pub fn ddnet_server_main<const IS_INTERNAL_SERVER: bool>(
    ...
    let fs: Arc<FileSystem> = Arc::new(FileSystem::new(&sys.log, "org", "", "DDNet"));
