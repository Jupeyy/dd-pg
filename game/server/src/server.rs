...
use shared_game_wasm::game::state_wasm_manager::GameStateWasmManager;
...
use game_interface::{
    ...
    votes::{MapVote, MiscVote, VoteState, VoteType, Voted},
};
...
pub struct Server {
    ...
    // pools
    player_ids_pool: Pool<LinkedHashSet<PlayerId>>,
    player_snap_pool: Pool<Vec<u8>>,
    player_network_stats_pool: Pool<LinkedHashMap<PlayerId, PlayerNetworkStats>>,
...
impl Server {
    ...
    pub fn client_disconnect(
        ...
        con_id: &NetworkConnectionId,
        _reason: &str,
    ) -> Option<PoolLinkedHashMap<PlayerId, ServerClientPlayer>> {
            ...
            if player.players.contains_key(player_id) {
                match player_msg {
                    ...
                    ClientToServerPlayerMessage::StartVote(vote) => {
                        match vote {
                            VoteType::Map(_) | VoteType::Misc(_) => {
                            ...
                            | VoteType::VoteKickPlayer { voted_player_id } => {
                                if *player_id != voted_player_id {
                                        ...
                                        )
                                    {
                                        ...
                                        if can_be_kicked && self.game_server.cur_vote.is_none() {
                                            self.game_server.cur_vote = Some(ServerVote {
                                                state: VoteState {
                                                    vote,
                                                    // filled on the fly instead
                            ...
                            match votes {
                                MsgClLoadVotes::Map { cached_votes } => {
                                    if !client.loaded_map_votes {
                                        ...
                                        if cached_votes
                                            .is_none_or(|hash| hash != self.map_votes_hash)
                                        {
                                            self.network.send_unordered_to(
                                                &GameMessage::ServerToClient(
                                ...
                                MsgClLoadVotes::Misc { cached_votes } => {
                                    if !client.loaded_misc_votes {
                                        ...
                                        if cached_votes
                                            .is_none_or(|hash| hash != self.misc_votes_hash)
                                        {
                                            self.network.send_unordered_to(
                                                &GameMessage::ServerToClient(
                                                    ServerToClientMessage::LoadVote(
                            ...
                            if !std::mem::replace(&mut client.requested_account_rename, true) {
                                    ...
                                    self.accounts.as_ref().zip(self.db.as_ref())
                                {
                                    if let Some(pool) = db.pools.get(&accounts.kind) {
                                        ...
                                        let pool = pool.clone();
                                        let con_id = *con_id;
                                        self.db_requests.push(self.io.io_batcher.spawn(
                                            async move {
                                                let rename_res =
    ...
    pub fn dbg_game<'a>(
            ...
            || config.client_server_sync_log.projectiles
        {
            ...
            let player_infos: LinkedHashMap<_, _> = stages
                ...
                .collect();
            let projectiles: LinkedHashMap<_, _> = stages
                .iter()
    ...
    pub fn register(&mut self) {
        ...
        if let LocalServerState::Ready { browser_info, .. } =
    ...
    pub fn run(&mut self) {
        ...
        while self.is_open.load(std::sync::atomic::Ordering::Relaxed) {
                    ...
                    if vote.state.yes_votes > vote.state.no_votes {
                        match &vote.state.vote {
                            ...
                            VoteType::VoteKickPlayer { voted_player_id } => {
                                ...
                                {
                                    // tell the game that the player is about to be kicked
                                    self.game_server.game.vote_command(
                                        VoteCommand::AboutToBeKicked(*voted_player_id),
                                    );
                            ...
                            VoteType::VoteSpecPlayer { voted_player_id } => {
                                ...
                                self.game_server
                                    .game
                                    .vote_command(VoteCommand::JoinSpectator(*voted_player_id));
                            }
                            VoteType::Misc(vote) => {
                                // exec the rcon command in the game
                                self.game_server
                                    .game
            ...
            while is_next_tick(cur_time, &mut self.last_tick_time, ticks_in_a_second) {
                ...
                // game ticks
                self.game_server.game.tick(Default::default());
