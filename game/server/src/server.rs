    ...
    pub fn new(
        ...
        *shared_info.sock_addr.lock().unwrap() = Some(sock_addr);
        ...
        Self {
            ...
            game_server: ServerGame::new(&sys, &config_game.sv.map, &thread_pool, &io),
            ...
            player_infos_pool: MtPool::with_sized(config_game.sv.max_players as usize, || {
                Vec::with_capacity(config_game.sv.max_players as usize)
            }),
