...
use crate::{
    ...
    client::{
        ClientSnapshotForDiff, ClientSnapshotStorage, ServerClient, ServerClientPlayer,
...
use game_interface::{
    client_commands::{ClientChatCommand, ClientCommand},
...
impl Server {
    ...
    pub fn try_client_connect(
        ...
        if self.can_another_player_connect() {
            ...
            self.network_queued_clients.insert(
                *con_id,
                ServerNetworkQueuedClient::new(timestamp, ip, ClientAuth { cert }, network_stats),
            );
                                ...
                                if self.game_server.cur_vote.is_none() {
                                    self.game_server.cur_vote = Some(ServerVote {
                                        state: VoteState {
                                            ...
                                            yes_votes: 1,
                                            no_votes: 0,
                                            allowed_to_vote_count: self.clients.len(),
                                        },
            ...
            GameMessage::ClientToServer(client_to_server_msg) => {
                match client_to_server_msg {
                    ClientToServerMessage::Ready(player_info) => {
                        ...
                        if let Some(client) = self.try_client_ready(con_id) {
                            ...
                            self.add_player_for_client(
                                ...
                                PlayerClientInfo {
                                    ...
                                    is_dummy: false,
                                    player_index: 0,
                                    unique_identifier: Self::user_id_to_player_unique_id(&user_id),
                    ...
                    ClientToServerMessage::AddLocalPlayer(player_info) => {
                        if self.can_another_player_connect() {
                            if let Some(client) = self.clients.get(con_id) {
    ...
    pub fn register(&mut self) {
        ...
        let characters = serde_json::to_string(&RegisterInfo {
            characters: characters.drain().map(|(_, c)| c).collect::<Vec<_>>(),
        })
        .unwrap();
                        ...
                        match http
                            .custom_request(
                                ...
                                vec![
                                    ...
                                    ("Info-Serial", "123").into(),
                                    ("content-type", "application/json").into(),
                                ],
                                Some(characters.as_bytes().to_vec()),
                            )
                            .await
                            .map_err(|err| anyhow!(err))
                            .and_then(|res| {
                            ...
                            Err(err) => {
                                dbg!((master_server, err, &characters));
                            }
                        }
                    }
                        ...
                        GameEvents::NetworkEvent(net_ev) => match net_ev {
                            ...
                            NetworkEvent::NetworkStats(stats) => {
                                ...
                                if let Some(client) = self.clients.get_mut(&con_id) {
                                    ...
                                    self.network_queued_clients.get_mut(&con_id)
                                {
                                    client.network_stats = network_stats;
                                }
                                // every second
