...
use shared_base::{
    ...
    network::{
        messages::{MsgClChatMsg, MsgSvChatMsg, MsgSvServerInfo},
...
impl ServerNetworkClient {
    pub fn new(connect_timestamp: &Duration, public_key: Arc<Vec<u8>>) -> Self {
        Self {
            connect_timestamp: *connect_timestamp,
            auth: ClientAuth { public_key },
        }
    }
}
    ...
    pub fn try_client_connect(
        ...
        if self.can_another_player_connect() {
            self.network_clients
                .insert(*con_id, ServerNetworkClient::new(timestamp, public_key));
            ...
            self.network_queued_clients.insert(
                con_id.clone(),
                ServerNetworkQueuedClient::new(timestamp, ClientAuth { public_key }),
            );
    ...
    pub fn client_disconnect(
        ...
        if found.is_some() {
            ...
            if !self.network_queued_clients.is_empty() {
                ...
                self.try_client_connect(&con_id_queue, &timestamp_queue, p.auth.public_key);
            }
            return None;
        }
                ...
                match client_to_server_msg {
                    ClientToServerMessage::Ready(player_info) => {
                        ...
                        if was_client_readied {
                            let unique_identifier = if let Some(ClientAuth { public_key }) =
                                self.clients.get_mut(con_id).map(|client| &client.auth)
                            {
                                let hash = generate_hash_for(public_key.as_slice());
                                Some(hash)
                            } else {
                                None
                            };
                        ...
                        if self.can_another_player_connect() {
                            self.add_player_for_client(
                                ...
                                PlayerClientInfo {
                                    ...
                                    player_index: 0,
                                    unique_identifier: if let Some(ClientAuth { public_key }) =
                                        self.clients.get(con_id).map(|con| &con.auth)
                                    {
                                        Some(generate_hash_for(public_key.as_slice()))
                                    } else {
                                        None
                                    },
                                },
                ...
                for (con_id, timestamp_nanos, event) in events.drain(..) {
                    match event {
                        GameEvents::NetworkEvent(net_ev) => match net_ev {
                            NetworkEvent::Connected { public_key, .. } => {
                                self.logger
                                    .log(LogLevel::Debug)
                                    .msg("connect time sv: ")
                                    .msg_var(&timestamp_nanos.as_nanos());
                                self.try_client_connect(&con_id, &timestamp_nanos, public_key);
                            }
