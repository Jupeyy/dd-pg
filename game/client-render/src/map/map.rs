...
use crate::{
    containers::entities::Entities,
    map::client_map_buffered::{
...
impl RenderMap {
        ...
        if map_item.version < 2 || map_item.synchronized > 0 {
            ...
            total_time = std::time::Duration::from_nanos(
                (mix::<f64, f64>(
                    ...
                    game.intra_tick_from_start(intra_tick_time, &cur_tick, animation_start_tick),
    ...
    fn render_layer(
        ...
        let entities = pipe.entities_container.get_or_default(
            "TODO:",
            pipe.graphics,
            pipe.fs,
            pipe.io_batcher,
        ...
        if physics_opacity_val == 100 && !render_info.is_physics_layer {
            return;
        } else if physics_opacity_val == 0 && render_info.is_physics_layer {
            return;
        }
