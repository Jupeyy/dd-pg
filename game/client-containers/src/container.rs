...
const CONTAINER_MAX_TASKS: usize = 128;
...
struct DefaultItem<L> {
    ...
    // notifier must be before the task
    task: IoBatcherTask<(L, ContainerLoadedItemDir)>,
}
...
pub struct Container<A, L> {
    ...
    resource_http_download_index: Arc<tokio::sync::Mutex<Option<anyhow::Result<AssetsIndex>>>>,
    ...
    L: ContainerLoad<A> + Sync + Send + 'static,
{
    ...
    pub fn update<'a>(
        ...
        try_max_items: Option<usize>,
    ) {
        if self
            .last_update_interval_time
            .is_none_or(|time| cur_time.saturating_sub(time) >= *update_interval)
            || try_max_items.is_some_and(|max_items| self.items.len() > max_items)
        {
    ...
    async fn load_container_item(
        ...
        resource_http_download: Option<(TokioArcMutex<Option<anyhow::Result<AssetsIndex>>>, Url)>,
                ...
                if let Some((url, name, hash, ty)) = http_entry.zip(base_path.to_str()).and_then(
                    ...
                    match res {
                        Ok(file) => {
                            ...
                            if write_to_disk {
                                if let Err(err) = fs
                                    .write_file(base_path.join(name).as_ref(), file.to_vec())
                                    .await
                                {
                                    log::warn!(
        ...
        Q: Borrow<ContainerKey>,
    {
        ...
        if item_res.is_some() {
            ...
            if let Some((name, load_item_res, should_return_new_item)) = task {
