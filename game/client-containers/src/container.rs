...
use graphics::{
    graphics::GraphicsBase, graphics_mt::GraphicsMultiThreaded, image::texture_2d_to_3d,
};
    ...
    Self: Sized,
{
    ...
    fn convert<B: GraphicsBackendInterface>(self, graphics: &mut GraphicsBase<B>) -> A;
}
    ...
    L: ContainerLoad<A> + Sync + Send + 'static,
{
    pub fn new(
        io: IO,
        runtime_thread_pool: Arc<rayon::ThreadPool>,
        default_item: TokIOBatcherTask<L>,
        log: &SystemLog,
        container_name: &str,
    ) -> Self {
    ...
    pub fn load<B: GraphicsBackendInterface>(
        item_name: &str,
        io: &IO,
        runtime_thread_pool: &Arc<rayon::ThreadPool>,
    ...
    pub fn get_or_default<B: GraphicsBackendInterface>(
        &mut self,
        name: &str,
