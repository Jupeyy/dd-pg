...
pub async fn load_file_part(
    ...
    let file = fs.read_file(&part_full_path).await;
    ...
    match file {
        Err(err) => {
            if !is_default {
                ...
                let file_def = fs.read_file(&png_path_def).await;
...
pub async fn load_sound_file_part_and_upload(
    ...
    let file = match fs.read_file(weapon_path.as_ref()).await {
        Ok(file) => Ok(file),
        Err(err) => {
            if !is_default {
                // try to load default part instead
