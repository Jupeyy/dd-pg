...
impl ContainerLoad<Pickup> for LoadPickup {
    ...
    fn convert(self, texture_handle: &GraphicsTextureHandle) -> Pickup {
        Pickup {
            armor: Self::load_file_into_texture(texture_handle, self.armor, &self.pickup_name),
            health: Self::load_file_into_texture(texture_handle, self.health, &self.pickup_name),
        }
    }
}
