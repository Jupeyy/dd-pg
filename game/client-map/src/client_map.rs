    ...
    pub fn new(
        ...
        Self {
            task: io.io_batcher.spawn(async move {
                ...
                let file_futures = read_files.keys().map(|read_file_path| {
                    let read_file_path = read_file_path.to_string();
                    let fs = file_system.clone();
                    async move {
                        let file = fs.open_file(&read_file_path).await;
                        (read_file_path, file)
                    }
                });
                ...
                for (file_name, file) in files {
                    let file = file?;
                    match read_files.get_mut(&file_name).unwrap() {
                        shared_base::datafile::ReadFile::Image(_image_index, img_file_data) => {
                            *img_file_data = file;
                        }
                    }
                }
