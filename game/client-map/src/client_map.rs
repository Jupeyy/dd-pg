...
use client_render_game::{
    ...
    render_game::{RenderGameCreateOptions, RenderGameInterface, RenderModTy},
};
...
use shared::render::render_wasm_manager::{RenderGameMod, RenderGameWasmManager, RENDER_MODS_PATH};
use shared_game_wasm::game::state_wasm_manager::{
...
pub enum GameLoading {
    Task {
        task: IoBatcherTask<RenderGameMod>,
                        ...
                        match render_mod {
                            ...
                            | RenderModTy::Required { name, hash } => {
                                ...
                                let module = match file {
                                    Ok(file) => {
                                        RenderGameWasmManager::load_module(&fs, &file).await
                                    }
                                    Err(err) => Err(err),
                                };
                        ...
                        match load_game {
                            ...
                            GameLoading::Game(mut load_game) => {
                                if load_game.continue_map_loading() {
                                    match GameStateWasmManager::new(
                                        game_mod,
                                        map,
                                        map_name,
                                        game_options,
                                        ...
                                        Ok(game) => {
                                            load_game
                                                .set_chat_commands(game.info.chat_commands.clone());
