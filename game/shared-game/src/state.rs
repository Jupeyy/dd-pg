                ...
                for simulation_event in evs.iter() {
                    match simulation_event {
                        ...
                        SimulationWorldEvent::Global(ev) => {
                            world_events.insert(
                                event_id_generator.next_id(),
                                GameWorldEvent::Global(match ev {
                                    GameWorldGlobalEvent::System(ev) => GameWorldGlobalEvent::System(match ev {
                                        GameWorldSystemMessage::PlayerJoined { id, name } => {
                                            GameWorldSystemMessage::PlayerJoined {
                                                id: *id,
                                                name: game_pools.mt_string_pool.new_str(name.as_str()),
                                            }
                                        }
                                        GameWorldSystemMessage::PlayerLeft { id, name } => {
                                            GameWorldSystemMessage::PlayerLeft {
                                                id: *id,
                                                name: game_pools.mt_string_pool.new_str(name.as_str()),
                                            }
                                        }
                                        GameWorldSystemMessage::Custom(ev) => GameWorldSystemMessage::Custom(
                                            game_pools.mt_string_pool.new_str(ev.as_str()),
                                        ),
                                    }),
                                    GameWorldGlobalEvent::Action(ev) => GameWorldGlobalEvent::Action(match ev {
                                        GameWorldAction::Kill {
                                            killer,
                                            assists,
                                            victims,
                                            ...
                                            killer: *killer,
                                            assists: {
                                                let mut res = game_pools.entity_id_pool.new();
                                                res.extend(assists.iter());
                                                res
                                            },
                                            victims: {
                                                let mut res = game_pools.entity_id_pool.new();
                                                res.extend(victims.iter());
                                                res
                                            },
                                            weapon: *weapon,
                                            flags: *flags,
                                        ...
                                        GameWorldAction::RaceFinish {
                                            ...
                                            characters: {
                                                let mut res = game_pools.entity_id_pool.new();
                                                res.extend(characters.iter());
                                                res
                                            },
                                            finish_time: *finish_time,
                                        },
                                        GameWorldAction::Custom(ev) => {
                                            GameWorldAction::Custom(game_pools.mt_string_pool.new_str(ev.as_str()))
                                        }
                                    }),
                                }),
                            );
