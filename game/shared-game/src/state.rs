pub mod state {
    ...
    use crate::entities::flag::flag::{FlagCore, FlagRenderInfo};
    use crate::entities::laser::laser::{LaserCore, LaserRenderInfo};
    use crate::entities::pickup::pickup::PickupRenderInfo;
    use crate::entities::projectile::projectile::{ProjectileCore, ProjectileRenderInfo};
    ...
    pub trait GameStateInterface {
        ...
        fn intra_tick(
            ...
            ((system.time_get_nanoseconds().as_nanos()
                - self.monotonic_tick_start_time().as_nanos()) as f64
        ...
        fn monotonic_tick_start_time(&self) -> Duration;
        /// TODO: remove this
    ...
    pub struct GameState {
        ...
        /// the time when the first `cur_monotonic_tick` was set (to 0)
    ...
    impl GameState {
        pub fn new(create_pipe: &GameStateCreatePipe, options: &GameStateCreateOptions) -> Self {
        ...
        pub(crate) fn insert_new_stage(&mut self, stage_id: TGameElementID, stage_index: usize) {
            self.stages.insert(
                stage_id.clone(),
                GameStage::new(stage_index, stage_id.clone(), &mut self.world_pool),
            );
        }
        ...
        fn on_character_spawn(spawns: &GameSpawns, character: &mut Character) {
            let pos = spawns.spawns.get(0).copied().unwrap_or(
                spawns
                    .spawns_red
                    .get(0)
                    .copied()
    ...
    impl GameStateInterface for GameState {
        ...
        fn monotonic_tick_start_time(&self) -> Duration {
            self.monotonic_tick_start_time
        }
        ...
        fn add_stage(&mut self) -> TGameElementID {
            ...
            self.stages.insert(
                stage_id.clone(),
                GameStage::new(0, stage_id.clone(), &mut self.world_pool),
            );
            stage_id
        }
        ...
        fn players_inputs(&self, pool: &mut Vec<(TGameElementID, PlayerInput)>) {
            pool.reserve(self.players.len());
            self.players.iter().for_each(|(id, player)| {
                pool.push((id.clone(), player.input.clone()));
            });
        }
