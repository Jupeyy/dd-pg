pub mod state {
    ...
    use crate::match_state::match_state::MatchType;
    ...
    impl GameState {
        ...
        pub(crate) fn add_char_to_stage_checked<'a>(
            ...
            let team = match stage.match_manager.game_match.ty {
                MatchType::Solo => None,
                MatchType::Team { .. } => Some(stage.world.evaluate_character_team()),
            };
        ...
        fn stage_lasers(&self, ratio: f64) -> PoolVec<LaserRenderInfo> {
            ...
            self.game
                ...
                .flat_map(|(stage_id, stage)| {
                    stage.world.characters.iter().map(|(id, character)| {
                        (
                            Some(*stage_id),
                            (id, Some(character), &character.player_info),
                            self.game.players.player(id).is_some(),
                        )
                    })
                })
                .chain(no_char_players)
                .for_each(|(stage_id, (id, character, info), is_player)| {
                    character_infos.insert(
                        *id,
                        CharacterInfo {
                            info: info.player_info.clone(),
                            skin_info: match character.and_then(|c| c.core.team) {
        ...
        fn player_join(&mut self, client_player_info: &PlayerClientInfo) -> GameEntityId {
            if let Some((timeout_player_id, character_info)) = client_player_info
                ...
                .and_then(|id| {
                    self.game
                        .timeout_players
                        .remove(&(id, client_player_info.player_index))
                })
