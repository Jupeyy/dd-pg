pub mod flag {
    ...
    impl Flag {
        pub fn new(game_el_id: &TGameElementID, pos: &vec2, pool: &mut FlagPool) -> Self {
            Self {
    ...
    impl<'a> EntityInterface<FlagCore, FlagReusableCore, SimulationPipeFlag<'a>> for Flag {
        fn pre_tick(pipe: &mut SimulationPipeFlag) {
            todo!()
        }

        fn tick(pipe: &mut SimulationPipeFlag) {}

        fn tick_deferred(pipe: &mut SimulationPipeFlag) {}
