pub mod pickup {
    ...
    impl Pickup {
        pub fn new(game_el_id: &TGameElementID, pos: &vec2, pool: &mut PickupPool) -> Self {
            Self {
    ...
    impl<'a> EntityInterface<PickupCore, PickupReusableCore, SimulationPipePickup<'a>> for Pickup {
        fn pre_tick(pipe: &mut SimulationPipePickup) {
            todo!()
        }

        fn tick(pipe: &mut SimulationPipePickup) {}

        fn tick_deferred(pipe: &mut SimulationPipePickup) {}
