...
pub mod character {
    ...
    pub struct BuffProps {
        pub remaining_tick: GameTickType,
        pub interact_tick: Option<GameTickType>,
    ...
    impl CloneWithCopyableElements for CharacterReusableCore {
        fn copy_clone_from(&mut self, other: &Self) {
            ...
            self.player_info.clone_from(&other.player_info);
        }
    }
    ...
    impl Character {
        pub fn new(
            ...
            reusable_core.player_info = player_info;
        ...
        /// sets the despawn info to a silent move to spectator
        /// this never generates a game event
        pub fn move_to_spec_silent(&mut self) {
            self.despawn_info = CharacterDespawnType::MoveToSpecSilent;
        }
        ...
        pub fn give_ninja(&mut self) {
            self.reusable_core.buffs.insert(
                CharacterBuff::Ninja,
                BuffProps {
                    remaining_tick: 15 * TICKS_PER_SECOND,
                    interact_tick: None,
        ...
        fn fire_weapon(
            ...
            core.attack_recoil = (TICKS_UNTIL_RECOIL_ENDED).into();
            match core.active_weapon {
                ...
                WeaponType::Laser => {
                    self.entity_events.push(CharacterEvent::Laser {
                        pos: proj_start_pos,
                        dir: direction,
                        energy: pipe.collision.get_tune_at(&proj_start_pos).laser_reach,
                    });
                    self.entity_events.push(CharacterEvent::Sound {
                        pos: *self.pos.pos() / 32.0,
        ...
        fn fire_ninja(&mut self, fire: &Option<(NonZeroU64, CharacterInputCursor)>) {
            if !self.can_fire() {
                return;
            }
        ...
        fn handle_buffs_and_debuffs(&mut self, pipe: &mut SimulationPipeCharacter) {
            self.reusable_core.buffs.retain_with_order(|_, buff| {
                buff.remaining_tick -= 1;
                buff.remaining_tick > 0
            });
        ...
        fn handle_ninja(&mut self, pipe: &mut SimulationPipeCharacter) {
            ...
            let Some(interact_tick) = &mut buff.interact_tick else {
                return;
            };
            *interact_tick -= 1;
            if *interact_tick == 0 {
                self.core.core.vel = buff.interact_cursor_dir * buff.interact_val;
                buff.interact_tick = None;
            } else {
            }
            ...
            pipe.characters.for_other_characters_in_range(
                ...
                &mut |char, removed_chars| {
                    ...
                    if self
                        .reusable_core
                        .interactions
                        .contains(&char.base.game_element_id)
                    {
                        return;
                    }
    ...
    impl Drop for Character {
        fn drop(&mut self) {
            ...
            let (is_dead, add_to_no_char_players, death_effect) = match &mut self.despawn_info {
                ...
                CharacterDespawnType::MoveToSpecSilent => {
                    // nothing to do
                    (false, true, false)
                }
            ...
            } = &self.ty
            {
                ...
                if add_to_no_char_players {
                    no_char_players.insert(
                        self.base.game_element_id,
                        NoCharPlayer::new(
                            self.reusable_core.player_info.clone(),
