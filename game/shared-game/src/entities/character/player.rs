pub mod player {
    ...
    use crate::snapshot::snapshot::SnapshotNoCharPlayer;
    ...
    pub struct Players {
        players: LinkedHashMap<PlayerId, Player>,
    }
    ...
    pub enum NoCharPlayerType {
        Spectator {
            spectated_character: Option<CharacterId>,
        },
        Dead {
    ...
    pub struct NoCharPlayers {
        players: LinkedHashMap<PlayerId, NoCharPlayer>,

        // force higher hierarchy val
        _passed: PhantomData<PoolLinkedHashMap<PlayerId, NoCharPlayer>>,
        _n: PhantomData<PoolLinkedHashMap<PlayerId, SnapshotNoCharPlayer>>,
    }
    ...
    impl NoCharPlayers {
        ...
        pub fn player(&self, id: &PlayerId) -> Option<NoCharPlayer> {
        ...
        pub fn insert(&mut self, id: PlayerId, player: NoCharPlayer) {
            self.players.insert(id, player);
        }
        pub fn remove(&mut self, id: &PlayerId) -> Option<NoCharPlayer> {
            self.players.remove(id)
        }
        pub(crate) fn move_to_back(&mut self, id: &PlayerId) {
            self.players.to_back(id);
        ...
        pub(crate) fn pooled_clone_into(
            &self,
            copy_pool: &mut PoolLinkedHashMap<PlayerId, NoCharPlayer>,
        ) {
        ...
        pub(crate) fn count_players_per_side(&self, stage_id: StageId) -> (usize, usize) {
            let mut red = 0;
            let mut blue = 0;
        ...
        pub(crate) fn any_player_in(&self, stage_id: StageId) -> bool {
        ...
        where
            for<'a> F: HiFnMut<(&'a PlayerId, &'a mut NoCharPlayer), bool>,
        {
            self.players
        ...
        where
            for<'a> F: HiFnMut<(&'a PlayerId, &'a NoCharPlayer), ()>,
        {
            self.players
                .iter()
