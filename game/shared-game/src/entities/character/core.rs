pub mod character_core {
    ...
    use crate::{
        ...
        simulation_pipe::simulation_pipe::{
            SimulationEventWorldEntityType, SimulationPipeCharactersGetter, SimulationWorldEvents,
        },
    ...
    pub struct CoreEvents<'a> {
        pub(crate) simulation_events: &'a SimulationWorldEvents,
                ...
                if self.queued_jumps > 0 || **jump {
                    if (self.jumped & 1) == 0 {
                        if grounded && ((self.jumped & 2) == 0 || self.jumps != 0) {
                            character_events.push_ev(CharacterEvent::Sound {
                                pos: Some(*pos.pos() / 32.0),
                            ...
                            character_events.push_ev(CharacterEvent::Effect {
                                pos: *pos.pos() / 32.0,
