pub mod laser {
    ...
    use math::math::{distance, lerp, normalize};
    ...
    impl Laser {
        ...
        fn do_bounce(&mut self, pipe: &mut SimulationPipeLaser) -> bool {
            ...
            self.core.next_eval_in =
                ((TICKS_PER_SECOND as f32 * delay / 1000.0) as GameTickType).into();
        ...
        pub fn lerped_pos(laser1: &Laser, laser2: &Laser, ratio: f64) -> vec2 {
            lerp(&laser1.core.pos, &laser2.core.pos, ratio as f32)
        }
        pub fn lerped_from(laser1: &Laser, laser2: &Laser, ratio: f64) -> vec2 {
            lerp(&laser1.core.from, &laser2.core.from, ratio as f32)
        }
    }
