pub mod laser {
    ...
    impl Laser {
        ...
        fn hit_character(pipe: &mut SimulationPipeLaser, from: &vec2, to: &vec2) -> bool {
            let (ent, core, _) = pipe.laser.split_mut(pipe.cur_core_index);
            ...
            let mut pHit = None;
            if core.can_hit_others {
                let intersection = if core.can_hit_own {
            ...
            if let LaserType::Shotgun = core.ty {
                /* TODO: ddrace
                vec2 Temp;
            ...
            } else if let LaserType::Rifle = core.ty {
        ...
        fn laser_die(pipe: &mut SimulationPipeLaser) {
            let (ent, core, _) = pipe.laser.split_mut(pipe.cur_core_index);
        ...
        fn do_bounce(pipe: &mut SimulationPipeLaser) {
            let (ent, core, _) = pipe.laser.split_mut(pipe.cur_core_index);
            core.eval_tick = pipe.cur_tick;

            if core.energy < 0.0 {
                Self::laser_die(pipe);
                return;
            }
            core.m_PrevPos = core.pos;
            ...
            // TODO: (m_WasTele)
            {
                core.m_PrevPos = core.m_TelePos;
                core.pos = core.m_TelePos;
                core.m_TelePos = vec2::new(0.0, 0.0);
            }

            let mut To = core.pos + core.dir * core.energy;
            ...
            if res > 0 {
                ...
                let cur_pos = core.pos;
                if !Self::hit_character(pipe, &cur_pos, &To) {
                    let (_, core, _) = pipe.laser.split_mut(pipe.cur_core_index);
                    core.from = core.pos;
                    core.pos = To;
                    core.energy = -1.0;
                }
    ...
    impl<'a> EntityInterface<LaserCore, LaserReusableCore, SimulationPipeLaser<'a>> for Laser {
        ...
        fn tick(pipe: &mut SimulationPipeLaser) {
            ...
            let (_, core, _) = pipe.laser.split_mut(pipe.cur_core_index);
            if (pipe.cur_tick - core.eval_tick) as f32 > (TICKS_PER_SECOND as f32 * delay / 1000.0)
            {
                Self::do_bounce(pipe);
            }
        }
