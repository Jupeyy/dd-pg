pub mod simulation_pipe {
    ...
    use game_interface::events::{GameWorldGlobalEvent, GameWorldsEvents};
    ...
    impl SimulationEvents {
        ...
        pub fn for_each<F>(&self, mut f: F)
        where
            for<'a> F: HiFnMut<(&'a GameEntityId, &'a SimulationWorldEvents), ()>,
        {
            self.events
                .iter()
                .for_each(move |(id, ev)| f.call_mut((id, ev)))
        }
    }
