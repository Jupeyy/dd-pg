...
pub mod world {
    ...
    impl GameWorld {
        ...
        pub(crate) fn evaluate_character_team(&self) -> GameTeam {
            let mut red_team = 0;
            let mut blue_team = 0;
        ...
        fn tick_characters(&mut self, pipe: &mut SimulationPipeStage) {
            ...
            characters.for_each(|(id, (character, other_chars))| {
                ...
                character.entity_events.drain(..).for_each(|ev| {
                    ...
                    self.simulation_events
                        .push(SimulationEventWorldEntity::Character { player_id: *id, ev });
                });
            });
        }
                        ...
                        let max_by = |&spawn1: &&vec2, &spawn2: &&vec2| {
                            let sum_dist_spawn = |spawn: &vec2| {
                                characters
                                    .values()
                                    .map(|char| {
                                        // multiply by factor further away from players of the other team
