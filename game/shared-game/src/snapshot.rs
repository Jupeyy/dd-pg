pub mod snapshot {
    ...
    use game_interface::types::{
        ...
        snapshot::{
            FromSnapshotBuildMode, SnapshotClientInfo, SnapshotLocalPlayer, SnapshotLocalPlayers,
        },
        weapons::WeaponType,
    };
    ...
    use crate::{
        ...
        simulation_pipe::simulation_pipe::SimulationEvents,
        stage::stage::Stages,
    ...
    pub struct SnapshotCharacter {
        ...
        pub pos: vec2,
        pub hook: (Hook, Option<CharacterId>),

        pub game_el_id: CharacterId,
    }

    pub type PoolSnapshotCharacters = LinkedHashMap<CharacterId, SnapshotCharacter>;
    pub type SnapshotCharacters = PoolLinkedHashMap<CharacterId, SnapshotCharacter>;
    ...
    pub type PoolSnapshotProjectiles = LinkedHashMap<ProjectileId, SnapshotProjectile>;
    pub type SnapshotProjectiles = PoolLinkedHashMap<ProjectileId, SnapshotProjectile>;
    ...
    pub type PoolSnapshotLasers = LinkedHashMap<LaserId, SnapshotLaser>;
    pub type SnapshotLasers = PoolLinkedHashMap<LaserId, SnapshotLaser>;
    ...
    pub type PoolSnapshotPickups = LinkedHashMap<PickupId, SnapshotPickup>;
    pub type SnapshotPickups = PoolLinkedHashMap<PickupId, SnapshotPickup>;
    ...
    pub type PoolSnapshotFlags = LinkedHashMap<CtfFlagId, SnapshotFlag>;
    pub type SnapshotFlags = PoolLinkedHashMap<CtfFlagId, SnapshotFlag>;
    ...
    pub struct SnapshotPool {
        pub(crate) stages_pool: Pool<LinkedHashMap<StageId, SnapshotStage>>,
        no_char_players_pool: Pool<LinkedHashMap<PlayerId, SnapshotNoCharPlayer>>,
        local_players_pool: Pool<LinkedHashMap<PlayerId, SnapshotLocalPlayer>>,
    ...
    #[derive(Serialize, Deserialize)]
    pub struct Snapshot {
        pub stages: PoolLinkedHashMap<StageId, SnapshotStage>,
        pub no_char_players: PoolLinkedHashMap<PlayerId, SnapshotNoCharPlayer>,

        pub local_players: PoolLinkedHashMap<PlayerId, SnapshotLocalPlayer>,
    ...
    impl SnapshotManager {
        ...
        pub(crate) fn build_stages(
            &self,
            stages: &mut PoolLinkedHashMap<StageId, SnapshotStage>,
            game: &GameState,
        ) {
        ...
        pub(crate) fn convert_to_game_stages(
            mut snap_stages: PoolLinkedHashMap<StageId, SnapshotStage>,
            stages: &mut Stages,
            ...
            let dummy_ids: LinkedHashSet<CharacterId> = Default::default();
            let character_ids = match &mode {
                FromSnapshotBuildMode::Default => &dummy_ids,
                FromSnapshotBuildMode::Unpredicted { keep_characters } => keep_characters,
            };
                ...
                if let Some(snap_stage) = snap_stages.get(id) {
                    ...
                    stage.world.projectiles.retain(|id, ent| {
                        if snap_stage.world.projectiles.contains_key(id)
                            || character_ids.contains(&ent.character_id)
                        {
                            true
                        } else {
            ...
            snap_stages.drain().for_each(|(snap_stage_id, snap_stage)| {
                ...
                snap_stage.world.characters.values().for_each(|char| {
                    ...
                    if !character_ids.contains(&char.game_el_id) {
                ...
                snap_stage.world.projectiles.values().for_each(|proj| {
                    ...
                    let stage_proj = state_stage
                        ...
                        .unwrap();
                    if !character_ids.contains(&proj.owner_game_el_id) {
                ...
                snap_stage.world.lasers.values().for_each(|laser| {
                    ...
                    if !character_ids.contains(&laser.owner_game_el_id) {
        ...
        pub fn build_from_snapshot(
            ...
            write_game_state
                ...
                .retain_with_order(hi_closure!(
                    [no_char_players: &PoolLinkedHashMap<PlayerId, SnapshotNoCharPlayer>],
                    |id: &PlayerId, _: &mut NoCharPlayer| -> bool {
            ...
            no_char_players.drain().for_each(|(id, p)| {
                ...
                write_game_state.game.no_char_players.insert(id, p.player);
            ...
            Self::convert_to_game_stages(
                ...
                mode,
                &mut write_game_state.game.simulation_events,
            );
