pub mod snapshot {
    ...
    use super::super::{
        ...
        state::state::{GameState, GameStateInterface},
    };
    use bincode::{Decode, Encode};
    use hashlink::LinkedHashMap;
    use pool::{
        datatypes::PoolVec as SingleThreadedPoolVec,
        mt_datatypes::{PoolLinkedHashMap, PoolVec},
        mt_pool::Pool,
    };
    ...
    pub struct SnapshotClientInfo {
        pub client_player_ids: SingleThreadedPoolVec<TGameElementID>,
    ...
    impl SnapshotWorld {
        pub fn new(character_pool: &SnapshotCharactersPool) -> Self {
            Self {
                characters: character_pool.characters_pool.new(),
            }
        }
    }
    ...
    impl SnapshotStage {
        pub fn new(
            id: &TGameElementID,
            stage_index: usize,
            character_pool: &SnapshotCharactersPool,
        ) -> Self {
            Self {
                world: SnapshotWorld::new(character_pool),
                game_el_id: id.clone(),
                stage_index,
            }
        }
    ...
    pub struct SnapshotPool {
        ...
        local_players_pool: Pool<Vec<SnapshotLocalPlayer>>,
    }
    ...
    pub struct Snapshot {
        ...
        pub local_players: PoolVec<SnapshotLocalPlayer>,
        pub time_since_connect_nanos: u64,
    }
    ...
    impl SnapshotManager {
        pub fn new(options: &SnapshotManagerCreateOptions) -> Self {
            Self {
                ...
                character_pool: SnapshotCharactersPool::new(
                    options.hint_max_local_players.unwrap_or(64),
                ),
            }
        }
        ...
        pub fn build_for(&self, game: &GameState, client: SnapshotClientInfo) -> Snapshot {
            ...
            client.client_player_ids.iter().for_each(|id| {
                if let Some(player) = game.players.get(id) {
                    res.local_players.push(SnapshotLocalPlayer {
                        game_el_id: id.clone(),
            ...
            game.stages.values().for_each(|stage| {
                let mut characters = self.character_pool.characters_pool.new();
                stage.world.get_characters().iter().for_each(|(id, char)| {
                    let mut snap_char = SnapshotCharacter {
                        core: *char.get_core(),
                        reusable_core: self.character_pool.character_reusable_cores_pool.new(),
                ...
                res.stages.insert(
                    ...
                    SnapshotStage {
                        world: SnapshotWorld { characters },
        ...
        pub fn convert_to_game_state(
            ...
            snapshot.stages.values().for_each(|stage| {
                ...
                // go through all characters of the stage
            ...
            write_game_state.set_cur_monotonic_tick(snapshot.monotonic_tick);
