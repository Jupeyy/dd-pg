...
use anyhow::anyhow;
use client_types::console::parser::{Command, Syn};
...
const LOCAL_PLAYER_ACTIONS: [(&str, BindActionsLocalPlayer); 18] = [
...
fn bind_keys_str_to_bind_keys(bind_keys_str: &str) -> anyhow::Result<Vec<BindKey>> {
    ...
    for bind_key_str in bind_keys_str.split('+') {
        ...
        cap_bind_key_str.make_ascii_uppercase();
        cap_bind_key_str = {
            let res: Vec<_> = cap_bind_key_str
                .chars()
                .collect::<Vec<char>>()
                .windows(2)
                .flat_map(|c| {
