        ...
        F: Copy + FromFixed + ToFixed,
    {
        ...
        for i in 0..points.len() - 1 {
            if time_nanos >= points[i].time && time_nanos <= points[i + 1].time {
                ...
                for c in 0..channels {
                    let v0: ffixed = points[i].value[c].to_fixed();
                    let v1: ffixed = points[i + 1].value[c].to_fixed();
        ...
        let mut res = T::default();
        for c in 0..channels {
            res[c] = points[points.len() - 1].value[c];
        }
        res
    }
        ...
        stream_handle.render_quads(
            hi_closure!([
                ...
                for i in (0..num_segments).step_by(2) {
                    ...
                    stream_handle
                        .add_vertices(
                            StreamedQuad::new().pos_free_form(
                                vec2::new(
                                    pos.x,
                                    pos.y
                                ),
