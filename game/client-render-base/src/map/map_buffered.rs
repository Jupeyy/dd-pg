...
use shared_base::mapdef_06::{DdraceTileNum, TILE_SWITCHTIMEDOPEN};
...
impl ClientMapBuffered {
                    ...
                    layers: def
                        ...
                        .map(|(l, def)| match def {
                            ...
                            MapLayer::Tile(def) => {
                                ...
                                if visuals.buffer_object.is_some() {
    ...
    pub fn finish_upload_tile_layer(
            ...
            .is_some_and(|raw_data| !raw_data.as_slice().is_empty())
        {
            ...
            TileLayerVisuals {
                base: visuals,
                buffer_object: Some(buffer_object_handle.create_buffer_object(raw_data.unwrap())),
            }
        } else {
            TileLayerVisuals {
                base: visuals,
    ...
    pub fn finish_upload_physics_tile_layer(
        ...
        for (ty, visuals, overlay_mem) in overlays {
            ...
            overlay_buffer_objects.push(PhysicsTileLayerOverlayVisuals { visuals, ty });
        }
        ...
        for cur_text_overlay in 0..text_overlay_count + 1 {
            ...
            if let Some(data) = Self::upload_tile_layer_buffer(
                ...
                if cur_text_overlay == 0 {
                    ...
                    res.quad_count_for_indices =
                        res.quad_count_for_indices.max(data.quad_count_for_indices);
                    res.overlays
                        .push((text_overlay_type.unwrap(), data.visuals, data.mem));
                }
            }
        }
        res
        ...
        L: BorrowMut<PhysicsTileLayerVisuals>,
    {
        ...
        for cur_text_overlay in 0..text_overlay_count + 1 {
            ...
            let mut buffer_object = if cur_text_overlay == 0 {
                &layer.user_mut().borrow_mut().base.buffer_object
            } else {
            ...
            *if cur_text_overlay == 0 {
                &mut layer.user_mut().borrow_mut().base.buffer_object
            } else {
        ...
        T: BorrowMut<TileLayerVisuals>,
    {
        ...
        Self::update_tile_layer(
            tp,
            &mut layer.user.borrow_mut().buffer_object,
            layer_width,
            layer_height,
            x,
            y,
    ...
    pub fn tile_set_preview(
        ...
        let buffer = ClientMapBuffered::upload_design_tile_layer(
            graphics_mt,
            &tiles,
