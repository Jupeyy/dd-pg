...
use graphics::{
    buffer_object_handle::BufferObjectIndex, graphics::Graphics,
    handles::backend::GraphicsBackendHandle,
};
...
use graphics_types::{
    commands::{
        AllCommands, CommandsRender, SColorf, GRAPHICS_DEFAULT_UNIFORM_SIZE,
        GRAPHICS_MAX_UNIFORM_RENDER_COUNT,
    },
    rendering::{ColorRGBA, GlColorf, State, StateTexture},
};
...
use pool::{
    mt_datatypes::{PoolVec, StringPool},
    mt_pool::Pool,
};
...
impl MapPipeline {
    ...
    fn tile_pipeline_layout(has_sampler: bool, is_border: bool) -> BackendPipelineLayout {
        ...
        let mut vert_push_constant_size = std::mem::size_of::<UniformTileGPos>();
        if is_border {
            vert_push_constant_size = std::mem::size_of::<UniformTileGPosBorder>();
        }
    ...
    fn render_tile_layer_fill_execute_buffer(
        ...
        match state.texture_index {
            ...
            StateTexture::ColorAttachmentOfPreviousPass => {
                panic!("color attachments don't support 3d images")
            }
            StateTexture::None => {
                // nothing to do
            }
...
impl BackendCustomPipeline for MapPipeline {
    ...
    fn pipe_layout_of(&self, name: u64, is_textured: bool) -> BackendPipelineLayout {
        ...
        match name {
            MapPipelineNames::TilePipeline => Self::tile_pipeline_layout(is_textured, false),
            MapPipelineNames::TileBorderPipeline => Self::tile_pipeline_layout(is_textured, true),
    ...
    fn fill_exec_buffer(
        ...
        let (command, _) =
            bincode::decode_from_slice::<CommandsRenderMap, _>(&cmd, bincode::config::standard())
                .unwrap();
        match command {
...
impl MapGraphics {
    pub fn new(graphics: &mut Graphics) -> Self {
        Self {
            ...
            mod_name: Pool::with_capacity(32),
            cmd_pool: Pool::with_capacity(32),
        }
    }
    ...
    pub fn render_tile_layer(
        ...
        bincode::encode_into_std_write(
            CommandsRenderMap::TileLayer(cmd),
            &mut std::io::BufWriter::<&mut Vec<u8>>::new(&mut pooled_cmd),
