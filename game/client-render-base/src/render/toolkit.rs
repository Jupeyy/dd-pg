...
impl ToolkitRender {
    ...
    pub fn render_hook_for_player(
        ...
        if let Some(hook_pos) = player_render_info.lerped_hook_pos {
            ...
            self.render_hook(cur_hook, pos, hook_pos, hook_dir, 1.0, quad_scope);
    ...
    pub fn render_ninja_weapon(
        ...
        let quad_offset = if character_render_info.cursor_pos.x < 0.0 {
            quad_scope.set_rotation(-PI / 2.0 - frame.rotation * PI * 2.0);
        ...
        if attack_time <= 1.0 / 6.0 && !ninja.muzzles.is_empty() {
            ...
            let cursor = character_render_info.cursor_pos;
