...
fn get_render_tee_anim_scale_and_base_size(_anim: &AnimState, info: &TeeRenderInfo) -> (f32, f32) {
    (info.size / RENDER_TEE_SIZE, info.size)
}
...
pub fn offset_to_mid(metrics: &SkinMetrics, anim: &AnimState, info: &TeeRenderInfo) -> vec2 {
    let (scale, _) = get_render_tee_anim_scale_and_base_size(anim, info);
...
impl RenderTeeMath {
    pub fn new(
        ...
        // general
        let (anim_scale, base_size) = get_render_tee_anim_scale_and_base_size(anim, info);
        ...
        get_render_tee_body_scale(base_size, &mut body_scale_x, &mut body_scale_y);
