...
///
/// A few guarantees this should give is, that a string can safely passed as name
/// for a file path without changing directory or simiar.
/// As well as passing it inside an URL.
/// Also it should not contain escape characters, to prevent fatal errors while
/// working with those strings.
/// Not allowing hash like names helps keeping the string pool clean for the use cases
...
impl ReducedAsciiString {
    ...
    fn is_char_valid(char: &ascii::AsciiChar) -> bool {
        char.is_alphabetic() || char.is_ascii_digit() || *char == ascii::AsciiChar::new('_')
    }
