pub use accounts_types::account_id::AccountId;
...
impl PlayerUniqueId {
    /// intercompability with Option
    pub fn and_then<U, F: FnOnce(Self) -> Option<U>>(self, op: F) -> Option<U> {
        match self {
            PlayerUniqueId::Account(_) => op(self),
            PlayerUniqueId::CertFingerprint(_) => op(self),
        }
    }
}
