...
impl GameTickCooldownAndLastActionCounter {
    ...
    /// to the current cooldown length without modifying
    /// the cooldown counter.
    /// This is useful if a forced weapon switch happend
    /// (e.g. ninja collected / lost)
    pub fn advance_ticks_passed_to_cooldown_len(&mut self) {
        match self {
            GameTickCooldownAndLastActionCounter::None => {}
            GameTickCooldownAndLastActionCounter::Cooldown {
                ticks_passed,
            ...
            | GameTickCooldownAndLastActionCounter::LastActionCounter {
                ...
                if *ticks_passed < cooldown_len.get() {
                    *ticks_passed = cooldown_len.get();
                }
            }
        }
    }
