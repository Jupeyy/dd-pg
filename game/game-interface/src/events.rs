...
use crate::types::{flag::FlagType, game::GameEntityId, weapons::WeaponType};

/// ID to uniquely identify an event
#[derive(
    Debug, Hiarc, Copy, Clone, Hash, PartialEq, Eq, PartialOrd, Ord, Serialize, Deserialize,
)]
pub struct EventId(u64);
/// First event ID
pub const FIRST_EVENT_ID: EventId = EventId(0);

/// Generates new unique Event-IDs
#[hiarc_safer_refcell]
#[derive(Debug, Hiarc)]
pub struct EventIdGenerator(EventId);
...
impl EventIdGenerator {
    ...
    pub fn reset_id_for_client(&mut self, next_id: EventId) {
        *self = Self(next_id);
    }
    ...
    pub fn next_id(&mut self) -> EventId {
        let res = self.0;
        self.0 = EventId(res.0 + 1);
        res
    }

    /// Get the next unique event id without
    /// advancing the internal id tracker.
    /// This is useful to sync the id
    /// with the client over
    /// [`EventIdGenerator::reset_id_for_client`]
    #[must_use]
...
pub enum GameCharacterEventEffect {
    ...
    Death,
    AirJump,
    DamageIndicator { pos: vec2, vel: vec2 },
    HammerHit { pos: vec2 },
}
