...
use egui::{vec2, Color32, Sense, Window};
...
pub fn render(main_frame_only: bool, pipe: &mut UIPipe<UserData>, ui: &mut egui::Ui) {
    ...
    let window_res = Window::new("Auto-mapper-rule-creator").show(ui.ctx(), |ui| {
            ...
            .is_some_and(|i| auto_mapper.rules.get(i).is_some())
        {
            ...
            ui.add_space(10.0);
            for y in 0..9 {
                ui.allocate_exact_size((tile_size, tile_size).into(), Sense::click());
            }
            let available_rect = ui.available_rect_before_wrap();
            ui.painter().rect_filled(
                egui::Rect::from_min_size(
                ...
                for y in 0..16 {
                    ui.horizontal(|ui| {
                        for x in 0..16 {
                            ...
                            ui.image((tile_texture.id(), vec2(tile_size, tile_size)));
                        }
                    });
                }
            });
