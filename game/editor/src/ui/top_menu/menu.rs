...
use crate::{
    ...
    ui::{
        user_data::{EditorMenuDialogMode, EditorMenuHostDialogMode, EditorUiEvent, UserData},
        ...
        .show_inside(ui, |ui| {
            egui::ScrollArea::horizontal().show(ui, |ui| {
                        ...
                        if let Some(selected) = file_dialog.update(ui.ctx()).selected() {
                            ...
                            | EditorMenuDialogMode::Save { .. } = menu_dialog_mode
                            {
                                match mode {
                                    ...
                                    DialogMode::SelectDirectory => todo!(),
                                    DialogMode::SaveFile => {
                                        pipe.user_data
                                            .ui_events
                                ...
                                let pub_key_der = cert.serialize_der().unwrap();
                                let private_key_der = cert.serialize_private_key_der();

                                *mode = EditorMenuHostDialogMode::HostNetworkOptions {
                                    map_path: selected,
                                    port: 0,
                ...
                if let EditorMenuDialogMode::Host {
                    mode:
                        EditorMenuHostDialogMode::HostNetworkOptions {
                            port,
                            password,
                            pub_key_der,
                            ..
                ...
                } = menu_dialog_mode
                {
                    if !main_frame_only {
                        ...
                        let window_res = window.show(ui.ctx(), |ui| {
                            ...
                            // TODO: cache this
                            let hash = generate_hash_for(&pub_key_der);
                            ui.label(format!("{}", fmt_hash(&hash)));
                        ...
                        if host {
                            ...
                            pipe.user_data.ui_events.push(EditorUiEvent::HostMap {
                                map_path,
                                port,
                                password,
                                private_key_der,
                                ...
                                if i.pointer.primary_down() {
                                    Some((
                                        !window_res.response.rect.intersects({
                                            let min =
                                                i.pointer.interact_pos().unwrap_or_default().into();
                                            let max = min;
                                            [min, max].into()
                                        }),
