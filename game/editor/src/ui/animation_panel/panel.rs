...
pub fn render(
    ...
    if main_frame_only {
        panel = panel.exact_height(map.user.ui_values.animations_panel_height);
    } else {
        panel = panel.default_height(map.user.ui_values.animations_panel_height);
    }
    ...
    let res = panel.show_inside(ui, |ui| {
        fn add_selector<A: Point + DeserializeOwned + PartialOrd + Clone>(
            ui: &mut egui::Ui,
            anims: &Vec<AnimBaseSkeleton<EditorAnimationProps, A>>,
        ...
        fn add_group<'a, A: Point + DeserializeOwned + PartialOrd + Clone>(
            ...
            anims: &'a mut Vec<AnimBaseSkeleton<EditorAnimationProps, A>>,
            index: Option<usize>,
            name: &'a str,
        ) {
    ...
    if !main_frame_only && res.response.rect.height() != map.user.ui_values.animations_panel_height
    {
        map.user.ui_values.animations_panel_height = res.response.rect.height();
    }
...
fn handle_anim_time_change(pipe: &mut UIPipe<UserDataWithTab>) {
    let map = &mut pipe.user_data.editor_tab.map;
...
fn handle_point_insert(pipe: &mut UIPipe<UserDataWithTab>) {
    let map = &mut pipe.user_data.editor_tab.map;
