...
pub fn layer_name(
    ...
    if !layer.name().is_empty() {
        ...
        MapLayerSkeleton::Tile(layer) => layer.layer.attr.image_array.map(|image| {
            (
                "\u{f302}",
                format!(" {}", resources.image_arrays[image].def.name.as_str()),
            )
        }),
        MapLayerSkeleton::Quad(layer) => layer.layer.attr.image.map(|image| {
            (
                "\u{f03e}",
                format!(" {}", resources.images[image].def.name.as_str()),
            )
        }),
        MapLayerSkeleton::Sound(layer) => layer.layer.attr.sound.map(|sound| {
            (
                "\u{f001}",
                format!(" {}", resources.sounds[sound].def.name.as_str()),
            )
        }),
    } {
        job.append(
