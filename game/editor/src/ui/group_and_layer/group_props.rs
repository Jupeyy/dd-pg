...
use crate::{
    actions::actions::{ActChangeGroupAttr, ActChangePhysicsGroupAttr, EditorAction},
                                ...
                                let old_layer_tiles: Vec<_> = group
                                    ...
                                    .map(|layer| match layer {
                                        ...
                                        EditorPhysicsLayer::Game(layer) => {
                                            MapTileLayerPhysicsTiles::Game(layer.def.tiles.clone())
                                        }
                                        EditorPhysicsLayer::Front(layer) => {
                                            MapTileLayerPhysicsTiles::Front(layer.def.tiles.clone())
                                        }
                                        EditorPhysicsLayer::Tele(layer) => {
                                            MapTileLayerPhysicsTiles::Tele(layer.def.tiles.clone())
                                        }
                                        EditorPhysicsLayer::Speedup(layer) => {
                                            MapTileLayerPhysicsTiles::Speedup(
                                                layer.def.tiles.clone(),
                                            )
                                        }
                                        ...
                                        EditorPhysicsLayer::Tune(layer) => {
                                            MapTileLayerPhysicsTiles::Tune(layer.def.tiles.clone())
                                        }
                                    })
                                    .collect();
                                tab.client.execute(
                                    EditorAction::ChangePhysicsGroupAttr(
                                        ActChangePhysicsGroupAttr {
                                            ...
                                            old_layer_tiles: old_layer_tiles,
                                        },
                                    ),
                                    Some(&format!("change-physics-group-attr")),
                                );
                            }
                        });
                });
