...
use math::math::vector::{dvec2, ffixed, nffixed, nfvec4};
use time::Duration;
        ...
        let (mut selected_quads, point, pos_offset, pos_anim, color_anim) =
            match &pipe.user_data.tools.active_tool {
                ActiveTool::Quads(ActiveToolQuads::Brush) => {
                    ...
                    if let Some((selection, quad)) = brush
                        .last_selection
                        .as_mut()
                        .map(|selection| {
                ...
                ActiveTool::Quads(ActiveToolQuads::Selection) => {
                    ...
                    let point = selection.range.as_ref().map(|range| range.point).flatten();
                    (
                        selection
                            .range
                            .as_mut()
            ...
            egui::Grid::new("design group attr grid")
                ...
                .show(ui, |ui| {
                    ...
                    if matches!(point, QuadPointerDownPoint::Center) && !animations_panel_open {
                        fn combobox_name(ty: &str, index: usize, name: &String) -> String {
                            name.is_empty()
                        ...
                        if can_change_pos_anim {
                            // pos anim
                            ui.label("pos anim");
                            egui::ComboBox::new(&format!("quad-select-pos-anim"), "")
                                .selected_text(
                                    animations
                                        .pos
                            ...
                            if ui.add(egui::DragValue::new(&mut millis)).changed() {
                                quad.pos_anim_offset = Duration::milliseconds(millis as i64);
                            }
                            ui.end_row();
                        }
                        if can_change_color_anim {
                            // color anim
                            ui.label("color anim");
                            egui::ComboBox::new(&format!("quad-select-color-anim"), "")
                                .selected_text(
                                    animations
                                        .color
                            ...
                            if ui.add(egui::DragValue::new(&mut millis)).changed() {
                                quad.color_anim_offset = Duration::milliseconds(millis as i64);
                            }
                            ui.end_row();
                        }

                        // slice
                        ui.button("TODO: slice");
                        ui.end_row();
                        // square
                        ui.button("TODO: square");
                        ui.end_row();
                        // align
                        ui.button("TODO: align");
                        ui.end_row();
                            ...
                            || (can_change_color_anim && quad.color_anim.is_some())
                        {
                            ...
                            ui.label(job).on_hover_ui(|ui| {
                                ...
                                egui_commonmark::CommonMarkViewer::new("quad-prop-color-tooltip")
                                    .show(ui, &mut cache, TEXT_QUAD_PROP_COLOR);
                            });
