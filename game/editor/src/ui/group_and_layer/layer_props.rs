...
use crate::{
    actions::actions::{
        ActAddRemQuadLayer, ActAddRemSoundLayer, ActAddRemTileLayer, ActChangeQuadLayerAttr,
        ActChangeSoundLayerAttr, ActChangeTileLayerDesignAttr, ActRemQuadLayer, ActRemSoundLayer,
                ...
                let res = window.show(ui.ctx(), |ui| {
                    egui::Grid::new("design group attr grid")
                        ...
                        .show(ui, |ui| {
                            ...
                            ui.label("color anim");
                            egui::ComboBox::new(&format!("tile-layer-select-color-anim"), "")
                                .selected_text(
                                    map.animations
                                        .color
                            ...
                            if ui.add(egui::DragValue::new(&mut millis)).changed() {
                                attr.color_anim_offset = Duration::milliseconds(millis as i64);
                            }
                            ui.end_row();
                            // name
                            ui.label("Name");
