...
use network::network::network::{
    NetworkClientCertCheckMode, NetworkServerCertMode, NetworkServerCertModeResult,
};
...
impl Editor {
    pub fn new(
        ...
        let mut res = Self {
            ...
            ui: EditorUiRender::new(graphics, tp.clone(), font_data),
            ...
            match &server {
                Some(server) => match &server.cert {
                    NetworkServerCertModeResult::Cert { cert } => {
                        NetworkClientCertCheckMode::CheckByCert { cert: cert.der() }
                    }
                ...
                None => match &server_cert_hash {
                    Some(hash) => NetworkClientCertCheckMode::CheckByPubKeyHash { hash: &hash },
        ...
        for ev in std::mem::take(&mut self.ui_events) {
            match ev {
                ...
                EditorUiEvent::HostMap {
                    ...
                    self.load_map_impl(
                        map_path.as_ref(),
                        Some(NetworkServerCertMode::FromCertifiedKeyPair { cert }),
                        Some(port),
                        Some(password),
                    );
                }
