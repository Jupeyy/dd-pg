...
use client_render_base::map::{
    ...
    render_pipe::{Camera, GameStateRenderInfo},
...
use crate::{
    ...
    tools::{quad_layer::shared::QUAD_POINT_RADIUS, utils::render_rect},
...
impl QuadBrush {
                ...
                if let Some(edit_quad) = layer.layer.quads.get(last_active.quad_index) {
                        ...
                        && latest_modifiers.ctrl
                    {
                        if let Some(cursor_pos) = &last_active.cursor_in_world_pos {
                            ...
                            super::shared::rotate(&center[0], ffixed::from_num(diff), points);
                        }
                    } else {
                        // handle position
    ...
    fn render_brush(
        ...
        if let Some(buffer_object_index) = &brush.render.buffer_object_index {
            ...
            let game = &map.game_state_info();
            let cur_time = &map.get_time();
            let camera = &map.game_camera();
