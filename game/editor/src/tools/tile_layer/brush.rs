...
use map::{
    map::groups::{
        layers::tiles::{MapTileLayerPhysicsTiles, MapTileLayerTiles, Tile, TileFlags},
        MapGroupAttr,
    },
    types::NonZeroU16MinusOne,
};
use math::math::vector::{ubvec4, usvec2, vec2, vec4};
...
use crate::{
    ...
    tools::utils::{render_filled_rect, render_rect, render_rect_from_state},
...
impl TileBrush {
                        ...
                        let (tiles, texture) = match layer {
                            EditorLayerUnionRef::Physics { layer, .. } => (
                                MapTileLayerTiles::Physics(match layer {
                                    ...
                                    EditorPhysicsLayer::Tele(_) => {
                                        MapTileLayerPhysicsTiles::Tele(todo!())
                                    }
                                    EditorPhysicsLayer::Speedup(_) => {
                                        MapTileLayerPhysicsTiles::Speedup(todo!())
                                    }
                                    EditorPhysicsLayer::Switch(_) => {
                                        MapTileLayerPhysicsTiles::Switch(todo!())
                                    }
                                    EditorPhysicsLayer::Tune(_) => {
                                        MapTileLayerPhysicsTiles::Tune(todo!())
                                    }
                                }),
                                entities_container
    ...
    pub fn handle_brush_draw(
        ...
        // apply brush
            ...
            if latest_pointer.primary_down() {
                ...
                let mut brush_x = 0;
                let mut brush_y = 0;
                let mut brush_w = brush.w.get();
                let mut brush_h = brush.h.get();
                if x < 0 {
                                ...
                                ActTilePhysicsLayerReplaceTiles {
                                    base: ActTilePhysicsLayerReplTilesBase {
                                        ...
                                        old_tiles: match layer {
                                            EditorPhysicsLayer::Arbitrary(_) => todo!(),
                                            EditorPhysicsLayer::Game(layer) => {
                                                MapTileLayerPhysicsTiles::Game(Self::collect_tiles(
                                                    &layer.def.tiles,
    ...
    fn render_brush_repeating_internal(
        ...
        if let BrushVisual::Design(TileLayerVisuals {
            buffer_object_index: Some(buffer_object_index),
            base,
        })
            ...
            while height > 0 {
                ...
                for y in 0..brush_h {
                    ...
                    while width > 0 {
                        ...
                        let draw_count = brush_w as usize * 6;
                        ...
                        brush.map_render.render_tile_layer(
                            ...
                            PoolVec::from_without_pool(vec![quad_offset]),
                            PoolVec::from_without_pool(vec![draw_count]),
                            1,
                        );
    ...
    fn render_brush_internal(
        ...
        }) = &brush.render
        {
            ...
            brush.map_render.render_tile_layer(
                ...
                buffer_object_index,
                &ColorRGBA::new(1.0, 1.0, 1.0, 1.0),
                PoolVec::from_without_pool(vec![0]),
                PoolVec::from_without_pool(vec![
                    brush.w.get() as usize * brush.h.get() as usize * 6,
                ]),
                1,
            );
        }
    ...
    fn render_brush(
        ...
        if let Some(TileBrushDownPos { world, .. }) = &self.shift_pointer_down_world_pos {
            ...
            let width = (pos_cur.x - pos_old.x).abs() as u16;
            let height = (pos_cur.y - pos_old.y).abs() as u16;
            if width > 0 && height > 0 {
                self.render_brush_repeating_internal(
                    brush,
                    map,
                    canvas_handle,
                    map.groups.user.pos - pos_old,
                    None,
                    usvec2::new(
                        (pos_cur.x - pos_old.x)
                            .clamp(f32::MIN, 0.0)
    ...
    pub fn update(
        ...
        if self.brush.is_none() || self.pointer_down_world_pos.is_some() {
    ...
    pub fn render(
        ...
        if latest_keys_down.contains(&egui::Key::Space) {
            ...
            let texture = match layer.as_ref().unwrap() {
                EditorLayerUnionRef::Physics { .. } => {
                    &entities_container.get_or_default(&"default".into()).physics
                }
            ...
            self.tile_picker.map_render.render_tile_layer(
                ...
                &color,
                PoolVec::from_without_pool(vec![0]),
                PoolVec::from_without_pool(vec![16 * 16 * 6]),
                1,
            );
