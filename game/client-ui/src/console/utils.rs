...
use client_types::console::{
    parser::{self, CommandParseResult, CommandType, CommandsTyped, Syn},
    ConsoleEntry,
};
use config::{
    config::ConfigEngine,
...
pub fn syn_vec_to_config_val(args: &[(Syn, Range<usize>)]) -> Option<String> {
    args.get(0).map(|(arg, _)| match arg {
...
pub fn try_apply_config_val(
    ...
    config_engine
        .try_set_from_str(cmd_text.clone(), None, set_val.clone(), None, 0)
        .or_else(|err| {
            config_game
                .try_set_from_str(cmd_text.clone(), None, set_val.clone(), None, 0)
                .or_else(|err_game| {
