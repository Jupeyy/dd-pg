...
use shared_base::{
    ...
    server_browser::{ServerBrowserInfo, ServerBrowserServer},
};
...
pub fn render(mut body: TableBody<'_>, pipe: &mut UiRenderPipe<UserData>, cur_page: &str) {
    ...
    body.rows(
        ...
        |mut row| {
            ...
            let (clicked, restart_clicked) = super::entry::render(row, server, cur_page == "LAN");
            let clicked = clicked
