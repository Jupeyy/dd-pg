...
impl<const CAP: usize> Decode for NetworkStr<CAP> {
    fn decode<D: bincode::de::Decoder>(
        ...
        let decoded: Self = bincode::serde::decode_from_reader(decoder.reader(), conf)?;
        Ok(decoded)
    }
}
...
impl<'a, const CAP: usize> bincode::BorrowDecode<'a> for NetworkStr<CAP> {
    fn borrow_decode<D: bincode::de::BorrowDecoder<'a>>(
        ...
        let conf = *decoder.config();
        let decoded: String = bincode::decode_from_reader(decoder.reader(), conf)?;
...
#[derive(Decode, Encode, Serialize, Deserialize, Copy, Clone, Default, Debug, PartialEq, Eq)]
pub enum WeaponType {
    // TODO
    #[default]
    TODO,
...
pub struct InputVar<V> {
    pub val: V,
    // the version increases by releasing/moving the key
    pub change_version: u64,
}

impl<V: PartialEq> InputVar<V> {
...
impl<V> DerefMut for InputVar<V> {
...
pub struct MsgObjPlayerInput {
    pub dir: InputVar<i32>,
    pub cursor: InputVar<MsgObjPlayerInputCursor>,

    pub jump: InputVar<bool>,
    pub fire: InputVar<bool>,
    pub hook: InputVar<bool>,
    pub flags: InputVar<i32>,
    pub weapon_req: InputVar<WeaponType>,
    // TODO: next prev weapon, needed?
    pub weapon_diff: InputVar<i32>,
}
