...
use anyhow::anyhow;
use base::{benchmark::Benchmark, join_all, reduced_ascii_str::ReducedAsciiString};
...
impl CDatafileWrapper {
    ...
    pub fn into_map(self, images: &[Vec<u8>]) -> anyhow::Result<Map> {
        ...
        let mut tune_zones: LinkedHashMap<u8, MapLayerTilePhysicsTuneZone> = Default::default();
...
mod test {
    ...
    fn convert_back_and_forth_for_map(map_name: &str) {
        ...
        let map_legacy = io.io_batcher.spawn(async move {
            let map = fs
                .read_file(format!("legacy/maps/{}.map", map_name2).as_ref())
                .await?;
        ...
        let legacy_images: Vec<_> = map_legacy
            .images
            .iter()
            .map(|i| (i.img_name.clone(), i.internal_img.clone(), i.item_data))
            .collect();

        let map = map_legacy.into_map(&images).unwrap();
        let images_new: Vec<Vec<u8>> =
            map.resources
                .images
                .iter()
                .map(|i| {
                    if let Some((index, _)) = read_files.keys().enumerate().find(|(_, name)| {
                        **name == format!("legacy/mapres/{}.png", i.name.as_str())
                    }) {
                        images[index].clone()
                    } else {
                        let (_, internal_img, item_data) = legacy_images
                            .iter()
                            .find(|(name, _, _)| i.name.as_str().eq(name))
                            .unwrap();
                        save_png_image(
                ...
                .collect();
        let image_arrays_new: Vec<Vec<u8>> =
            map.resources
                .image_arrays
                .iter()
                .map(|i| {
