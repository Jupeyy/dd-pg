...
use egui::{
    epaint::PathStroke, pos2, text::LayoutJob, vec2, Align2, Color32, DragValue, FontId, Key,
    KeyboardShortcut, Modifiers, Pos2, Rect, RichText, Shape, Stroke, TextFormat, UiBuilder, Vec2,
};
...
impl Timeline {
    ...
    fn handle_input_value_points(
        ...
        if !is_readonly {
            ...
            ui.input(|i| {
                ...
                let pointer_in_point_radius = |point: &mut dyn Point| {
                    let point_center = self.offset_of_point(point.time());
    ...
    fn controls_ui(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            } else if ui.button(Self::icon_font_text(ui, "\u{f04c}")).clicked() {
                self.play_dir = PlayDir::Paused;
                self.last_time = None;
            }
        });
