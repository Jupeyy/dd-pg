...
impl Players {
    ...
    fn render_weapon(
        ...
        let weapon_texture = weapons.get_or_default::<ContainerKey>(&"TODO".try_into().unwrap());
    ...
    fn render_ninja_weapon(
        ...
        let ninja = ninjas.get_or_default::<ContainerKey>(&"TODO".try_into().unwrap());
        self.quad_container.render_quad_container_as_sprite(
            quad_offset,
            weapon_pos.x,
            weapon_pos.y,
    ...
    pub fn render(&mut self, pipe: &mut PlayerRenderPipe) {
        ...
        for (character_id, player_render_info) in render_infos.iter() {
            ...
            let in_air = !pipe
                .collision
                .check_pointf(pos.x * 32.0, (pos.y + 0.5) * 32.0);
            ...
            let character_info = pipe.character_infos.get(character_id);
            ...
            let skin = if is_ninja {
                ...
                pipe.skins
                    .get_or_default_opt(character_info.map(|char| &char.info.skin))
            };
            ...
            let tee_render_info = TeeRenderInfo {
                ...
                feet_flipped: false,
                size: 2.0, // yes a tee is 2 tiles big (rendering wise)
            ...
            if let Some(name) = character_info
                ...
                .and_then(|n| {
                    (pipe.nameplates
                        && (pipe.own_nameplate
                            || !pipe.own_character.is_some_and(|id| *id == *character_id)))
                    .then_some(n)
                })
            {
