...
impl Players {
    ...
    fn render_hook(
        ...
        if let Some(hook_pos) = player_render_info.lerped_hook_pos {
            ...
            // current hook
            let cur_hook = hooks.get_or_default(&"TODO:".into());
    ...
    fn render_weapon(
        ...
        let weapon_texture = weapons.get_or_default(&"TODO:".into());
        ...
        if true {
            if current_weapon == WeaponType::Gun || current_weapon == WeaponType::Shotgun {
                ...
                if !weapon.muzzles.is_empty() {
                    ...
                    let muzzle_index = self.rng.random_int() as usize % weapon.muzzles.len();
                    ...
                    if alpha_muzzle > 0.0 {
                        ...
                        let texture = &weapon.muzzles[muzzle_index];
    ...
    fn render_ninja_weapon(
        ...
        let ninja = ninjas.get_or_default(&"TODO:".into());
        self.quad_container.render_quad_container_as_sprite(
            quad_offset,
            weapon_pos.x,
            weapon_pos.y,
        ...
        if attack_time <= 1.0 / 6.0 && !ninja.muzzles.is_empty() {
            let muzzle_index = self.rng.random_int() as usize % ninja.muzzles.len();
            ...
            self.quad_container.render_quad_container_as_sprite(
                ...
                1.0,
                quad_scope,
                (&ninja.muzzles[muzzle_index]).into(),
            );
        }
    ...
    pub fn render(&mut self, pipe: &mut PlayerRenderPipe) {
        ...
        for (character_id, player_render_info) in render_infos.iter() {
            ...
            let skin = if is_ninja {
                &pipe.ninjas.get_or_default(&"TODO:".into()).skin
            } else {
                let mut dummy = None;
