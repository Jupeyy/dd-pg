...
pub struct GameObjectsRenderPipe<'a> {
    ...
    pub projectiles: &'a LinkedHashMap<ProjectileId, ProjectileRenderInfo>,
    pub flags: &'a LinkedHashMap<CtfFlagId, FlagRenderInfo>,
    pub lasers: &'a LinkedHashMap<LaserId, LaserRenderInfo>,
    pub pickups: &'a LinkedHashMap<PickupId, PickupRenderInfo>,
...
impl GameObjectsRender {
    ...
    pub fn render_projectile(
        ...
        if ty == WeaponType::Grenade {
            ...
            effects.smoke_trail(&pos, &(vel * -1.0), alpha, 0.0, proj.owner_id);

            quad_scope
                .set_rotation((pipe.cur_time.as_secs_f32() as f64 * PI_F64 * 2.0 * 2.0) as f32);
        } else {
            let mut effects = Effects::new(pipe.particle_manager, *pipe.cur_time);
            effects.bullet_trail(&pos, alpha, proj.owner_id);
