...
use game_config::config::{ConfigDummyScreenAnchor, ConfigGame, ConfigMap};
use game_interface::{
...
use shared_base::network::{messages::RenderModification, types::chat::NetChatMsg};
use sound::{
...
pub struct RenderGameResult {
    pub player_events: LinkedHashMap<PlayerId, Vec<PlayerFeedbackEvent>>,
}
...
pub struct RenderGameInput {
    pub players: PoolLinkedHashMap<PlayerId, RenderGameForPlayer>,
    pub dummies: PoolLinkedHashSet<PlayerId>,
    ...
    pub character_infos: PoolLinkedHashMap<CharacterId, CharacterInfo>,
    pub stages: PoolLinkedHashMap<StageId, StageRenderInfo>,
...
impl RenderGame {
    ...
    fn render_ingame(
        ...
        } else if let Some(character_stage_info) = character_info.and_then(|c| {
            c.stage_id
        ...
        if let Some((_, p)) = player_info {
            cam.pos = match p.cam_mode {
                ...
                RenderPlayerCameraMode::OnCharacter {
                    ...
                    .character_infos
                    .get(&character_id)
                    .and_then(|character| character.stage_id)
                    .and_then(|stage_id| render_info.stages.get(&stage_id))
                    .and_then(|stage| stage.world.characters.get(&character_id))
                    .map(|character| character.lerped_pos)
            ...
            )
        {
            ...
            self.players.render(&mut PlayerRenderPipe {
                ...
                spartial_sound: render_info.settings.spartial_sound,
...
impl RenderGame {
    ...
    fn handle_action_feed(
        &mut self,
        cur_time: &Duration,
        character_infos: &PoolLinkedHashMap<CharacterId, CharacterInfo>,
        ev: GameWorldAction,
    ) {
        match ev {
    ...
    fn sound_or_effect_event_precond(
        ...
        local_players: &PoolLinkedHashMap<PlayerId, RenderGameForPlayer>,
        local_dummies: &PoolLinkedHashSet<PlayerId>,
                ...
                let main_listener_character_info = match local_players.front() {
                    Some((player_id, p)) => {
                        let char_id = match &p.render_for_player.cam_mode {
                            ...
                            RenderPlayerCameraMode::AtPos(_) => None,
                            RenderPlayerCameraMode::OnCharacter { character_id, .. } => {
                                Some(character_id)
                            }
                        };
        ...
        for (player_id, player) in input.players.iter() {
            let cam_pos = match player.render_for_player.cam_mode {
                RenderPlayerCameraMode::Default => input
                    ...
                    .map(|c| c.lerped_pos),
                RenderPlayerCameraMode::AtPos(at) => Some(at),
                RenderPlayerCameraMode::OnCharacter {
                    character_id,
                    fallback_pos,
                } => Some(
