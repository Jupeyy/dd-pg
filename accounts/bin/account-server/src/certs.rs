...
use crate::{db::DbConnectionShared, internal_err::InternalErr, shared::Shared};
...
pub fn generate_key_and_cert(
    ...
    let time_secs = if first_key { 1 } else { 2 } * 30 * 24 * 60 * 60;
    let validity = Validity::from_now(Duration::new(time_secs, 0))?;
...
pub async fn certs_request(shared: Arc<Shared>) -> response::Result<Json<Vec<Vec<u8>>>> {
    ...
    Ok(Json(
        certs
            ...
            .collect::<anyhow::Result<Vec<_>>>()
            .map_err(|err| InternalErr(("certs".into(), err)))?,
    ))
}
