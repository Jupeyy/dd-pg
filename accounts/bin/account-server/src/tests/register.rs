...
use accounts_shared::account_server::{register::RegisterResponse, sign::AuthResponse};
use anyhow::anyhow;
...
async fn account_register_hardening() {
    let test = async move {
        ...
        // invalid otps
        let register_data = accounts_shared::client::register::register_data(
            Default::default(),
            Default::default(),
            EmailAddress::from_str("test@localhost")?,
            "",
        )?;
        let register_res = crate::login_token_email::register_impl(
        ...
        let mut register_data = accounts_shared::client::register::register_data(
            [
