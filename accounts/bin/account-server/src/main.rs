...
pub(crate) async fn prepare_http(
    ...
    let shared_clone = shared.clone();
    let pool_clone = pool.clone();
    let login_email = axum::Router::new().route(
        "/email",
        axum::routing::post(move |payload: Json<_>| {
    ...
    app = app.nest(
        "/login",
        Router::new().merge(token_email).merge(login_email),
    );
...
pub(crate) async fn prepare(
    ...
    let (listener, app, shared) =
        prepare_http(&details.http, db, email, &pool, &details.limitter).await?;
