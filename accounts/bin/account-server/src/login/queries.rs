...
impl<'a> Query<LoginTokenData> for LoginTokenQry<'a> {
    async fn prepare_mysql(
        ...
        Ok(connection
            .prepare(include_str!("login_token_data.sql"))
            .await?)
    }
    fn query_mysql<'b>(
        &'b self,
    ...
    fn row_data(row: &MySqlRow) -> anyhow::Result<LoginTokenData> {
        Ok(LoginTokenData {
            email: row
                .try_get("email")
...
impl<'a> Query<()> for InvalidateLoginToken<'a> {
    async fn prepare_mysql(
        ...
        Ok(connection
            .prepare(include_str!("invalidate_login_token.sql"))
            .await?)
    }
    fn query_mysql<'b>(
        &'b self,
...
impl<'a> Query<()> for TryCreateAccount<'a> {
    async fn prepare_mysql(
        ...
        Ok(connection.prepare(include_str!("add_account.sql")).await?)
    }
    fn query_mysql<'b>(
        &'b self,
        statement: &'b sqlx::mysql::MySqlStatement<'static>,
...
impl<'a> Query<LoginData> for LoginQry<'a> {
    ...
    fn row_data(row: &MySqlRow) -> anyhow::Result<LoginData> {
        Ok(LoginData {
            account_id: row
                .try_get("id")
