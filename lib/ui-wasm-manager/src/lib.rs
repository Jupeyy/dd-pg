...
use anyhow::anyhow;
use base_fs_traits::traits::{FileSystemInterface, FileSystemWatcherItemInterface};
...
enum UIRenderCallback {
    Wasm(UIWasmEntry),
    Native(Box<dyn UIRenderCallbackFunc<(), GraphicsBackend>>),
}
...
pub enum UIWasmLoadingType {
    ShowCurrentPage,
    ShowLoadingPage(Box<dyn UIRenderCallbackFunc<(), GraphicsBackend>>),
}

const MODS_PATH: &str = "mods/ui";

const BLUR_RADIUS: f32 = 13.0;
...
impl UIWasmManager {
    ...
    pub fn new(
        ...
        error_404_page: Box<dyn UIRenderCallbackFunc<(), GraphicsBackend>>,
    ...
    pub fn register_path(
        ...
        mod_name: &str,
        path: &str,
        cb: Box<dyn UIRenderCallbackFunc<(), GraphicsBackend>>,
    ) -> Option<()> {
    ...
    pub fn run_ui_path(
        ...
        match self.ui_paths.get_mut(path) {
            Some(cb) => match cb {
                UIRenderCallback::Wasm(ui) => {
                    if blur && ui.call_has_blur().unwrap_or(false) {
                        ...
                        match ui.call_main_frame(
                            ...
                            Ok(_) => {
                                render_blur(graphics, true, BLUR_RADIUS, &BLUR_COLOR);
                    ...
                    match ui.call(
                        ...
                        Ok(output) => {
                            ...
                            if self.ui.context.egui_ctx.pixels_per_point() != output.zoom_level
                                || self
                                    .ui
                                    .ui_state
                                    .zoom_level
                ...
                UIRenderCallback::Native(cb) => {
                    let window_width = graphics.window_width();
                    let window_height = graphics.window_height();
                    let window_pixels_per_point = graphics.window_pixels_per_point();
                    if blur && cb.has_blur() {
                        graphics.next_switch_pass();
                    ...
                    if self.ui.context.egui_ctx.pixels_per_point() != zoom_level
                        || self
                            .ui
                            .ui_state
                            .zoom_level
