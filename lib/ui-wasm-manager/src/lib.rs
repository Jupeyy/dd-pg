    ...
    for<'a> U: 'a,
{
                        ...
                        match loading_entry {
                            Ok(loading_entry) => match loading_entry.get_storage() {
                                Ok(item) => {
                                    ...
                                    self.run_ui_path(
                                        path, io, graphics, backend, sound, pipe, inp, window, blur,
                                    )
                                }
                                Err(err) => {
    ...
    pub fn render(
        ...
        let (success, platform_output) = match self.run_ui_path(
            path,
            io,
            graphics,
            backend,
                ...
                if self.show_cur_page_during_load {
                    match self.run_ui_path(
                        ...
                        UiPageRunReturn::RuntimeError(_) => (
                            false,
                            self.render(
                                "404", io, graphics, backend, sound, pipe, inp, window, blur,
                            ),
                        ),
                    }
                } else {
            ...
            UiPageRunReturn::Error404(err) => {
                self.err.set(err);
                match self.run_ui_path("404", io, graphics, backend, sound, pipe, inp, window, blur)
                {
                    UiPageRunReturn::Loading => {
                        panic!("this should never happen")
                    }
