...
use super::{
    connection::{
        NetworkConnection, NetworkConnectionID, INVALID_NETWORK_CON_IDENTIFIER,
        MIN_NETWORK_CON_IDENTIFIER,
    },
    event::NetworkEvent,
        ...
        tokio::spawn(async move {
            ...
            tokio::spawn(async move {
                let mut ping_interval = tokio::time::interval(if !is_server {
                    Duration::from_secs(1) / 50 // 50 per second for client. The client is usually more interested in network stats.
                } else {
                    Duration::from_secs(1) / 1 // 1 per second for server.
                });
                let res = tokio::select! {
                    res = Self::handle_connection_recv_unordered_reliable(
                        ...
                        logger.clone(), debug_printing,  packet_plugins.clone()) => {res}
                    ...
                    connection_identifier, sys.clone(), logger.clone(), debug_printing, packet_plugins.clone()) => {res}
