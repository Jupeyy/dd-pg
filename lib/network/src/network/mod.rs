...
pub mod tests {
    ...
    use crate::network::{
        network::{Network, NetworkInOrderChannel},
        quinn_network::{
    ...
    use super::{
        network::{
            NetworkClientInitOptions, NetworkConnectingInterface, NetworkConnectionID,
            NetworkConnectionInterface, NetworkEndpointInterface, NetworkEventToGameEventGenerator,
            NetworkGameEvent, NetworkServerInitOptions,
        },
    ...
    impl NetworkEventToGameEventGenerator for TestGameEventGenerator {
        async fn generate_from_binary(
            ...
            let msg = bincode::decode_from_slice::<TestGameMessage, _>(
                bytes,
        ...
        async fn generate_from_network_event(
            ...
            match network_event {
                NetworkGameEvent::Disconnected(_) => {
                    self.is_shutdown
                        .store(true, std::sync::atomic::Ordering::SeqCst);
                    true
                }
                ...
                NetworkGameEvent::NetworkStats(_) => false, // don't notify because of a ping event
                _ => true,
            }
        }
    }
        ...
        E: NetworkEndpointInterface<Z> + Clone + Send + Sync + 'static,
    {
        ...
        let (mut network_server, _, addr, notifier_server) = Network::<E, C, Z>::init_server(
            "0.0.0.0:0",
        ...
        let (mut network_client, notifier) = Network::<E, C, Z>::init_client(
            ...
            &sys,
            Some(NetworkClientInitOptions::new().with_debug_priting(true)),
        );
        ...
        *game_event_generator_server.cur_test_name.blocking_lock() = "huge packet".to_string();
        ...
        network_client2.close();
        network_client3.close();
    ...
    */
    ...
    fn max_datagram_size_tests() {
        ...
        let game_event_generator_server = Arc::new(TestGameEventGenerator::new());
        let game_event_generator_client = Arc::new(TestGameEventGenerator::new());
        let (mut network_server, _, addr, notifier_server) = QuinnNetwork::init_server(
            "0.0.0.0:0",
            game_event_generator_server.clone(),
            &cert,
            &sys,
        ...
        for i in 0..12 {
            ...
            let t = std::thread::spawn(move || {
                ...
                loop {
                    ...
                    total_packets_thread.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                    total_packets_this_thread.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
