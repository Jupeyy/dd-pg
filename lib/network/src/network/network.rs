...
pub enum NetworkClientCertCheckMode<'a> {
    ...
    CheckByCertHash { hash: &'a Hash },
    // not recommended, only useful for debugging
    DisableCheck,
}

pub enum NetworkServerCertMode<'a> {
    FromCert {
        cert: &'a Certificate,
    },
    FromPrivatePubDer {
...
impl<'a> NetworkClientInitOptions<'a> {
    pub fn new(cert_check: NetworkClientCertCheckMode<'a>) -> Self {
        Self {
            ...
            while let Some(event) = events.events.pop_front() {
                match event {
                    ...
                    NetworkLogicEvent::Send((connection_id, packet, packet_order)) => {
                        let packet_send = NetworkPacket::User {
                            signature: thread
                                .sign_key
                                .as_ref()
                                .map(|sign_key| sign_key.sign(&packet)),
                            packet,
                        };
                        let connections_ = thread.connections.clone();
                        let con_id = connection_id;
