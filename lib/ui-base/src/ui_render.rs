    ...
    B: GraphicsBackendInterface + 'static,
{
            ...
            (*start_index..*end_index)
                ...
                .for_each(|prim_index| {
                    ...
                    while remaining_indices > 0 {
                        let mut vertices = draw_triangles.get_raw_handle(remaining_indices / 3);

                        match &clipped_primitives[prim_index].primitive {
                            Primitive::Mesh(mesh) => {
                                vertices.get().iter_mut().enumerate().for_each(
                                    |(vert_index, vertex)| {
    ...
    full_output.textures_delta.free.iter().for_each(|tex_id| {
        let _ = ui.textures.remove(tex_id);
    });
