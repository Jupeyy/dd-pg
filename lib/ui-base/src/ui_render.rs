...
use graphics::{
    graphics::Graphics,
    streaming::{DrawScopeImpl, DrawStreamImplSimplified},
};
use graphics_types::{
    commands::{TexFlags, TexFormat},
    rendering::{BlendType, ColorMaskMode, GlColor, StencilMode, WrapType, WriteVertexAttributes},
...
fn render_ui_impl<T, C1: 'static, C2: 'static, C3: 'static>(
    ui: &mut UI<T>,
    ...
    clipped_primitives.iter().for_each(|v| match &v.primitive {
        Primitive::Mesh(mesh) => {
            ...
            draw_triangles.set_stencil_mode(if as_stencil {
                StencilMode::FillStencil
            } else {
                StencilMode::None
            });
            draw_triangles.set_color_mask(if as_stencil {
...
pub fn render_ui_1<T, C1: 'static>(
    ui: &mut UI<T>,
    native_pipe: &mut UINativePipe<T>,
    custom_callback_type1: &mut C1,
    full_output: FullOutput,
...
pub fn render_ui_2<T, C1: 'static, C2: 'static>(
    ui: &mut UI<T>,
    native_pipe: &mut UINativePipe<T>,
    custom_callback_type1: &mut C1,
    custom_callback_type2: &mut C2,
