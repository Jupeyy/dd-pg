use std::{collections::BTreeSet, hash::Hash, iter::Peekable};
...
impl<T: Clone + Hash + PartialEq + Eq> Binds<T> {
    ...
    fn process_impl(&mut self, consume_click_events: bool) -> BindsProcessResult<T> {
        ...
        fn find_longest_chain_action<'a, F>(
                ...
                Some(next_key) => match keys.get(next_key) {
                    Some(key_binds) => match key_binds {
                        ...
                        BindTarget::ScancodeAndActions((cur_scan, actions)) => {
                            let res = find_longest_chain_action(key_iter.clone(), cur_scan, false);
