...
mod tests {
    ...
    fn prepare_backend() -> (
        ...
        let tp = Arc::new(rayon::ThreadPoolBuilder::new().build().unwrap());
        ...
        config_dbg.gfx = config::config::EDebugGFXModes::All;
        let config_gl = config::config::ConfigBackend::default();
    ...
    #[test]
    fn vk_backend() {
        let (backend, stream_data) = prepare_backend();
    ...
    fn shader_compile() {
        ...
        let backend_files = io.io_batcher.spawn(async move {
            ...
            compiler.compile("shader/wgsl", "compile.json").await?;
