...
pub struct GraphicsBackend {
    ...
    backend_cmds_in_use: BackendCommands,
...
impl GraphicsBackend {
    pub fn new() -> GraphicsBackend {
        GraphicsBackend {
            ...
            backend_cmds_in_use: BackendCommands::default(),
    ...
    pub fn init(&mut self) -> Result<Rc<RefCell<dyn GraphicsStreamDataInterface>>, ()> {
        ...
        buffer.cmds.borrow_mut().push(AllCommands::Misc(cmd_swap));
    ...
    fn run_cmds_impl(
        ...
        backend.start_commands(buffer, &stream_data, buffer.cmds.borrow().len(), 0);

        for cmd in &*buffer.cmds.borrow() {
        ...
        self.backend_cmds_in_use.cmds.borrow_mut().clear();
        stream_data.borrow_mut().set_vertices_unsafe(res.unwrap());
        std::mem::swap(
            &mut *self.backend_cmds_in_use.cmds.borrow_mut(),
            &mut *buffer.cmds.borrow_mut(),
        );
    }
    ...
    pub fn resized(
        ...
        buffer
            .cmds
            .borrow_mut()
            .push(AllCommands::Misc(cmd_viewport));
