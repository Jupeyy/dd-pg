...
use base_io::io_batcher::TokIOBatcherTask;
...
use graphics_types::{
    command_buffer::{AllCommands, CommandUpdateViewport, Commands, SBackendCapabilites},
...
impl GraphicsBackendBase {
    ...
    pub fn load_io(&mut self, io_pipe: &mut GraphicsBackendLoadIOPipe) {
        let fs = io_pipe.io.fs.clone();
        self.backend_files = Some(io_pipe.io.io_batcher.spawn(async move {
    ...
    pub fn init_while_io(&mut self, pipe: &mut GraphicsBackendLoadWhileIOPipe) {
        ...
        let benchmark = Benchmark::new(pipe.config.dbg.bench);
        ...
        benchmark.bench("\tinitializing the backend instance (while io)");
        ...
        self.backend
            ...
            .unwrap();
        benchmark.bench("\tinitializing the backend (while io)");
    ...
    pub fn init(&mut self) -> Result<Rc<RefCell<dyn GraphicsStreamDataInterface>>, ()> {
        ...
        let stream_data: Rc<RefCell<dyn GraphicsStreamDataInterface>> =
            Rc::new(RefCell::new(BackendStreamData::default()));
        self.run_cmds(&buffer, &stream_data);
        Ok(stream_data)
    }
    ...
    fn run_cmds_impl(
        ...
        for cmd in self.backend_cmds_in_use.drain(..) {
            backend.run_command(cmd).unwrap(); // TODO: don't unwrap, return instead
        }
