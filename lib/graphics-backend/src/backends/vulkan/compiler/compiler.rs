...
impl ShaderCompiler {
    ...
    pub async fn compile(&mut self, path: &str, compile_json_file: &str) -> anyhow::Result<()> {
        let json_file = self
            .fs
            .open_file(&(path.to_string() + "/" + compile_json_file))
            .await?;
        ...
        for file_res in
            futures::future::join_all(files_to_compile.into_iter().map(|file_to_compile| {
                ...
                async move {
                    let shader_source = fs
                        .open_file(&(path.to_string() + "/" + &file_to_compile.input))
                        .await?;
