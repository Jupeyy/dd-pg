...
use base::{benchmark::Benchmark, join_thread::JoinThread};
...
impl VulkanBackendLoading {
    ...
    pub fn new(
        ...
        let auto_thread_count = if options.gl.thread_count == 0 {
            // auto should not alloc more than 8 threads
...
impl VulkanBackend {
    ...
    fn cmd_switch_canvas_mode(&mut self, cmd: CommandSwitchCanvasMode) -> anyhow::Result<()> {
        ...
        match &cmd.mode {
            CommandSwitchCanvasModeType::Offscreen {
                ...
                should_queue_full_compile: self.render_setup_queue_full_pipeline_creation,
            })?,
    ...
    fn wait_frame(&mut self) -> anyhow::Result<()> {
        ...
        for (_, current_command_group) in self.current_command_groups.drain() {
            if !current_command_group.cmds.is_empty() {
                self.command_groups.push(current_command_group);
            }
        }
    ...
    pub fn new(
        ...
        for i in 0..thread_count {
            ...
            let thread = std::thread::Builder::new()
                ...
                .spawn(move || {
                    Self::run_thread(receiver, frame, device, queue_index, custom_pipes)
                })?;
