...
use graphics_types::{
    command_buffer::{
        ...
        CommandsRenderStream, ERunCommandReturnTypes, PrimType, SBackendCapabilites,
        SQuadRenderInfo, SRenderSpriteInfo, StreamDataMax, TexFlags,
        GRAPHICS_MAX_PARTICLES_RENDER_COUNT, GRAPHICS_MAX_QUADS_RENDER_COUNT,
    },
...
impl VulkanBackend {
        ...
        match &cmd_param {
            AllCommands::Render(render_cmd) => match render_cmd {
                ...
                CommandsRender::Stream(cmd) => match cmd {
                    CommandsRenderStream::Render(cmd) => self.cmd_render(cmd, exec_buffer),
    ...
    fn prepare_presented_image_data_image(
        ...
        if self.get_presented_img_data_helper_image == vk::Image::null() || needs_new_img {
            ...
            self.get_presented_img_data_helper_mem.size = mem_alloc_info.allocation_size;
    ...
    fn get_presented_image_data_impl(
        ...
        if uses_rgba_like_format && self.last_presented_swap_chain_image_index != u32::MAX {
            ...
            *height = viewport.height;
            ...
            let final_layout = self.device.final_layout();
            let swap_img =
    ...
    #[must_use]
    fn get_vulkan_extensions(window: &BackendWindow) -> Result<Vec<String>, ArrayString<4096>> {
        let mut vk_extensions = Vec::<String>::new();

        let ext_list_res = window.enumerate_required_vk_extensions();
    ...
    fn get_presentation_mode(&mut self, vk_io_mode: &mut vk::PresentModeKHR) -> bool {
        let res = unsafe {
            self.ash_vk
                .surface
                .get_physical_device_surface_present_modes(self.vk_gpu)
        };
        if res.is_err() {
            self.error.lock().unwrap().set_error(
                EGFXErrorType::Init,
    ...
    fn get_surface_properties(
        ...
        let capabilities_res = unsafe {
            self.ash_vk
                .surface
                .get_physical_device_surface_capabilities(self.vk_gpu)
        };
    ...
    fn get_format(&mut self) -> bool {
        ...
        let res = unsafe {
            self.ash_vk
                .surface
                .get_physical_device_surface_formats(self.vk_gpu)
        };
        if res.is_err() && *res.as_ref().unwrap_err() != vk::Result::INCOMPLETE {
            self.error.lock().unwrap().set_error(
                EGFXErrorType::Init,
    ...
    fn create_render_pass_impl(
        ...
        color_attachment.final_layout = self.device.final_layout();
    ...
    fn create_sync_objects(&mut self) -> bool {
        ...
        let create_semaphore_info = create_semaphore_info_builder.build();
    ...
    > {
        ...
        let layers_res = Self::get_vulkan_layers(dbg_mode, &entry);
        if let Err(err) = layers_res {
            return Err(err);
        }
        let mut layers = layers_res.unwrap();

        let instance_res = Self::create_vulkan_instance(
            dbg_mode,
            &entry,
            error,
            &mut layers,
        ...
        let gpu_res = Self::select_gpu(&instance, dbg_mode, logger, window.is_headless());
        ...
        let device_res = Self::create_logical_device(
            &physical_gpu,
            graphics_queue_index,
            &instance,
            &layers,
        ...
        let mut surface = window.create_surface(&entry, &instance).unwrap();
    ...
    */
    ...
    pub fn init_instance_while_io(
        ...
        let swap_chain = ash_surface.create_swapchain(&instance, &device).unwrap();
