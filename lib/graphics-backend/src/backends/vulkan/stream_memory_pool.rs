...
impl<T> StreamMemory<T> {
    pub fn flush(
        ...
        if flush_size > 0 {
            ...
            mem_range.memory = self.mem.inner_arc().mem(frame_resources);
...
impl<T> StreamMemoryPool<T> {
    ...
    pub fn try_alloc(
        &mut self,
        mut new_instance_func: impl FnMut(
            &HiArc<Buffer>,
            vk::DeviceSize,
            usize,
        ) -> anyhow::Result<Vec<T>>,
    ...
    pub fn get(
        &mut self,
        new_instance_func: impl FnMut(&HiArc<Buffer>, vk::DeviceSize, usize) -> anyhow::Result<Vec<T>>,
        count: usize,
