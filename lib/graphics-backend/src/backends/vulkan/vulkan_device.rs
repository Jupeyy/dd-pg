...
use graphics_types::command_buffer::{GlVertexTex3DStream, StreamDataMax};
use libc::c_void;
...
impl Device {
    pub fn new(
        ...
        Self {
            ...
            logger: log.logger("vulkan_device"),
        }
    }
    ...
    pub fn shrink_unused_caches(&mut self) {
        ...
        freeed_memory += self.staging_buffer_cache.shrink(&mut self.ash_vk.device);
        freeed_memory += self
            .staging_buffer_cache_image
            .shrink(&mut self.ash_vk.device);
        if freeed_memory > 0 {
            ...
            if is_verbose(&*self.dbg) {
                self.logger
                    .log(LogLevel::Info)
                    .msg("deallocated chunks of memory with size: ")
                    .msg_var(&freeed_memory)
                    .msg(" from all frames (staging buffer)");
            }
        }
        freeed_memory = 0;
        freeed_memory += self.vertex_buffer_cache.shrink(&mut self.ash_vk.device);
        if freeed_memory > 0 {
        ...
        for image_buffer_cache in &mut self.image_buffer_caches {
            freeed_memory += image_buffer_cache.1.shrink(&mut self.ash_vk.device);
        }
        if freeed_memory > 0 {
    ...
    pub fn get_memory_command_buffer(
        ...
        if !self.used_memory_command_buffer[cur_image_index as usize] {
            ...
            if unsafe {
                self.ash_vk.device.reset_command_buffer(
    ...
    pub fn memory_barrier(
        ...
        unsafe {
            self.ash_vk.device.cmd_pipeline_barrier(
                *mem_command_buffer,
                source_stage,
                destination_stage,
    ...
    pub fn build_mipmaps(
        ...
        for i in 1..mip_map_level_count {
            ...
            unsafe {
                self.ash_vk.device.cmd_blit_image(
                    *mem_command_buffer,
                    image,
    ...
    pub fn create_texture_image(
        ...
        let buffer_block_res =
            allocator.get_mem_block_image(upload_data.mem.as_ptr() as *mut c_void);
    ...
    pub fn create_image_ex(
        ...
        let res = unsafe { self.ash_vk.device.create_image(&image_info, None) };
        if res.is_err() {
            // TODO dbg_msg("vulkan", "failed to create image!");
        }
        *image = res.unwrap();

        let mem_requirements = unsafe { self.ash_vk.device.get_image_memory_requirements(*image) };

        let image_allocation = self.get_image_memory(
            mem_requirements.size,
            mem_requirements.alignment,
            mem_requirements.memory_type_bits,
        );
        if let Err(_) = image_allocation {
            unsafe {
                self.ash_vk.device.destroy_image(*image, None);
            }
            return false;
        }
        *image_memory = image_allocation.unwrap();

        if let Err(_) = unsafe {
            self.ash_vk.device.bind_image_memory(
                *image,
    ...
    pub fn image_barrier(
        ...
        } else if old_layout == self.final_layout()
    ...
    pub fn copy_buffer_to_image(
        ...
        unsafe {
            self.ash_vk.device.cmd_copy_buffer_to_image(
                *command_buffer,
                buffer,
                image,
                vk::ImageLayout::TRANSFER_DST_OPTIMAL,
    ...
    where
        TStreamMemName: Default + StreamMemory + std::fmt::Debug,
    {
        let mut it: usize = 0;
        ...
        if buffer_mem.mem == vk::DeviceMemory::null() {
            ...
            let buffer_alloc = mem.create_buffer(
                new_buffer_size,
            ...
            if buffer_alloc.is_err() {
                return false;
            }
            let (stream_buffer, stream_buffer_memory) = buffer_alloc.unwrap();
        ...
        TStreamMemName: Default + StreamMemory + std::fmt::Debug,
    {
        ...
        if !Self::create_stream_buffer_unallocated::<
            TStreamMemName,
            TInstanceTypeName,
            INSTANCE_TYPE_COUNT,
            BUFFER_CREATE_COUNT,
    ...
    pub fn create_index_buffer(
        ...
        let staging_allocation =
            self.get_staging_buffer(ptr_raw_data, data_size as u64, cur_image_index);
        ...
        let buffer_alloc = self.mem.create_buffer(
            buffer_data_size,
        ...
        let (vertex_buffer, vertex_buffer_memory) = buffer_alloc.unwrap();
    ...
    pub fn create_buffer_object(
        ...
        let staging_buffer = allocator
            .get_mem_block(upload_data.mem.as_ptr() as *mut c_void)
            .unwrap();

        let block_allocation = self.get_vertex_buffer(buffer_data_size, cur_image_index);
        if let Err(_) = block_allocation {
            return false;
        }
        let mem = block_allocation.unwrap();
    ...
    pub fn delete_buffer_object(&mut self, buffer_index: u128, cur_image_index: u32) {
        let delete_obj = self.buffer_objects.remove(&buffer_index).unwrap();
    ...
    pub fn copy_buffer(
        ...
        unsafe {
            self.ash_vk.device.cmd_copy_buffer(
                *command_buffer,
                src_buffer,
                dst_buffer,
                &[copy_region],
    ...
    pub fn create_new_textured_descriptor_sets_impl(
        ...
        let res = unsafe { self.ash_vk.device.allocate_descriptor_sets(&des_alloc_info) };
        if res.is_err() {
            return false;
        }
    ...
    pub fn destroy_uniform_descriptor_set_layouts(&mut self) {
        ...
        unsafe {
            self.ash_vk.device.destroy_descriptor_set_layout(
