...
pub struct DescriptorLayouts {
    pub standard_textured_descriptor_set_layout: HiArc<DescriptorSetLayout>,
    pub standard_2d_texture_array_descriptor_set_layout: HiArc<DescriptorSetLayout>,

    pub vertex_uniform_descriptor_set_layout: HiArc<DescriptorSetLayout>,
    pub vertex_fragment_uniform_descriptor_set_layout: HiArc<DescriptorSetLayout>,

    pub samplers_layouts: Arc<[HiArc<DescriptorSetLayout>; SAMPLER_TYPES_COUNT]>,
}
...
pub struct Device {
    ...
    pub vk_gpu: HiArc<PhyDevice>,

    pub non_flushed_memory_ranges: Vec<vk::MappedMemoryRange>,

    pub samplers: Arc<[(HiArc<Sampler>, HiArc<DescriptorSets>); SAMPLER_TYPES_COUNT]>,

    pub textures: HashMap<u128, CTexture>,
    pub buffer_objects: HashMap<u128, BufferObject>,

    pub standard_texture_descr_pool: HiArc<parking_lot::Mutex<DeviceDescriptorPools>>,

    pub layouts: DescriptorLayouts,

    // command buffers
    pub command_pool: HiRc<CommandPool>,
...
impl Device {
    ...
    ) -> anyhow::Result<HiArc<DescriptorSetLayout>> {
    ...
    fn create_descriptor_set_layouts(
        device: &HiArc<LogicalDevice>,
    ) -> anyhow::Result<(HiArc<DescriptorSetLayout>, HiArc<DescriptorSetLayout>)> {
    ...
    fn create_texture_samplers(
        ...
        (HiArc<Sampler>, HiArc<DescriptorSetLayout>),
        (HiArc<Sampler>, HiArc<DescriptorSetLayout>),
        (HiArc<Sampler>, HiArc<DescriptorSetLayout>),
    )> {
        Ok((
    ...
    pub fn new(
        ...
        instance: HiArc<Instance>,
        device: HiArc<LogicalDevice>,
        vk_gpu: HiArc<PhyDevice>,

        graphics_queue: HiArc<Queue>,
        ...
        let samplers: [HiArc<Sampler>; SAMPLER_TYPES_COUNT] =
            [repeat.0, clamp_to_edge.0, texture_2d_array.0];

        let sampler_layouts: [HiArc<DescriptorSetLayout>; SAMPLER_TYPES_COUNT] =
    ...
    pub fn prepare_staging_mem_range(
        &mut self,
        frame_resources: &mut FrameResources,
        block: &HiArc<MemoryBlock>,
    ) {
        let block_mem = block.inner_arc().buffer_mem(frame_resources);
    ...
    pub fn create_texture_image(
        ...
        pixel_size: usize,
        mip_map_level_count: usize,
    ) -> anyhow::Result<(HiArc<Image>, MemoryImageBlock), ImageAllocationError> {
        ...
        if let FlushType::None = staging_and_image_buffer.is_flushed {
            let block_mem = staging_buffer.inner_arc().buffer_mem(frame_resources);
    ...
    pub fn create_texture_image_view(
        &self,
        frame_resources: &mut FrameResources,
        tex_image: &HiArc<Image>,
        img_format: vk::Format,
        view_type: vk::ImageViewType,
        depth: usize,
        mip_map_level_count: usize,
    ) -> anyhow::Result<HiArc<ImageView>> {
    ...
    ) -> anyhow::Result<(HiArc<Sampler>, HiArc<DescriptorSetLayout>)> {
    ...
    pub fn create_image_view(
        ...
        view_create_info.image = image.inner_arc().img(frame_resources);
    ...
    pub fn create_index_buffer(
        ...
        data_size: usize,
    ) -> anyhow::Result<(HiArc<Buffer>, HiArc<DeviceMemoryBlock>), BufferAllocationError> {
    ...
    pub fn create_buffer_object(
        ...
        let vertex_buffer = mem.inner_arc().buffer(frame_resources).clone().unwrap();
    ...
    ) -> anyhow::Result<HiArc<DescriptorSets>, ImageAllocationError> {
    ...
    pub fn create_new_sampler_descriptor_set(
        device: &HiArc<LogicalDevice>,
        layouts: &[HiArc<DescriptorSetLayout>; SAMPLER_TYPES_COUNT],
        sampler_descr_pool: &HiArc<parking_lot::Mutex<DeviceDescriptorPools>>,
        samplers: &[HiArc<Sampler>; SAMPLER_TYPES_COUNT],
    ...
    pub fn get_texture_sampler(
        samplers: &[HiArc<Sampler>; SAMPLER_TYPES_COUNT],
        sampler_type: ESupportedSamplerTypes,
    ) -> &HiArc<Sampler> {
        &samplers[sampler_type as usize]
    }
}
