...
pub struct VulkanAllocatorBufferCacheEntry {
    pub staging: HiArc<MemoryBlock>,
    pub device: HiArc<MemoryBlock>,
...
pub struct VulkanAllocatorLocalData {
    command_buffers: HiRc<CommandBuffers>,
}
...
pub struct VulkanAllocator {
    pub mem: Memory,
    pub staging_buffer_cache: HiArc<spin::Mutex<MemoryCache>>,
    pub staging_buffer_cache_image: HiArc<spin::Mutex<MemoryCache>>,
    pub vertex_buffer_cache: HiArc<spin::Mutex<MemoryCache>>,
    pub image_buffer_caches: BTreeMap<u32, HiArc<spin::Mutex<MemoryCache>>>,
...
impl VulkanAllocator {
    pub fn new(
        ...
        Ok(HiArc::new(parking_lot::Mutex::new(Self {
    ...
    pub fn get_image_memory(
        ...
        let mem: &HiArc<spin::Mutex<MemoryCache>>;
        match it {
            None => {
    ...
    pub fn get_vertex_buffer(
        &mut self,
        required_size: vk::DeviceSize,
    ) -> anyhow::Result<HiArc<MemoryBlock>, BufferAllocationError> {
        self.mem
    ...
    pub fn get_descriptor_pool_for_alloc(
        device: &HiArc<LogicalDevice>,
        descriptor_pools: &HiArc<parking_lot::Mutex<DeviceDescriptorPools>>,
        set_count: usize,
        layout: &HiArc<DescriptorSetLayout>,
    ) -> anyhow::Result<Vec<HiArc<DescriptorSets>>> {
        let mut cur_alloc_num = set_count;

        let mut pool_index_offset = 0;

        let mut res: Vec<HiArc<DescriptorSets>> = Vec::new();
    ...
    pub fn create_uniform_descriptor_sets(
        device: &HiArc<LogicalDevice>,
        descriptor_pools: &HiArc<parking_lot::Mutex<DeviceDescriptorPools>>,
        set_layout: &HiArc<DescriptorSetLayout>,
        set_count: usize,
        bind_buffer: &HiArc<Buffer>,
