...
impl CanvasSetupNative {
    pub fn create_image_views(
        ...
        let mut image_views: Vec<Arc<ImageView>> = Default::default();

        image_views.reserve(images.len());

        for i in 0..images.len() {
            let mut view_create_info = vk::ImageViewCreateInfo::default();
            view_create_info.image = images[i].img(&mut FrameResources::new(None));
    ...
    pub(crate) fn create_offscreen_images(
        ...
        let mut images: Vec<(Arc<Image>, MemoryImageBlock)> = Default::default();
