...
use crate::window::BackendWindow;

use super::vulkan_error::{CheckResult, Error};
...
impl Instance {
    pub fn new(
        ...
        let extensions = Self::get_vulkan_extensions(window)?;
    ...
    fn get_vulkan_extensions(window: &BackendWindow) -> anyhow::Result<Vec<String>> {
        let mut vk_extensions = Vec::<String>::new();

        let ext_list = window.enumerate_required_vk_extensions()?;
    ...
    fn create_vulkan_instance(
        ...
        if let Err(res_err) = res {
            ...
            if let Some(_err_crit) = crit_error_msg {
                return Err(anyhow!("Creating instance failed."));
            } else if res.is_err()
