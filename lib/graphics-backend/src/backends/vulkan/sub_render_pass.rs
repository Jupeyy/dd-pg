...
impl SubRenderPass {
        ...
        if !attrs.is_empty() {
            match &pipe_container.mode {
                PipelineContainerCreateMode::AtOnce => {
                    ...
                    for (index, (res_pipeline, res_pipe_layout)) in pipelines.iter().enumerate() {
                        ...
                        *pipe_item = PipelineContainerItem::Normal {
                            pipeline: *res_pipeline,
                            pipeline_layout: *res_pipe_layout,
                        }
                    }
                }
    ...
    pub fn new(
        ...
        additional_pipes.resize_with(name_offset as usize, || {
            PipelineContainer::new(logical_device.clone(), mode.clone())
        });

        let mut res = Self {
            standard_pipeline: PipelineContainer::new(logical_device.clone(), mode.clone()),
            standard_line_pipeline: PipelineContainer::new(logical_device.clone(), mode.clone()),
            standard_blur_pipeline: PipelineContainer::new(logical_device.clone(), mode.clone()),
            standard_3d_pipeline: PipelineContainer::new(logical_device.clone(), mode.clone()),
            blur_pipeline: PipelineContainer::new(logical_device.clone(), mode.clone()),
            prim_ex_pipeline: PipelineContainer::new(logical_device.clone(), mode.clone()),
            prim_ex_rotationless_pipeline: PipelineContainer::new(
                logical_device.clone(),
                mode.clone(),
            ),
            sprite_multi_pipeline: PipelineContainer::new(logical_device.clone(), mode.clone()),
            additional_pipes,
        };
