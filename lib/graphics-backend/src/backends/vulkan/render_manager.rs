...
use graphics_types::rendering::{BlendType, ColorMaskMode, State, StateTexture, StencilMode};
...
impl<'a> RenderManager<'a> {
    ...
    fn get_pipeline_and_layout(
        ...
        match pipe_item {
            super::vulkan_types::PipelineContainerItem::Normal {
                pipeline_layout,
                pipeline,
            ...
            super::vulkan_types::PipelineContainerItem::MaybeUninit {
                ...
                match &mut *pipe_and_layout {
                    Some((pipeline_layout, pipeline)) => (*pipeline, *pipeline_layout),
                    None => {
                        ...
                        let mut res = pipeline_manager
                            .create_graphics_pipeline_ex(&[creation_props.attr.clone()])
                            .unwrap();
                        let (pipeline, pipeline_layout) = res.remove(0);

                        *pipe_and_layout = Some((pipeline_layout, pipeline));
                        (pipeline, pipeline_layout)
                    }
                }
            }
...
impl<'a> BackendRenderInterface for RenderManager<'a> {
    ...
    fn get_is_textured(&self, state: &State) -> bool {
        if let StateTexture::Texture(_) | StateTexture::ColorAttachmentOfPreviousPass =
            state.texture_index
        {
            return true;
        }
