...
use crate::backends::vulkan::{
    image::Image, memory::SMemoryImageBlock, vulkan_allocator::IMAGE_BUFFER_CACHE_ID,
...
pub struct BackendSurfaceHeadless {
    images: Vec<(Arc<Image>, SMemoryImageBlock<IMAGE_BUFFER_CACHE_ID>)>,
}
...
impl BackendSurfaceHeadless {
    ...
    unsafe fn destroy_surface_images_headless(&mut self, _device: &mut Device) {
        /* TODO: self.images.iter_mut().for_each(|img| {
            device.ash_vk.device.device.destroy_image(img.img, None);
    ...
    pub fn create_swapchain(
        ...
        match self {
            ...
            Self::Headless { surface, .. } => Ok(BackendSwapchain::Headless {
                images: surface.images.iter().map(|img| img.0.image).collect(),
                device: device.clone(),
            }),
        }
    }
