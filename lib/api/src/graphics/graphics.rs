...
impl GraphicsStreamDataInterface for BackendStreamData {
    ...
    fn set_vertices_unsafe(&mut self, _vertices: &'static mut [SVertex]) {
...
impl GraphicsBackendInterface for GraphicsBackend {
    fn run_cmds(
        ...
        upload_param(2, self.actual_run_cmds);
        unsafe { run_cmds() };
        drop(stream_data_ref);
        *stream_data.borrow_mut().vertices_count_mut() = 0;
        drop(cmds);
        buffer.cmds.borrow_mut().clear();
    }

    fn mem_alloc(
        &self,
        _alloc_type: graphics_types::types::GraphicsMemoryAllocationType,
        req_size: usize,
    ...
    fn get_backend_mt(&self) -> Arc<dyn GraphicsBackendMtInterface + Sync + Send + 'static> {
        panic!("this is currently not supported in the wasm api")
    }
}
