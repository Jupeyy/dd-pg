...
use base_io_traits::fs_traits::{
    FileSystemInterface, FileSystemPath, FileSystemType, FileSystemWatcherItemInterface, HashMap,
    HashSet,
};
...
use virtual_fs::{host_fs, mem_fs, AsyncReadExt, AsyncWriteExt, OpenOptionsConfig};
...
impl FileSystemInterface for FileSystem {
    ...
    async fn entries_in_dir(&self, path: &Path) -> anyhow::Result<HashSet<String>> {
        let mut file_list = HashSet::<String>::default();
