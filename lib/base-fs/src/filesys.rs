...
use base_fs_traits::traits::{FileSystemInterface, MAX_PATH_LEN};
...
impl FileSystem {
    ...
    async fn files_or_dirs_of_dir_impl<'a, T>(
        &self,
        path: &str,
        file_read_cb: &'a mut T,
        fs_path: FileSystemPath,
    ...
    pub async fn files_or_dirs_of_dir<'a, T>(&self, path: &str, file_read_cb: &'a mut T)
    where
        T: FnMut(String) + Send + Sync,
    {
        let mut file_list = Vec::<String>::new();
     ...
     */
    pub fn watch_for_change(&self, path: &str, file: Option<&str>) -> FileSystemWatcherItem {
        let watch_path = self.get_path(path, FileSystemPath::OfType(FileSystemType::ReadWrite));
        FileSystemWatcherItem::new(&self.logger_fs_watch, &watch_path, file, &self.fs_watcher)
    }
}
