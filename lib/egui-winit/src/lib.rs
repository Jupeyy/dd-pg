...
use winit::keyboard::{self, Key, ModifiersKeyState, ModifiersState};
        ...
        match event {
            ...
            WindowEvent::KeyboardInput { event, .. } => {
                ...
                if let Some(text) = &event.text {
                    if event.state.is_pressed() {
                        let _consumed =
                            if is_printable_char(text.chars().next().unwrap()) && !is_mac_cmd {
                                self.egui_input
                                    .events
            ...
            WindowEvent::ModifiersChanged(state) => {
                ...
                self.egui_input.modifiers.ctrl =
                    !(state.state() & ModifiersState::CONTROL).is_empty();
