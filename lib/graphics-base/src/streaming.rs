...
use graphics_types::{
    ...
    rendering::{GlPoint, GlVertex, RenderMode, SVertex, State, WriteVertexAttributes},
...
pub trait DrawScopeImpl<const VERTEX_COUNT: usize> {
    ...
    fn wrap_clamp(&mut self) {
        self.get_draw_scope().wrap_clamp();
    }

    fn blend_additive(&mut self) {
...
impl<'a, const VERTEX_COUNT: usize> DrawStream<'a, VERTEX_COUNT> {
    pub fn new(
        stream_handle: &'a mut dyn GraphicsStreamHandleInterface,
...
pub trait GraphicsStreamHandleInterface {
