use base::counted_index::CountedIndexGetIndexUnsafe;
use graphics_traits::GraphicsStreamHandler;
use graphics_types::{
    command_buffer::{SColor, STexCoord},
    rendering::{GlPoint, GlVertex, SVertex, State, TextureIndex, WriteVertexAttributes},
...
impl<'a, const VERTEX_COUNT: usize> DrawScope<'a, VERTEX_COUNT> {
     ...
     */
    pub fn get_raw_handle(&mut self, number_of_prims: usize) -> &mut [GlVertex] {
    ...
    'a: 'b,
{
    ...
    fn set_texture(&'b mut self, tex_index: &TextureIndex) {
    ...
    fn get_canvas_mapping(&'b mut self) -> (f32, f32, f32, f32) {
        self.get_draw_scope().get_canvas_mapping()
    }

    fn get_raw_handle(&'b mut self, number_of_prims: usize) -> &'b mut [GlVertex] {
