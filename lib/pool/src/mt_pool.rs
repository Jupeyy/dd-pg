...
/// Since it uses a spin-lock, it can also safely be used inside a async context.
#[cfg_attr(feature = "enable_hiarc", derive(hiarc::Hiarc))]
#[derive(Debug)]
...
impl<T: Recyclable + Send> Pool<T> {
    ...
    pub fn with_capacity(capacity: usize) -> Self {
        Self {
            pool: Arc::new(PoolInner {
                pool: spin::Mutex::new(Vec::with_capacity(capacity)),
        ...
        F: FnMut() -> T,
    {
        let res = Self {
            pool: Arc::new(PoolInner {
                pool: spin::Mutex::new(Vec::with_capacity(new_size)),
