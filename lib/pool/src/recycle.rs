...
impl<T: Recyclable> Recycle<T> {
    ...
    /// Replaces self with a [Self::new_without_pool] item.
    /// This might be useful inside `Drop`-traits where
    /// the pool item is moved leaving default behind.
    /// Else this can cause undesired heap allocations.
    /// Note: If `T`'s `Default`-implementation causes a heap
    /// allocation, this does so too.
    pub fn take_by_item_without_pool(&mut self) -> Self {
        std::mem::replace(self, Self::new_without_pool())
    }
}
