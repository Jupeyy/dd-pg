                ...
                let mut play = || {
                    let plays = &mut self
                        ...
                        .or(self
                            ...
                            .get_mut(scene_id)
                            .map(|(scene, _)| scene))
                        .ok_or(anyhow!("scene with id {} does not exist", scene_id))?
                        .sounds
                        .get_mut(sound_id)
                        .ok_or(anyhow!("sound with id {} does not exist.", sound_id))?
                        .plays;

                    let play = plays
                        .get_mut(play_id)
                        .ok_or(anyhow!("sound play with id {play_id} does not exist"))?;
                ...
                SoundCommand::State(cmd) => match cmd {
                    SoundCommandState::SoundScene(cmd) => match cmd {
                        ...
                        SoundCommandSoundScene::Destroy { id } => {
                            if let None = self.scenes.remove(&id) {
                                self.inactive_scenes
                                    .remove(&id)
                                    .ok_or(anyhow!("scene not found."))?;
                            }
                        }
                        ...
                        SoundCommandSoundObject::Create { id, scene_id, mem } => {
                            let scene = self
                                ...
                                .or(self
                                    ...
                                    .get_mut(&scene_id)
                                    .map(|(scene, _)| scene))
                                .ok_or(anyhow!("scene did not exist"))?;

                            let res = scene
                                .sounds
                                .insert(id, Sound::new(&self.mem_allocator, mem)?)
                                .is_none();
                    ...
                    SoundCommandState::SoundListener(cmd) => match cmd {
                        ...
                        SoundCommandSoundListener::Destroy { id, scene_id } => {
                            ...
                            scene
                                .listeners
                                .remove(&id)
                                .ok_or(anyhow!("listener not found."))?;
                        }
                    },
                    SoundCommandState::Swap => {
                        // check which scenes were inactive and pause those
                        loop {
                            if let Some((&scene_id, scene)) = self.scenes.front() {
                                if scene.last_active_sound_frame < self.cur_sound_frame {
                                    if let Some(mut scene) = self.scenes.remove(&scene_id) {
                                        ...
                                        self.inactive_scenes
                                            .insert(scene_id, (scene, paused_state));
                                    }
                                } else {
                                    break;
                                }
                ...
                SoundCommand::Play(cmd) => match cmd {
                    SoundCommandPlay::Play {
                        ...
                        let sound = scene
                            .sounds
                            .get_mut(&sound_id)
                            .ok_or(anyhow!("sound does not exist."))?;
                        if let Some(paused_state) = paused_state {
                            if let Some(non_started_plays) =
                                paused_state.non_started_plays.get_mut(&sound_id)
                            {
                                non_started_plays.insert(play_id, props);
                            } else {
                                let mut plays = LinkedHashMap::default();
                                plays.insert(play_id, props);
                                paused_state.non_started_plays.insert(sound_id, plays);
                            }
                        } else {
                            sound.play(
                                play_id,
                    ...
                    SoundCommandPlay::Stop {
                        ...
                        if let Some(paused_state) = paused_state {
                            ...
                            sound
                                .plays
                                .remove(&play_id)
                                .ok_or(anyhow!("sound play with id {play_id} did not exist"))?;
                        }
                    }
                },
            }
