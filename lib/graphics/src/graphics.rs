...
use graphics_types::{
    command_buffer::{
        ...
        CommandSwap, CommandTextureCreate, CommandTextureDestroy, CommandUpdateViewport, Commands,
        CommandsRender, CommandsRenderMap, CommandsRenderQuadContainer, CommandsRenderStream,
        PrimType, RenderCommand, SQuadRenderInfo, SRenderSpriteInfo, StreamDataMax, TexFlags,
        TexFormat,
    },
...
impl<'a, B: GraphicsBackendInterface + Clone + 'static> StagingGraphics<'a, B> {
    pub fn load_texture(
        &mut self,
        width: usize,
        height: usize,
...
impl<B: GraphicsBackendInterface + Clone> Clone for GraphicsBackendHandle<B> {
    fn clone(&self) -> Self {
        Self {
...
impl<B: GraphicsBackendInterface> GraphicsBackendHandleInterface for GraphicsBackendHandle<B> {
    fn run_backend_buffer(&mut self, stream_data: &Rc<RefCell<dyn GraphicsStreamDataInterface>>) {
        self.backend.run_cmds(&mut self.backend_cmds, stream_data);
    }
    ...
    fn mem_alloc(
        ...
        self.backend.mem_alloc(alloc_type, req_size)
    }
}
...
impl<B: GraphicsBackendInterface + Clone> Clone for GraphicsStreamHandle<B> {
    fn clone(&self) -> Self {
        Self {
...
impl<B: GraphicsBackendInterface> GraphicsStreamHandleInterface for GraphicsStreamHandle<B> {
    ...
    fn flush_vertices(&mut self, state: &State, vertices_offset: usize, draw_mode: DrawModes) {
        ...
        if flush_vertices_impl(
            ...
            self.backend_handle
                .add_cmd(AllCommands::Render(CommandsRender::Stream(
...
impl<B: GraphicsBackendInterface + Clone> Clone for GraphicsQuadContainerHandle<B> {
    fn clone(&self) -> Self {
        Self {
...
impl<B: GraphicsBackendInterface + Clone + 'static> GraphicsQuadContainerHandleInterface
...
impl<B: GraphicsBackendInterface + Clone> Clone for GraphicsBufferObjectHandle<B> {
    fn clone(&self) -> Self {
        Self {
...
impl<B: GraphicsBackendInterface + Clone + 'static> GraphicsBufferObjectHandle<B> {
...
impl<B: GraphicsBackendInterface + Clone + 'static> GraphicsBufferObjectHandleInterface
...
impl<B: GraphicsBackendInterface + Clone> Clone for GraphicsTextureHandle<B> {
    fn clone(&self) -> Self {
        Self {
...
impl<B: GraphicsBackendInterface + Clone + 'static> GraphicsTextureHandle<B> {
...
impl<B: GraphicsBackendInterface + Clone + 'static> GraphicsTextureHandleInterface
...
impl<B: GraphicsBackendInterface + Clone + 'static> GraphicsBase<B> {
    pub fn new(
        backend: B,
        stream_data: Rc<RefCell<dyn GraphicsStreamDataInterface>>,
        window: WindowProps,
    ...
    pub fn render_tile_layer(
        ...
        self.backend_handle
            .add_cmd(AllCommands::Render(CommandsRender::Map(
    ...
    pub fn last_render_call_as_second_pass_transition(&mut self) {
        let cmd = self.backend_handle.backend_cmds.get_last_added();
        if let Some(AllCommands::Render(CommandsRender::Stream(CommandsRenderStream::Render(
            cmd,
        )))) = cmd
        {
            self.backend_handle
            ...
            self.backend_handle
                .add_cmd(AllCommands::Render(CommandsRender::Stream(
                    CommandsRenderStream::RenderFirstPassBlurred(cmd),
                )));
        }
    }

    pub fn do_screenshot(&mut self) -> anyhow::Result<Vec<u8>> {
        let mut width: u32 = 0;
        let mut height: u32 = 0;
        let mut dest_data_buffer: Vec<u8> = Default::default();
        let _ = self.backend_handle.backend.do_screenshot(
            &mut width,
            &mut height,
...
impl<B: GraphicsBackendInterface + Clone + 'static> GraphicsHandlesInterface for GraphicsBase<B> {
...
impl<B: GraphicsBackendInterface + Clone + 'static> GraphicsInterface for GraphicsBase<B> {}
