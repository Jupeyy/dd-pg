...
impl VulkanAllocator {
    ...
    pub fn get_staging_buffer(
        ...
        let res = self
            .mem
            .GetBufferBlockImpl::<{ THREADED_STAGING_BUFFER_CACHE_ID }, { 8 * 1024 * 1024 }, 3, true>(
                res_block,
    ...
    pub fn get_staging_buffer_image(
        ...
        let res = self.mem
             .GetBufferBlockImpl::<{ THREADED_STAGING_BUFFER_IMAGE_CACHE_ID }, { 8 * 1024 * 1024 }, 3, true>(
                 res_block,
    ...
    pub fn free_mem_raw(&mut self, mem: *mut c_void) {
        ...
        if let Some(mut entry) = res {
            ...
            if entry.block.is_cached {
                ...
                self.mem
                    .CleanBufferPair(0, &mut entry.block.buffer, &mut entry.block.buffer_mem)
            }
        } else {
            let res = self
    ...
    pub fn free_mem(&mut self, mem: &'static mut [u8]) {
        self.free_mem_raw(mem.as_ptr() as *mut c_void);
    }

    fn queue_free_mem_frame(&mut self, mem: &'static mut [u8], frame_index: u32) {
        ...
        pointers.push(std::ptr::NonNull::new(mem.as_ptr() as *mut c_void).unwrap());
    }
        ...
        F: FnOnce(&SDeviceMemoryBlock, &SMemoryHeapQueueElement),
    {
        ...
        let res = self
            .mapped_memory_cache
            .get(&std::ptr::NonNull::new(mem.as_ptr() as *mut c_void).unwrap());
