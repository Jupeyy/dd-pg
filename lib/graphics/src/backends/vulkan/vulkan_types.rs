...
const GPUINFO_STRING_SIZE: usize = 256;
/*
...
#[derive(Default, Clone)]
pub struct SBufferContainer {
    pub buffer_object_index: usize,
}
...
#[derive(Default, Clone)]
pub struct SStreamMemory<TName: Clone + Default> {
...
#[derive(Default, Clone)]
pub struct SSwapChainMultiSampleImage {
    pub image: vk::Image,
    pub img_mem: SMemoryImageBlock<IMAGE_BUFFER_CACHE_ID>,
    pub img_view: vk::ImageView,
}
...
#[derive(Clone)]
pub struct SDelayedBufferCleanupItem {
    pub buffer: vk::Buffer,
    pub mem: SDeviceMemoryBlock,
    pub mapped_data: *mut c_void,
}

impl Default for SDelayedBufferCleanupItem {
    fn default() -> Self {
        Self {
            buffer: Default::default(),
            mem: Default::default(),
...
type TCommandBufferCommandCallback = dyn Fn(*const c_void, &SRenderCommandExecuteBuffer) -> bool;
type TCommandBufferFillExecuteBufferFunc = dyn Fn(&mut SRenderCommandExecuteBuffer, *const c_void);

pub struct SCommandCallback {
    is_render_command: bool,
    fill_execute_buffer: Box<TCommandBufferFillExecuteBufferFunc>,
    command_cb: Box<TCommandBufferCommandCallback>,
    // command should be considered handled after it executed
    cmd_is_handled: bool,
}

impl SCommandCallback {
    pub fn new(
        fill_execute_buffer: &'static TCommandBufferFillExecuteBufferFunc,
        command_cb: &'static TCommandBufferCommandCallback,
    ) -> Self {
        Self {
            is_render_command: Default::default(),
            fill_execute_buffer: Box::new(fill_execute_buffer),
            command_cb: Box::new(command_cb),
            cmd_is_handled: true,
        }
    }
}
