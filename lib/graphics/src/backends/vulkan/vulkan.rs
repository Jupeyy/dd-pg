...
use graphics_types::{
    command_buffer::{
        AllCommands, CommandClear, CommandCopyBufferObject, CommandCreateBufferContainer,
        CommandCreateBufferObject, CommandDeleteBufferContainer, CommandDeleteBufferObject,
        ...
        CommandTextureDestroy, CommandTextureUpdate, CommandUpdateBufferContainer,
        CommandUpdateBufferObject, CommandUpdateViewport, Commands, CommandsRender,
        ERunCommandReturnTypes, PrimType, SBackendCapabilites, SQuadRenderInfo, SRenderSpriteInfo,
        ScreenshotBuffData, StreamDataMax, TexFlags, GRAPHICS_MAX_PARTICLES_RENDER_COUNT,
...
use crate::{
    ...
    backends::{
        vulkan::vulkan_types::{MAX_SUB_PASS_COUNT, RENDER_PASS_TYPE_COUNT},
        GraphicsBackendInterface, GraphicsBackendMemory, GraphicsBackendMtInterface,
    },
    image::resize,
};

use base::{
    counted_index::CountedIndexGetIndexUnsafe,
    system::{self, LogLevel, System, SystemLogGroup, SystemLogInterface},
};
...
const VK_BACKEND_PATCH: usize = 1;
...
use super::{
    ...
    vulkan_types::{
        ...
        SDeviceMemoryBlock, SFrameBuffers, SFrameUniformBuffers, SMemoryBlock, SMemoryImageBlock,
        SPipelineContainer, SRenderCommandExecuteBuffer, SRenderThread, SShaderFileCache,
        SShaderModule, SSwapImgViewportExtent, StreamMemory, SwapChainImage,
...
impl VulkanBackend {
    ...
    fn command_cb(
        ...
        match &cmd_param {
            AllCommands::Render(render_cmd) => match render_cmd {
                ...
                CommandsRender::RenderTex3D => todo!(),
                ...
                CommandsRender::Text => todo!(),
                CommandsRender::QuadContainer(cmd) => {
                    self.cmd_render_quad_container(cmd, exec_buffer)
                }
            ...
            AllCommands::Misc(misc_cmd) => match misc_cmd {
                ...
                Commands::TextTexturesCreate => todo!(),
                Commands::TextTexturesDestroy => todo!(),
                Commands::TextTextureUpdate => todo!(),
                ...
                Commands::UpdateBufferObject(cmd) => self.cmd_update_buffer_object(cmd),
                Commands::CopyBufferObject(cmd) => self.cmd_copy_buffer_object(cmd),
                Commands::DleteBufferObject(cmd) => self.cmd_delete_buffer_object(cmd),
                Commands::CreateBufferContainer(cmd) => self.cmd_create_buffer_container(cmd),
                Commands::DeleteBufferContainer(cmd) => self.cmd_delete_buffer_container(cmd),
                Commands::UpdateBufferContainer(cmd) => self.cmd_update_buffer_container(cmd),
    ...
    fn fill_execute_buffer(
        ...
        match &cmd {
            AllCommands::Render(render_cmd) => match render_cmd {
                ...
                CommandsRender::Text => {}
                CommandsRender::QuadContainer(cmd) => {
                    self.cmd_render_quad_container_fill_execute_buffer(exec_buffer, cmd)
                }
                CommandsRender::QuadContainerEx(cmd) => {
            ...
            AllCommands::Misc(misc_cmd) => match misc_cmd {
                ...
                Commands::TextTexturesCreate => {}
                Commands::TextTexturesDestroy => {}
                Commands::TextTextureUpdate => {}
                Commands::CreateBufferObject(_cmd) => {}
                Commands::RecreateBufferObject(_cmd) => {}
                Commands::UpdateBufferObject(_cmd) => {}
                Commands::CopyBufferObject(_cmd) => {}
                Commands::DleteBufferObject(_cmd) => {}
                Commands::CreateBufferContainer(_cmd) => {}
                Commands::DeleteBufferContainer(_cmd) => {}
                Commands::UpdateBufferContainer(_cmd) => {}
    ...
    fn prepare_presented_image_data_image(
        ...
        if self.get_presented_img_data_helper_image == vk::Image::null() || needs_new_img {
            ...
            self.get_presented_img_data_helper_image =
                unsafe { self.vk_device.create_image(&image_info, None) }.unwrap();
            ...
            self.get_presented_img_data_helper_mem.mem =
                unsafe { self.vk_device.allocate_memory(&mem_alloc_info, None) }.unwrap();
            unsafe {
                self.vk_device.bind_image_memory(
                    self.get_presented_img_data_helper_image,
                    self.get_presented_img_data_helper_mem.mem,
                    0,
                )
            ...
            let sub_resource_layout = unsafe {
                self.vk_device.get_image_subresource_layout(
                    self.get_presented_img_data_helper_image,
                    sub_resource,
                )
            };
            ...
            self.get_presented_img_data_helper_fence =
                unsafe { self.vk_device.create_fence(&fence_info, None) }.unwrap();
        }
    ...
    fn get_presented_image_data_impl(
        ...
        if (uses_rgba_like_format && self.last_presented_swap_chain_image_index != u32::MAX) {
            ...
            *height = viewport.height;
            ...
            unsafe {
                self.vk_device.invalidate_mapped_memory_ranges(&[mem_range]);
            }
    ...
    fn start_render_thread(&mut self, thread_index: usize) {
        ...
        if !list.is_empty() {
            ...
            let mut guard = thread.0.lock().unwrap();
        ...
        if self.thread_count > 1 {
            ...
            for thread_index in 0..self.thread_count - 1 {
                if self.thread_helper_had_commands[thread_index] {
                    ...
                    let mut _guard = render_thread.0.lock().unwrap();
    ...
    fn execute_memory_command_buffer(&mut self) {
        if self.device.used_memory_command_buffer[self.cur_image_index as usize] {
            ...
            unsafe {
                self.vk_device.end_command_buffer(*memory_command_buffer);
            }
    ...
    fn clear_frame_data(&mut self, frame_image_index: usize) {
        ...
        for buffer_pair in &mut self.device.frame_delayed_buffer_cleanups[frame_image_index] {
            if !buffer_pair.mapped_data.is_null() {
                unsafe {
                    self.vk_device.unmap_memory(buffer_pair.mem.mem);
                }
            }
        ...
        for texture_pair in &mut self.device.frame_delayed_text_textures_cleanups[frame_image_index]
        {
            Device::destroy_text_texture(
                &mut self.device.frame_delayed_buffer_cleanups,
                &mut self.device.image_buffer_caches,
                &self.vk_device,
                &mut texture_pair.0,
        ...
        self.device.frame_delayed_text_textures_cleanups[frame_image_index].clear();
    ...
    fn end_render_pass(&mut self) -> bool {
        self.finish_and_exec_render_threads();

        let command_buffer = &mut self.main_draw_command_buffers[self.cur_image_index as usize];
        unsafe { self.vk_device.cmd_end_render_pass(*command_buffer) };

        let res = unsafe { self.vk_device.end_command_buffer(*command_buffer) };
        if res.is_err() {
            self.error.lock().unwrap().set_error(
                EGFXErrorType::RenderRecording,
    ...
    fn update_texture(
        ...
        let mut staging_buffer = SMemoryBlock::<STAGING_BUFFER_IMAGE_CACHE_ID>::default();
        ...
        let tex = &self.device.textures[texture_slot];
    ...
    fn create_swap_chain(&mut self, old_swap_chain: &mut vk::SwapchainKHR) -> bool {
        ...
        *old_swap_chain = self.vk_swap_chain_khr;
        ...
        let res = unsafe { self.vk_swap_chain_ash.create_swapchain(&swap_info, None) };
        if res.is_err() {
            let crit_error_msg = self.check_res.check_vulkan_critical_error(
                res.unwrap_err(),
                &self.error,
    ...
    fn destroy_image_views(&mut self) {
        for imgage_view in &mut self.swap_chain_image_view_list {
            unsafe {
                self.vk_device.destroy_image_view(*imgage_view, None);
            }
        }
    ...
    fn destroy_images_for_second_pass(&mut self) {
        self.image_list_for_double_pass.iter_mut().for_each(|img| {
            Device::destroy_textured_standard_descriptor_sets(&self.vk_device, img, 0);
            Device::destroy_textured_standard_descriptor_sets(&self.vk_device, img, 1);
            unsafe {
                self.vk_device.destroy_image(img.img, None);
            }
            unsafe {
                self.vk_device.destroy_image_view(img.img_view, None);
            }
    ...
    fn create_framebuffers_impl(&mut self, double_pass: bool) -> bool {
        ...
        for i in 0..self.device.swap_chain_image_count {
            ...
            let res = unsafe { self.vk_device.create_framebuffer(&framebuffer_info, None) };
            if res.is_err() {
                self.error
                    .lock()
                    .unwrap()
    ...
    fn destroy_framebuffers(&mut self) {
        for frame_buffer in &mut self.framebuffer_list {
            unsafe {
                self.vk_device.destroy_framebuffer(*frame_buffer, None);
            }
        }
    ...
    fn create_shader_module(
        ...
        let res = unsafe { self.vk_device.create_shader_module(&create_info, None) };
        if res.is_err() {
            self.error
                .lock()
                .unwrap()
    ...
    fn create_descriptor_set_layouts(&mut self) -> bool {
        ...
        let mut layout_bindings = [sampler_layout_binding];
    ...
    fn create_shaders(
        ...
        shader_module.vk_device = self.vk_device.clone();
    ...
    fn create_command_pool(&mut self) -> bool {
        ...
        for i in 0..self.thread_count {
            let res = unsafe { self.vk_device.create_command_pool(&create_pool_info, None) };
            if res.is_err() {
                self.error
                    .lock()
                    .unwrap()
    ...
    fn create_command_buffers(&mut self) -> bool {
        ...
        let res = unsafe { self.vk_device.allocate_command_buffers(&alloc_info) };
        if res.is_err() {
            self.error
                .lock()
                .unwrap()
    ...
    fn create_sync_objects(&mut self) -> bool {
        ...
        for i in 0..self.device.swap_chain_image_count {
            ...
            let res4 = unsafe { self.vk_device.create_fence(&fence_info, None) };
    ...
    fn cleanup_vulkan_swap_chain(&mut self, force_swap_chain_destruct: bool) {
        ...
        self.standard_stencil_only_pipeline.destroy(&self.vk_device);
        self.standard_stencil_pipeline.destroy(&self.vk_device);
        self.standard_3d_pipeline.destroy(&self.vk_device);
        self.blur_pipeline.destroy(&self.vk_device);
    ...
    fn cleanup_vulkan<const IS_LAST_CLEANUP: bool>(&mut self) {
        ...
        if IS_LAST_CLEANUP {
            ...
            for image_buffer_cache in &mut self.device.image_buffer_caches {
                image_buffer_cache.1.destroy(&self.vk_device);
            }
    ...
    fn cmd_texture_destroy(&mut self, cmd: &CommandTextureDestroy) -> bool {
        let image_index: usize = cmd.slot;
        let texture = &mut self.device.textures[image_index];
    ...
    /*
                #[must_use] fn Cmd_TextTextures_Create(&mut self,cmd: &CommandTextTextures_Create) -> bool
                {
                ...
                #[must_use] fn Cmd_TextTextures_Destroy(&mut self,cmd: &CommandTextTextures_Destroy) -> bool
                {
                ...
                #[must_use] fn Cmd_TextTexture_Update(&mut self,cmd: &CommandTextTexture_Update) -> bool
                {
    ...
    */
    ...
    fn cmd_next_subpass(&mut self) -> bool {
        ...
        let command_buffer = &mut self.main_draw_command_buffers[self.cur_image_index as usize];
        unsafe {
            self.vk_device.cmd_next_subpass(
                *command_buffer,
                if self.thread_count > 1 {
                    vk::SubpassContents::SECONDARY_COMMAND_BUFFERS
                } else {
    ...
    fn cmd_clear(&mut self, exec_buffer: &SRenderCommandExecuteBuffer, cmd: &CommandClear) -> bool {
        if exec_buffer.clear_color_in_render_thread {
            ...
            let command_buffer = unsafe { &mut *command_buffer_ptr };
            unsafe {
                self.vk_device.cmd_clear_attachments(
                    *command_buffer,
                    &clear_attachments,
                    &clear_rects,
                );
    ...
    fn cmd_render_first_subpass_blurred(
        ...
        struct FakeTexture {}
        impl CountedIndexGetIndexUnsafe for FakeTexture {
    ...
    #[must_use]
    fn try_swap_and_screenshot(&mut self) -> anyhow::Result<ScreenshotBuffData> {
        if !self.next_frame() {
            return Err(anyhow!("Could not swap frame."));
        }

        let mut width = u32::default();
        let mut height = u32::default();
        let mut screenshot_helper: Vec<u8> = Default::default();
        if let Ok(format) = self.get_presented_image_data_impl(
            &mut width,
            &mut height,
            &mut screenshot_helper,
            false,
            true,
        ) {
            screenshot_helper.resize(width as usize * height as usize * 4, Default::default());
            Ok(ScreenshotBuffData {
                img_data: screenshot_helper,
                width,
                height,
                format,
            })
        } else {
            Err(anyhow!("Could not read presented image data"))
        }
    }
    ...
    */
    ...
    #[must_use]
    fn cmd_update_buffer_object(&mut self, cmd: &CommandUpdateBufferObject) -> bool {
        let buffer_index: usize = cmd.buffer_index;
        let offset = cmd.offset as vk::DeviceSize;
        let ptr_raw_upload_data = cmd.upload_data.as_ptr() as *const c_void;
        let data_size = cmd.upload_data.len() as vk::DeviceSize;

        let mut staging_buffer = SMemoryBlock::<STAGING_BUFFER_CACHE_ID>::default();
        if !self
            .device
            .get_staging_buffer(&mut staging_buffer, ptr_raw_upload_data, data_size)
        {
            return false;
        }

        let mem_block = self.device.buffer_objects[buffer_index]
            .buffer_object
            .mem
            .clone();
        let vertex_buffer = mem_block.buffer;
        if !self.device.memory_barrier(
            vertex_buffer,
            offset + mem_block.heap_data.offset_to_align as vk::DeviceSize,
            data_size,
        ...
        if !self.device.copy_buffer(
            staging_buffer.buffer,
            vertex_buffer,
            staging_buffer.heap_data.offset_to_align as vk::DeviceSize,
            offset + mem_block.heap_data.offset_to_align as vk::DeviceSize,
            data_size,
            self.cur_image_index,
        ) {
        ...
        self.device
            .upload_and_free_staging_mem_block(&mut staging_buffer, self.cur_image_index);
    ...
    #[must_use]
    fn cmd_copy_buffer_object(&mut self, cmd: &CommandCopyBufferObject) -> bool {
        let read_buffer_index: usize = cmd.read_buffer_index;
        let write_buffer_index: usize = cmd.write_buffer_index;
        let read_mem_block = &self.device.buffer_objects[read_buffer_index]
            .buffer_object
            .mem;
        let write_mem_block = &self.device.buffer_objects[write_buffer_index]
            .buffer_object
            .mem;
        let read_buffer = read_mem_block.buffer;
        let write_buffer = write_mem_block.buffer;

        let data_size = cmd.copy_size as vk::DeviceSize;
        let read_offset = cmd.read_offset as vk::DeviceSize
            + read_mem_block.heap_data.offset_to_align as vk::DeviceSize;
        let write_offset = cmd.write_offset as vk::DeviceSize
            + write_mem_block.heap_data.offset_to_align as vk::DeviceSize;
    ...
    #[must_use]
    fn cmd_create_buffer_container(&mut self, cmd: &CommandCreateBufferContainer) -> bool {
        let container_index: usize = cmd.buffer_container_index;
        while container_index >= self.device.buffer_containers.len() {
        ...
        self.device.buffer_containers[container_index].buffer_object_index =
    ...
    #[must_use]
    fn cmd_update_buffer_container(&mut self, cmd: &CommandUpdateBufferContainer) -> bool {
        let container_index: usize = cmd.buffer_container_index;
        self.device.buffer_containers[container_index].buffer_object_index =
            cmd.vert_buffer_binding_index;
    ...
    #[must_use]
    fn cmd_delete_buffer_container(&mut self, cmd: &CommandDeleteBufferContainer) -> bool {
        let container_index: usize = cmd.buffer_container_index;
        let delete_all_bo: bool = cmd.destroy_all_buffer_objects;
        if delete_all_bo {
    ...
    */
    ...
    fn buffer_container_fill_execute_buffer(
        &self,
        exec_buffer: &mut SRenderCommandExecuteBuffer,
        state: &State,
        buffer_container_index: usize,
    ...
    fn cmd_render_quad_container_fill_execute_buffer(
        &self,
        exec_buffer: &mut SRenderCommandExecuteBuffer,
        cmd: &CommandRenderQuadContainer,
    ) {
        self.buffer_container_fill_execute_buffer(
            exec_buffer,
            &cmd.state,
            cmd.buffer_container_index,
            1,
    ...
    fn cmd_render_quad_container(
        ...
        let command_buffer = unsafe { &mut *command_buffer_ptr };
        let (pipeline, pipe_layout) = Self::get_standard_pipe_and_layout(
            &mut self.standard_line_pipeline,
            &mut self.standard_pipeline,
            &mut self.standard_stencil_only_pipeline,
            &mut self.standard_stencil_pipeline,
    ...
    fn run_thread(
        ...
        while !guard.finished {
            ...
            if !guard.finished {
                ...
                for _next_cmd in &self.thread_command_lists[thread_index] {
                ...
                self.thread_command_lists[thread_index].clear();

                if !has_error_from_cmd
                    && self.used_thread_draw_command_buffer[thread_index + 1]
                        [self.cur_image_index as usize]
                {
                    let graphic_thread_command_buffer = &mut self.thread_draw_command_buffers
                        [thread_index + 1][self.cur_image_index as usize];
                    unsafe {
                        self.vk_device
                            .end_command_buffer(*graphic_thread_command_buffer);
                    }
            ...
            if is_verbose(&*self.dbg) && benchmark_render_threads {
...
impl GraphicsBackendInterface for VulkanBackend {
    ...
    fn start_commands(
        ...
        self.device.update_stream_vertex_buffer(
            ...
            backend_buffer.num_vertices * std::mem::size_of::<GlVertex>(),
            self.cur_image_index,
        );
    }
...
impl GraphicsBackendMtInterface for VulkanBackendMt {
    fn mem_alloc(
        ...
        match alloc_type {
            GraphicsMemoryAllocationType::Buffer => {
                ...
                GraphicsBackendMemory::Static(unsafe {
    ...
    fn mem_free(&self, mem: GraphicsBackendMemory) {
        if let GraphicsBackendMemory::Static(_) = mem {
            ...
            panic!("This should not happen: have you tried to create a custom memory?");
        }
    }
}
