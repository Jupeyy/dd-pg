...
pub fn dilate_image_sub(
    ...
    for y in 0..sh {
        let src_img_offset = ((y + y) * w * bpp) + (x * bpp);
        let dst_img_offset = y * sw * bpp;
...
pub fn texture_2d_to_3d(
    ...
    *target_3d_img_height = image_height / split_count_height;
    ...
    let target_image_full_width = *target_3d_img_width as usize * image_color_channel_count as usize;
    thread_pool.install(|| {
        target_3d_img_buff_data
            ...
            .for_each(|(index, write_chunk)| {
                ...
                let y_src = index % *target_3d_img_height
                    + ((index / (split_count_width * *target_3d_img_height)) * *target_3d_img_height);
