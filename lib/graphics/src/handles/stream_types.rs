...
impl Into<[SVertex; 2]> for StreamedLine {
    fn into(self) -> [SVertex; 2] {
        self.vertices
    }
}
...
impl Into<[SVertex; 3]> for StreamedTriangle {
    fn into(self) -> [SVertex; 3] {
        self.vertices
    }
}
...
impl StreamedQuad {
    ...
    pub fn rotate_pos(mut self, rot: f32) -> Self {
        ...
        center.x = self.vertices[0].pos.x + (self.vertices[1].pos.x - self.vertices[0].pos.x) / 2.0;
        center.y = self.vertices[0].pos.y + (self.vertices[2].pos.y - self.vertices[0].pos.y) / 2.0;
...
impl Into<[SVertex; 4]> for StreamedQuad {
    fn into(self) -> [SVertex; 4] {
        self.vertices
    }
}
