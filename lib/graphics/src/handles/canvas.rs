pub mod canvas {
    ...
    use graphics_types::{
        commands::{
            AllCommands, CommandSwitchCanvasMode, CommandSwitchCanvasModeType,
            CommandUpdateViewport, CommandsMisc,
        },
        types::WindowProps,
    };
    ...
    impl GraphicsCanvasHandle {
        ...
        pub fn switch_canvas(&mut self, mode: CommandSwitchCanvasModeType) {
            match &mode {
                CommandSwitchCanvasModeType::Offscreen { id, .. } => {
                    self.cur_canvas_mode = GraphicsCanvasMode::Offscreen(*id);
                }
        ...
        fn get_cur_canvas(&self) -> &GraphicsCanvas {
            match self.cur_canvas_mode {
                ...
                GraphicsCanvasMode::Offscreen(_) => &self.canvases.offscreen,
            }
        }
