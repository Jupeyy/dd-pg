...
use graphics::{
    ...
    handles::{backend::backend::GraphicsBackendHandle, stream::stream::GraphicsStreamHandle},
};
...
impl WasmGraphicsLogicImpl {
    ...
    fn run_cmds(
        ...
        stream_data.deserialize_uniform_instances_from_vec(uniform_instances);
    ...
    pub fn get_wasm_graphics_logic_imports(
        ...
        fn run_cmds(
            ...
            let actually_run_cmds =
                read_param(instance.as_ref().unwrap(), &mut store, &mut param2, 3);
