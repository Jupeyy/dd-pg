...
pub struct WasmGraphicsLogicImpl {
    ...
    pub graphics_backend: Rc<RefCell<GraphicsBackend>>,
...
impl WasmGraphicsLogicImpl {
    ...
    fn run_cmds(
        ...
        drop(stream_data);
        if must_flush_cmds {
            self.graphics_backend.borrow_mut().run_cmds(
