...
impl State {
    ...
    pub fn clear_texture(&mut self) {
        self.texture_index = StateTexture::None;
    }

    pub fn set_texture(&mut self, tex_index: &dyn SharedIndexGetIndexUnsafe) {
        self.texture_index = StateTexture::Texture(tex_index.get_index_unsafe());
    }

    /// set's the color attachment of the previous render pass as texture
    pub fn set_color_attachment_texture(&mut self) {
        self.texture_index = StateTexture::ColorAttachmentOfPreviousPass;
    }
    ...
    /// like `clip` but clamps all values to the window size
    pub fn clip_clamped(
        &mut self,
        x: i32,
        y: i32,
        ...
        self.clip_x = x.clamp(0, window_w as i32);
        self.clip_y = y.clamp(0, window_h as i32);
        self.clip_w = w.clamp(0, window_w - self.clip_x as u32);
        self.clip_h = h.clamp(0, window_h - self.clip_y as u32);
    }
    ...
    /// automatic rounding to nearest integer that can be used in `clip`
    ...
    /// like `clip` but with automatic rounding to nearest integer
    ...
    pub fn clip_auto_rounding(&mut self, x: f32, y: f32, w: f32, h: f32) {
        ...
        (self.clip_x, self.clip_y, self.clip_w, self.clip_h) =
            Self::auto_round_clipping(x, y, w, h);
    }
...
#[repr(C)]
#[derive(Debug, Default, Copy, Clone, Encode, Decode, Serialize, Deserialize)]
