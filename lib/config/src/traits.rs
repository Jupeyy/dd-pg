...
impl ConfigInterface for String {
    ...
    fn try_set_from_str(
        ...
        if path.is_empty() {
            if let Some(val) = val {
                // validate
                if !if let Some(ConfigValue::String {
                    min_length,
                    max_length,
                }) = conf_val
                {
                    ...
                    return Err(ConfigFromStrErr::PathErr(
                        ConfigFromStrPathErr::ValidationError(
                            "The min/max length of the string was reached".to_string(),
                        ),
                    ));
                }
                *self = val;
...
impl<T: ConfigInterface + Default + ToString> ConfigInterface for Option<T> {
    ...
    fn try_set_from_str(
        ...
        if path.is_empty() {
            if let Some(val_inner) = &val {
                if val_inner.is_empty() {
                    *self = None;
                    ...
                    let mut t = T::default();
                    let v = t.try_set_from_str(path, modifier, val, conf_val, flags)?;
                    *self = Some(t);
                    return Ok(v);
                }
            }
            ...
            match self {
                Some(inner_val) => inner_val.try_set_from_str(path, modifier, val, conf_val, flags),
                None => {
                    let mut t = T::default();
                    let res = t.try_set_from_str(path, modifier, val, conf_val, flags)?;
        ...
        if path.is_empty() {
            if let Some(val) = val {
                *self = ConfRgb::from_html_color_code(&val)
                    .or_else(|err_html| {
                        ConfRgb::from_css_rgb_fn(&val).map_err(|err| {
                            anyhow!("no html or css like color tag found: {err_html}. {err}")
                        })
                    })
