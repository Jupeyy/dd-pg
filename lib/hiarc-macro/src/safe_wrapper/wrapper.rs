...
pub(crate) fn hiarc_safer_wrapper(
    ...
    match &mut base {
        Item::Impl(i) => {
            ...
            i.items.iter_mut().for_each(|item| if let syn::ImplItem::Fn(f) = item {
                f.attrs.retain_mut(|attr| {
                    let meta = attr.meta.to_token_stream().to_string();
                    if meta == "hiarc_force_impl" {
                        false
                    } else { meta != "hiarc_trait_is_immutable_self" }
                });
            });
        ...
        Item::Struct(s) => {
            ...
            attrs.iter_mut().for_each(|attr| {
                if let Meta::List(l) = &mut attr.meta {
                    if *l.path.get_ident().unwrap() == "derive" && l.tokens.to_string().contains("Hiarc") {
